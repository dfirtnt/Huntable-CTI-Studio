...
class LLMBenchmark:
    ...
    async def test_provider(
        ...
        try:
            ...
            return {
                ...
                'response': response.get('response', '')[:100] + '...' if len(response.get('response', '')) > 100 else response.get('response', ''),
                'error': None
            }
    ...
    async def benchmark_provider(self, provider: str, model_name: str) -> Dict[str, Any]:
        ...
        elif provider == "llamacpp" and not self.service._is_llamacpp_available():
            return {'available': False, 'reason': 'llama.cpp not available or model not found'}
        
        results = []
        
        # Cold start test (first request)
        console.print(f"  [yellow]Cold start test...[/yellow]")
        ...
        console.print(f"  [yellow]Warm requests test...[/yellow]")
        warm_results = []
        ...
        cold_start_time = cold_result['response_time']
        avg_warm_time = sum(r['response_time'] for r in warm_results if r['success']) / len([r for r in warm_results if r['success']]) if warm_results else 0
    ...
    def save_results(self, results: Dict[str, Any]):
        ...
        with open(md_file, 'w') as f:
            f.write(f"# LLM Provider Benchmark Report\n\n")
            ...
            for provider, result in results.items():
                if not result.get('available', False):
                    f.write(f"| {provider.title()} | {self.service._get_model_name(provider)} | ❌ No | - | - | - | - |\n")
                elif not result.get('successful', False):
                    f.write(f"| {provider.title()} | {self.service._get_model_name(provider)} | ⚠️ Failed | {result.get('cold_start_time', 0):.2f}s | - | {result.get('success_rate', 0):.1%} | - |\n")
                else:
                    f.write(f"| {provider.title()} | {self.service._get_model_name(provider)} | ✅ Yes | {result.get('cold_start_time', 0):.2f}s | {result.get('avg_warm_time', 0):.2f}s | {result.get('success_rate', 0):.1%} | {result.get('avg_response_length', 0):.0f} |\n")
