...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/tmp/simhash_backfill.log"), logging.StreamHandler()],
)
logger = logging.getLogger(__name__)


def run_psql_command(sql: str, timeout: int = 30) -> list[dict] | None:
    """Run PostgreSQL command and return results."""
    try:
        cmd = ["docker", "exec", "cti_postgres", "psql", "-U", "cti_user", "-d", "cti_scraper", "-t", "-c", sql]
        ...
        for line in lines:
            if line.strip():
                ...
                parts = line.split("|")
                if len(parts) >= 3:
                    row = {"id": parts[0].strip(), "title": parts[1].strip(), "content": parts[2].strip()}
                    data.append(row)
...
def backfill_simhash(batch_size: int = 100, max_batches: int | None = None):
