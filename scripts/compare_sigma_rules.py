...
def canonicalize_detection_logic(detection: dict[str, Any], atoms: list[Atom]) -> dict[str, Any]:
    ...
    if " or " in condition_lower or "|" in condition:
...
def _atom_to_key(atom: dict[str, Any] | Atom) -> str:
...
def _compute_service_penalty(service1: str | None, service2: str | None) -> float:
...
def compute_weighted_similarity(
    atom_jaccard: float, logic_similarity: float, service_penalty: float = 0.0, filter_penalty: float = 0.0
) -> float:
...
def compare_rules(rule1_path: str, rule2_path: str) -> dict[str, Any]:
    ...
    if atom_jaccard == 1.0 and service_penalty == 0.0 and filter_penalty == 0.0:
        ...
        weighted_sim = compute_weighted_similarity(atom_jaccard, logic_similarity, service_penalty, filter_penalty)

    # Build result
    result = {
        "rule1": {"file": rule1_path, "id": rule1_data.get("id", "N/A"), "title": rule1_data.get("title", "N/A")},
        "rule2": {"file": rule2_path, "id": rule2_data.get("id", "N/A"), "title": rule2_data.get("title", "N/A")},
        "similarity_metrics": {
