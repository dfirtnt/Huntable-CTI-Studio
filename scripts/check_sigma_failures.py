...
def check_sigma_failures(days: int = 7):
    ...
    try:
        ...
        executions = db_session.query(AgenticWorkflowExecutionTable).filter(
            AgenticWorkflowExecutionTable.created_at >= cutoff_date
        ...
        for exec in executions:
            ...
            has_no_rules = exec.sigma_rules is None or (isinstance(exec.sigma_rules, list) and len(exec.sigma_rules) == 0)
            error_mentions_sigma = exec.error_message and (
                'sigma' in exec.error_message.lower() or 
                'validation' in exec.error_message.lower() or
                'no valid' in exec.error_message.lower()
            )
            ...
            queued_count = db_session.query(SigmaRuleQueueTable).filter(
                SigmaRuleQueueTable.workflow_execution_id == exec.id
            ).count()
