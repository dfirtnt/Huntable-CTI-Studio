...
def compare_similarity_methods(sample_rules: int = 5, rank_diff_threshold: int = 5):
    ...
    """
    logger.info(f"Comparing similarity methods on {sample_rules} sample rules (rank diff threshold: {rank_diff_threshold})...")
    ...
    try:
        ...
        rules = session.query(SigmaRuleTable).filter(
            SigmaRuleTable.embedding.isnot(None),
            SigmaRuleTable.title_embedding.isnot(None)
        ).limit(sample_rules).all()
        ...
        with open(output_file, 'w') as f:
            json.dump({
                ...
                'summary': {
                    ...
                    'avg_rank_change': sum(m['rank_diff'] for m in all_mismatches if m['rank_diff']) / len(all_mismatches) if all_mismatches else 0,
...
def main():
    ...
    parser.add_argument('--rank-diff-threshold', type=int, default=5, help='Threshold for logging ranking differences (default: 5)')
