...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(
            f"  With threat hunting score: {result['with_score']} ({result['with_score'] / result['total'] * 100:.1f}%)"
        )
        print(
            f"  Without threat hunting score: {result['without_score']} ({result['without_score'] / result['total'] * 100:.1f}%)"
        )
        print(
            f"  Without metadata: {result['without_metadata']} ({result['without_metadata'] / result['total'] * 100:.1f}%)"
        )
