...
class DatabaseRestore:
    ...
    def create_database_snapshot(self) -> Optional[str]:
    ...
    def restore_database(self, backup_path: Path, force: bool = False) -> bool:
        ...
        try:
            ...
            try:
                ...
                subprocess.run([
                    'docker', 'exec', 'cti_postgres', 'rm', '-f', '/tmp/restore.sql'
                ], capture_output=True)
            except Exception:
                pass
    ...
    def show_restore_stats(self):
        ...
        try:
            ...
            result = subprocess.run([
                ...
                '-U', self.db_config['user'],
                '-d', self.db_config['database'],
                '-t', '-c', "SELECT pg_size_pretty(pg_database_size('cti_scraper'));"
            ], capture_output=True, text=True, timeout=30)
    ...
    def rollback_from_snapshot(self, snapshot_path: Path) -> bool:
        ...
        try:
            ...
            with gzip.open(snapshot_path, 'rt') as f_in:
                with open(temp_sql_path, 'w') as f_out:
                    shutil.copyfileobj(f_in, f_out)
            ...
            temp_sql_path.unlink(missing_ok=True)
            subprocess.run([
                'docker', 'exec', 'cti_postgres', 'rm', '-f', '/tmp/rollback.sql'
            ], capture_output=True)
