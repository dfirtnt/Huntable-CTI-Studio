...
def is_likely_edr_query(content: str, indicators: Set[str]) -> Tuple[bool, str]:
    ...
    if len(indicators) >= 2:
        ...
        table_indicators = {
            ...
            "EmailEvents", "ProcessRollup2", "ProcessCreate",
            "Endpoint.Processes", "logs-endpoint.events.process",
            "EventType = Process"
        }
    ...
    if len(indicators) == 1:
        ...
        strong_indicators = {
            ...
            "UrlClickEvents", "AlertInfo", "ProcessRollup2", "ProcessCreate",
            "ScriptControlScanTelemetry", "CommandHistory", "RegistryOperation",
            "ModuleLoad", "FileCreate", "FileWrite", "FileDelete",
            "NetworkConnectIP4", "NetworkConnectIP6",
            "Endpoint.Processes", "Endpoint.Registry", "Endpoint.Filesystem",
            "logs-endpoint.events.process", "logs-endpoint.events.file",
            "EventType = Process", "EventType = File"
        }
...
def analyze_articles(articles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for indicator in results["indicator_counts"]:
        articles_with_indicator = [
            a for a in articles 
            if indicator in find_indicators_in_content(a.get("content", ""))
        ]
        edr_count = sum(
            1 for a in articles_with_indicator
            if is_likely_edr_query(a.get("content", ""), 
...
def print_report(results: Dict[str, Any]):
    ...
    perfect = [
        (ind, stats) for ind, stats in results["indicator_precision"].items()
    ...
    print("=" * 80)
    all_sorted = sorted(
        results["indicator_precision"].items(),
        key=lambda x: (x[1]["precision"], x[1]["total_articles"]),
        reverse=True
    )
