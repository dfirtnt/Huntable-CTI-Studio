...
def fix_execution_statuses(dry_run: bool = True):
    ...
    try:
        ...
        executions = (
            ...
            .filter(AgenticWorkflowExecutionTable.status.in_(["completed", "failed"]))
            .all()
        )
        ...
        for execution in executions:
            ...
            if (
                ...
                and has_no_rules
                and execution.current_step == "promote_to_queue"
                and queued_count == 0
            ):
                ...
                fixed = True
                logger.info(
                    f"Execution {execution.id}: Fixed 'completed' -> 'failed' (SIGMA validation failure, {queued_count} queued rules)"
                )
            ...
            # If status is failed but only due to ranking threshold (no error message or generic)
                ...
                execution.status == "failed"
                and execution.current_step == "rank_article"
                and (
                    not execution.error_message
                    or "threshold" in execution.error_message.lower()
                    or execution.ranking_score is not None
                )
            ):
            ...
            if fixed:
                if not dry_run:
                    db_session.commit()
                    logger.info(
                        f"Execution {execution.id}: Updated status={execution.status}, step={execution.current_step}"
                    )
                else:
                    logger.info(
                        f"Execution {execution.id}: Would update status={execution.status}, step={execution.current_step} (DRY RUN)"
                    )
                    # Revert for dry run
