...
def find_corrupted_articles() -> list[int]:
    ...
    with db.get_session() as session:
        ...
        all_articles = (
            session.query(ArticleTable.id, ArticleTable.title, ArticleTable.content)
    ...
    with db.get_session() as session:
        for article_id in article_ids:
            try:
                ...
                session.execute(
                    text("DELETE FROM article_annotations WHERE article_id = :article_id"), {"article_id": article_id}
                )

                # Delete from content_hashes table
                session.execute(
                    text("DELETE FROM content_hashes WHERE article_id = :article_id"), {"article_id": article_id}
                )

                # Delete from simhash_buckets table
                session.execute(
                    text("DELETE FROM simhash_buckets WHERE article_id = :article_id"), {"article_id": article_id}
                )
