    ...
    if apply_content_filter:
        try:
            ...
            filter_result = content_filter.filter_content(
                content,
                min_confidence=content_filter_threshold,
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare training data for huntable-windows-BERT using LOLBAS scores"
    )
    parser.add_argument(
        '--min-hunt-score',
        type=float,
        default=0.0,
        help='Minimum hunt score for articles (default: 0.0 - include all)'
    )
    parser.add_argument(
        '--limit',
        type=int,
    ...
    parser.add_argument(
        '--balance',
        action='store_true',
        help='Balance positive/negative samples (limit majority class)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help='Content filter confidence threshold (default: 0.8)'
    )
    parser.add_argument(
        '--eval-split',
        type=float,
        default=0.2,
        help='Fraction of data to reserve for evaluation (default: 0.2)'
    )
    parser.add_argument(
        '--eval-output',
        type=Path,
        default=None,
        help='Output path for evaluation set (if --eval-split > 0)'
    )
    ...
    if positive_count > 0 and negative_count > 0:
        ...
        if ratio > 3:
            print(f"  ⚠️  High imbalance - consider using --balance flag")
    
    # Sample statistics
    if positive_count > 0:
        positive_lolbas_avg = sum(
            ex['lolbas_count'] for ex in training_data if ex['label'] == 1
        ) / positive_count
        print(f"\nPositive samples:")
