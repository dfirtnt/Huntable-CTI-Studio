...
from scripts.prepare_huntable_windows_training_data import (
    load_articles,
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare separate evaluation dataset (hold-out set)"
    )
    parser.add_argument(
        '--min-hunt-score',
        type=float,
        default=0.0,
        help='Minimum hunt score for articles'
    )
    parser.add_argument(
        '--limit',
        type=int,
        default=200,
        help='Number of articles for evaluation set'
    )
    parser.add_argument(
        '--min-lolbas-for-positive',
        type=int,
        default=1,
        help='Minimum LOLBAS matches for positive label'
    )
    parser.add_argument(
        '--output',
        type=Path,
    ...
    if args.exclude_training_ids.exists():
        with open(args.exclude_training_ids, 'r') as f:
    ...
    print(f"  python scripts/evaluate_huntable_windows_model.py --model models/huntable_windows_classifier.pkl --eval-data {args.output}")
