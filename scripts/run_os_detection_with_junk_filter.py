...
async def run_os_detection_with_junk_filter():
    ...
    try:
        ...
        for article_id in ARTICLE_IDS:
            # Get article
            article = db_session.query(db_manager.ArticleTable).filter(db_manager.ArticleTable.id == article_id).first()
            ...
            try:
                ...
                results.append(
                    {"article_id": article_id, "title": article.title, "url": article.canonical_url, "error": str(e)}
                )
        ...
        for r in results:
            if "error" in r:
                ...
                print(
                    f"{r['article_id']:<6} {r['detected_os']:<12} {r['method'][:24]:<25} {r['confidence']:<12} {reduction_str:<18}"
                )
        ...
        for r in results:
            ...
            if "error" in r:
                ...
                print(
                    f"  Content: {r['original_length']:,} → {r['filtered_length']:,} chars ({r['reduction_pct']:.1f}% reduction)"
                )
