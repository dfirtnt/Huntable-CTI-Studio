...
from scripts.eval_os_detection_manual import (
    MANUAL_TEST_DATA,
    calculate_accuracy,
...
async def evaluate_model(
    model_key: str, model_config: dict[str, Any], url_to_id: dict[str, int], junk_filter_threshold: float = 0.8
) -> dict[str, Any]:
    ...
    try:
        for test_item in MANUAL_TEST_DATA:
            ...
            try:
                ...
                print(
                    f"  {status} Article {article_id}: {detected_os} (truth: {human_truth}, method: {result['method']})"
                )
            except Exception as e:
                predictions.append("Unknown")
                results.append({"article_id": article_id, "url": url, "error": str(e)})
...
async def main():
    ...
    for model_key in args.models:
        ...
        try:
            result = await evaluate_model(model_key, model_config, url_to_id, args.junk_filter_threshold)
            ...
            traceback.print_exc()
            all_results[model_key] = {"model_key": model_key, "error": str(e)}
    ...
    sorted_results = sorted(
        [(k, v) for k, v in all_results.items() if "accuracy" in v], key=lambda x: x[1]["accuracy"], reverse=True
    )
