...
def parse_backup_timestamp(backup_name: str) -> datetime | None:
    ...
    try:
        ...
        if backup_name.startswith("cti_scraper_backup_"):
        ...
        if backup_name.startswith("pre_restore_snapshot_"):
...
def get_backup_metadata(backup_path: Path) -> dict | None:
...
def find_backups(backup_dir: Path) -> list[tuple[Path, datetime, float, dict | None]]:
...
def select_backups_to_keep(
    backups: list[tuple[Path, datetime, float, dict | None]], retention: dict
) -> list[tuple[Path, datetime, float, dict | None]]:
...
def apply_size_limit(
    backups_to_keep: list[tuple[Path, datetime, float, dict | None]], max_size_gb: float
) -> list[tuple[Path, datetime, float, dict | None]]:
...
def prune_backups(
    backup_dir: str | None = None, retention: dict | None = None, dry_run: bool = False, force: bool = False
) -> dict:
...
def main():
    ...
    retention = {"daily": args.daily, "weekly": args.weekly, "monthly": args.monthly, "max_size_gb": args.max_size_gb}

    # Prune backups
    result = prune_backups(backup_dir=args.backup_dir, retention=retention, dry_run=args.dry_run, force=args.force)
