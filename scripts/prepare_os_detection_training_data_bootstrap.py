...
def create_training_data_from_results(results_file: Path, min_confidence: str = "low") -> list[dict[str, Any]]:
    ...
    for article_id_str, detections in results.items():
        ...
        print(
            f"  Article {article_id}: {detected_os} → {os_label} (conf={confidence}, sim={similarities.get('Windows', 0):.3f})"
        )
