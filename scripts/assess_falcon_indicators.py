...
def is_likely_falcon_query(content: str, indicators: Set[str]) -> Tuple[bool, str]:
    ...
    if len(indicators) >= 2:
        ...
        event_indicators = {
            "ProcessRollup2", "ProcessCreate", "event_simpleName",
            "NetworkConnectIP4", "NetworkConnectIP6", "DnsRequest",
            "FileCreate", "FileWrite", "FileWritten", "FileDelete",
            "RegistryOperation", "ModuleLoad", "ScriptControlScanTelemetry",
            "CommandHistory"
        }
        field_indicators = {
            "ImageFileName", "ParentImageFileName", "ParentBaseFileName",
            "GrandparentImageFileName", "CommandLine", "ParentCommandLine",
            "ProcessId", "ParentProcessId", "UserSid", "IntegrityLevel",
            "SessionId", "RemoteAddressIP4", "RemoteAddressIP6",
            "LocalAddressIP4", "FileHashSha256", "SHA256HashData",
            "FileHashMd5", "MD5HashData", "FileHashSha1", "SHA1HashData",
            "RegistryKeyName", "RegistryValueName", "ScriptContent",
            "ScriptFileName", "ModuleFileName"
        }
        ...
        if has_event and (has_field or has_function):
            return True, f"Event type + field/function combination"
    ...
    if len(indicators) == 1:
        ...
        if indicator in event_types:
            ...
            if re.search(rf'\b{re.escape(indicator)}\b.*\|\s*(where|groupBy|filter)', 
        ...
        unique_fields = {
            "ImageFileName", "ParentImageFileName", "ParentBaseFileName",
            "GrandparentImageFileName", "UserSid", "IntegrityLevel",
            "SessionId", "RemoteAddressIP4", "RemoteAddressIP6",
            "LocalAddressIP4", "FileHashSha256", "SHA256HashData",
            "FileHashMd5", "MD5HashData", "FileHashSha1", "SHA1HashData",
            "RegistryKeyName", "RegistryValueName", "RegistryOperationType",
            "ScriptContent", "ScriptFileName", "ScriptEngine",
            "ModuleFileName", "ConnectionDirection"
        }
...
def analyze_articles(articles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for indicator in FALCON_INDICATORS:
        ...
        falcon_count = sum(
            ...
            if is_likely_falcon_query(a.get("content", ""), 
