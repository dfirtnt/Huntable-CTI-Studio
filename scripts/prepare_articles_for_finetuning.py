...
def get_article_by_id(db_session: Session, article_id: int) -> Optional[ArticleTable]:
    """Get article by ID."""
...
def analyze_article_content(article: ArticleTable) -> Dict[str, Any]:
    ...
    observable_keywords = [
        'command', 'cmd', 'powershell', 'cmd.exe', 'process',
        'registry', 'HKCU', 'HKLM', 'reg add', 'reg set',
        'service', 'sc create', 'sc start', 'schtasks',
        'file path', 'temp', '%TEMP%', '%APPDATA%',
        'event log', 'EventID', 'Sysmon', 'Event ID',
        'base64', 'encoded', 'obfuscated', 'decoded',
        'process chain', 'parent process', 'child process',
        'execution', 'launch', 'spawn', 'create process'
    ]
...
def check_existing_extraction(article_id: int, db_session: Session) -> Optional[Dict[str, Any]]:
    ...
    execution = db_session.query(AgenticWorkflowExecutionTable).filter(
        ...
        AgenticWorkflowExecutionTable.created_at.desc()
    ).first()
...
def create_training_example(
    ...
    manual_observables: Optional[List[Dict[str, Any]]] = None,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare specific articles for Extract Agent fine-tuning'
    )
    parser.add_argument(
        '--article-ids',
        type=int,
        nargs='+',
        required=True,
        help='Article IDs to prepare'
    )
    parser.add_argument(
        '--analyze-only',
        action='store_true',
        help='Only analyze articles, do not create training examples'
    )
    parser.add_argument(
        '--output',
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help='Minimum suitability score to include (0-10, default: 3.0)'
    )
    parser.add_argument(
        '--use-existing-extractions',
        action='store_true',
        help='Use existing extraction results if available'
    )
    parser.add_argument(
        '--apply-junk-filter',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help='Junk filter confidence threshold (default: 0.8)'
    )
    ...
    try:
        ...
        for article_id in args.article_ids:
            ...
            if analysis['keyword_breakdown']:
                top_keywords = sorted(
                    analysis['keyword_breakdown'].items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:5]
            ...
            if existing:
                ...
                print(f"   ⚠️  No existing extraction found")
            
            # Create training example if suitable
            if not args.analyze_only:
                if analysis['suitability_score'] >= args.min_suitability:
                    extraction_result = existing['extraction_result'] if existing and args.use_existing_extractions else None
                    ...
                    print(f"   ⏭️  Skipped (suitability score {analysis['suitability_score']:.1f} < {args.min_suitability})")
        ...
        print(f"Articles suitable (score >= {args.min_suitability}): {len([a for a in analyses if a['suitability_score'] >= args.min_suitability])}")
        ...
        if training_examples and not args.analyze_only:
            ...
            total_observables = sum(
                len(ex.get('extraction_result', {}).get('observables', []))
