...
def find_article_by_url(db_session, url: str) -> ArticleTable | None:
...
def get_existing_raw_response(article: ArticleTable, db_session) -> str | None:
    ...
    try:
        ...
        execution = (
            ...
            .order_by(desc(AgenticWorkflowExecutionTable.completed_at))
            .first()
        )
...
async def get_raw_llm_response(
    article: ArticleTable, filtered_content: str, db_session, llm_service: LLMService, use_existing: bool = True
) -> str:
    ...
    try:
        ...
        try:
            ...
            if not active_config or not active_config.agent_prompts:
                ...
                cmdline_prompt_config = (
                    agent_prompts.get("CmdlineExtract")
                    or agent_prompts.get("ExtractAgent")
                    or agent_prompts.get("ExtractAgentSettings")
                )
        ...
        if not raw_response:
            ...
            logger.warning(
                f"No raw_response in extraction result for article {article.id}, using string representation"
            )
...
async def process_article(
    ...
    if not article:
        ...
        return {"filtered_content": f"ARTICLE NOT FOUND: {url}", "raw_llm_response": "N/A"}
    ...
    try:
        raw_response = await get_raw_llm_response(
            article, filtered_content, db_session, llm_service, use_existing=use_existing_results
        )
    except Exception as e:
        logger.error(f"Error getting raw LLM response for article {article.id}: {e}")
        raw_response = f"ERROR: {str(e)}"

    return {"filtered_content": filtered_content, "raw_llm_response": raw_response}
...
async def main():
    ...
    parser.add_argument(
        "--force-fresh", action="store_true", help="Force fresh extraction even if existing results are available"
    )
    ...
    for i, article_def in enumerate(eval_articles, 1):
        ...
        try:
            result = await process_article(
                article_def, db_session, llm_service, args.junk_filter_threshold, use_existing_results=use_existing
            )
            if result:
            ...
            results.append({"filtered_content": f"ERROR PROCESSING: {url}", "raw_llm_response": f"ERROR: {str(e)}"})
            error_count += 1
    ...
    if use_existing:
        logger.info("   (Used existing results when available)")
    logger.info("=" * 60)
