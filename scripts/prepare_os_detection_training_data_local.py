...
async def label_article_with_local_service(
    article: dict[str, Any], service: OSDetectionService, use_fallback: bool = True
) -> str | None:
...
async def prepare_training_data(
    articles: list[dict[str, Any]], use_fallback: bool = True, fallback_model: str | None = None
) -> list[dict[str, Any]]:
...
def main():
    ...
    parser.add_argument(
        "--no-fallback", action="store_true", help="Disable LLM fallback (use only classifier/similarity)"
    )
    parser.add_argument(
        "--fallback-model",
        type=str,
    ...
    training_data = asyncio.run(
        prepare_training_data(articles, use_fallback=not args.no_fallback, fallback_model=args.fallback_model)
    )
