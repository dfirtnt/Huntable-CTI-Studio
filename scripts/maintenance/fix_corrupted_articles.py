...
async def fix_article(article_id: int) -> bool:
    ...
    try:
        ...
        if new_replacement_chars > 100 or new_non_printable > 500:
            print(f"⚠️ New content still appears corrupted: {new_replacement_chars} replacement chars, {new_non_printable} non-printable")
        ...
        if not content_text or content_length < 500:
            ...
            print(f"   Skipping update to preserve existing content")
            return False
...
async def fix_articles(article_ids: List[int], batch_size: int = 10):
    ...
    for i in range(0, total, batch_size):
        ...
        print(f"\n{'='*60}")
        print(f"Processing batch {i//batch_size + 1}/{(total + batch_size - 1)//batch_size} ({len(batch)} articles)")
        print(f"{'='*60}")
