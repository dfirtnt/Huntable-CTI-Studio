...
def check_extraction_status(article_ids: list) -> dict:
    ...
    try:
        for article_id in article_ids:
            ...
            # Check for execution
            execution = session.query(AgenticWorkflowExecutionTable).filter(
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs='+',
        help='Article IDs to check'
    )
    parser.add_argument(
        '--from-file',
        type=str,
        help='Read article IDs from test data JSON file'
    )
    parser.add_argument(
        '--regenerate',
        action='store_true',
        help='Regenerate test data file when extractions are complete'
    )
    ...
    if all_complete:
        ...
        if args.regenerate and args.from_file:
            ...
            from scripts.create_test_finetuning_data import main as regenerate_main
