...
def check_rule_131():
    ...
    try:
        # Find rule 131
        rule = db_session.query(SigmaRuleQueueTable).filter(SigmaRuleQueueTable.id == 131).first()
        ...
        if rule.workflow_execution_id:
            execution = (
                ...
                .filter(AgenticWorkflowExecutionTable.id == rule.workflow_execution_id)
                .first()
            )
