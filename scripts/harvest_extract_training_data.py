...
def load_json_results(filepath: str) -> Optional[Dict[str, Any]]:
...
def harvest_from_database(
    ...
    """
    query = db_session.query(AgenticWorkflowExecutionTable).join(
        ArticleTable
    ).filter(
...
def enrich_with_article_content(
    ...
    if article_ids:
        articles = db_session.query(ArticleTable).filter(
            ArticleTable.id.in_(list(article_ids))
        ).all()
...
def main():
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='outputs/training_data/extract_agent_training_data.json',
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Minimum number of observables required (default: 1)'
    )
    parser.add_argument(
        '--json-files',
        nargs='+',
        help='JSON result files to harvest from'
    )
    parser.add_argument(
        '--from-database',
        action='store_true',
        help='Harvest from database'
    )
    parser.add_argument(
        '--status-filter',
        type=str,
    ...
    parser.add_argument(
        '--auto-find-json',
        action='store_true',
        help='Automatically find JSON result files in project root'
    )
    parser.add_argument(
        '--apply-junk-filter',
        action='store_true',
    ...
    if json_files:
        ...
        try:
            json_examples = harvest_from_json_files(
