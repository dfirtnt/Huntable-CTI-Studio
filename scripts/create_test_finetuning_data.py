...
def check_extraction_result(article_id: int, db_session: Session) -> Optional[Dict[str, Any]]:
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=80.0,
        help='Minimum threat_hunting_score (default: 80.0)'
    )
    parser.add_argument(
        '--count',
        type=int,
        default=10,
        help='Number of test records to create (default: 10)'
    )
    parser.add_argument(
        '--apply-junk-filter',
        action='store_true',
        default=True,
        help='Apply junk filter (default: True)'
    )
    parser.add_argument(
        '--no-junk-filter',
        action='store_false',
        dest='apply_junk_filter',
        help='Do not apply junk filter'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--require-extraction',
        action='store_true',
        help='Only include articles with existing extraction results'
    )
    ...
    try:
        ...
        for article in articles:
            ...
            print(f"   {has_extraction} Article {article.id}: score={hunt_score:.1f}, observables={observables_count}, "
        ...
        if training_examples:
            total_observables = sum(
                len(ex['extraction_result']['observables'])
            ...
            print(f"   - Examples with extractions: {sum(1 for ex in training_examples if len(ex['extraction_result']['observables']) > 0)}/{len(training_examples)}")
        ...
        print(f"   3. Use for fine-tuning or combine with other training data")
