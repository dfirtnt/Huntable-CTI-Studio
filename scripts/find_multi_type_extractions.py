...
def find_multi_type_extractions():
    ...
    try:
        ...
        for result in results:
            article = db_session.query(ArticleTable).filter(ArticleTable.id == result["article_id"]).first()

            article_title = (
                article.title[:80] + "..."
                if article and article.title and len(article.title) > 80
                else (article.title if article else f"Article {result['article_id']}")
            )
            ...
            print("Extraction Types with Positive Counts:")
