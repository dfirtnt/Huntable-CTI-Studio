...
def has_sigma_patterns(content: str) -> bool:
    ...
    has_sigma_mention = 'sigma' in content_lower and (
        'rule' in content_lower or 
        'detection' in content_lower or
    ...
    has_sigma_code_block = bool(re.search(r'```.*sigma|```.*yaml.*title:.*logsource:', content_lower, re.DOTALL | re.IGNORECASE))
    ...
    for article in articles:
        ...
        if has_edr and has_sigma:
            ...
            for i, line in enumerate(lines):
                ...
                if any(pattern in line_lower for pattern in ['deviceprocessevents', '| where', 'index=', 'logs-endpoint', 'processrollup2']):
