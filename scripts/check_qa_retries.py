...
try:
    ...
    executions = (
        db_session.query(AgenticWorkflowExecutionTable).order_by(desc(AgenticWorkflowExecutionTable.id)).limit(50).all()
    )
            ...
            if qa_results:
                ...
                for agent_name, qa_result in qa_results.items():
                    if isinstance(qa_result, dict):
                        ...
                        is_parsing_failure = (
                            "could not be parsed" in str(feedback).lower() or "parsing failed" in str(feedback).lower()
                        )

                        if (
                            status in ["fail", "needs_revision"] or verdict in ["fail", "needs_revision"]
                        ) and not is_parsing_failure:
                            qa_retries.append(
                                {
                                    "agent": agent_name,
                ...
                if isinstance(agent_result, dict):
                    ...
                    if isinstance(raw, dict):
                        ...
                        if qa_result and isinstance(qa_result, dict):
                            ...
                            is_parsing_failure = (
                                "could not be parsed" in str(qa_feedback).lower()
                                or "parsing failed" in str(qa_feedback).lower()
                            )

                            # If status is fail/needs_revision (and not a parsing failure), check if there are items (indicating retry)
                            if (
                                qa_status in ["fail", "needs_revision"] or qa_verdict in ["fail", "needs_revision"]
                            ) and not is_parsing_failure:
