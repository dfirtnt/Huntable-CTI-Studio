...
async def remove_duplicates(articles_to_keep: list[int]):
    ...
    async with db.get_session() as session:
        ...
        await session.execute(
            ArticleTable.__table__.update().where(ArticleTable.id.in_(articles_to_remove)).values(archived=True)
        )
...
async def verify_cleanup():
    ...
    async with db.get_session() as session:
        ...
        result = await session.execute(select(func.count(ArticleTable.id)).where(ArticleTable.archived == True))
