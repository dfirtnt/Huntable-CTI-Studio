...
def find_matches(
    target_obs: list[dict], candidate_obs: list[dict], threshold: float = 0.7
...
def calculate_metrics():
    ...
    metrics["coverage_metrics"] = {
        "total_extractions": {"sonnet": total_sonnet, "gpt4o": total_gpt4o, "gpt4o_mini": total_gpt4o_mini},
        "avg_per_article": {
    ...
    metrics["agreement_metrics"] = {
        ...
        "consensus_strength": {
            ...
            "medium_confidence_pct": (
                (sonnet_gpt4o_matches + sonnet_mini_matches + gpt4o_mini_matches) / total_observables * 100
            )
    ...
    metrics["confidence_metrics"] = {
        "multi_model_consensus_rate": {
            ...
            "two_model": ((sonnet_gpt4o_matches + sonnet_mini_matches + gpt4o_mini_matches) / total_observables * 100)
            if total_observables > 0
            else 0,
            "any_consensus": (
                (all_three_matches + sonnet_gpt4o_matches + sonnet_mini_matches + gpt4o_mini_matches)
                / total_observables
                * 100
            )
            if total_observables > 0
...
def print_metrics_report(metrics: dict):
    ...
    print()
    print("Coverage Ratio (vs Sonnet baseline):")
    ...
    print()
    print("Unique Extractions (Single Model Only):")
    ...
    print(
        f"  Total Unique:   {agree['unique_extractions']['total_unique']:3d} ({agree['unique_extractions']['unique_percentage']:.1f}%)"
    )
    print()
    print("Consensus Strength:")
    print(
        f"  High Confidence (3 models):   {agree['consensus_strength']['high_confidence']:3d} ({agree['consensus_strength']['high_confidence_pct']:.1f}%)"
    )
    print(
        f"  Medium Confidence (2 models):  {agree['consensus_strength']['medium_confidence']:3d} ({agree['consensus_strength']['medium_confidence_pct']:.1f}%)"
    )
    print(
        f"  Low Confidence (1 model):     {agree['consensus_strength']['low_confidence']:3d} ({agree['consensus_strength']['low_confidence_pct']:.1f}%)"
    )
    print()
    print("Per-Article Agreement:")
    for article_id, data in agree["per_article_agreement"].items():
        print(
            f"  Article {article_id}: {data['all_three_count']:2d} common ({data['agreement_percentage']:.1f}%) - "
    ...
    print("Observable Type Diversity:")
    ...
    for tech, counts in qual["technique_detection"].items():
        print(f"  {tech}:")
        print(
            f"    Sonnet: {counts['sonnet']:3d}, GPT-4o: {counts['gpt4o']:3d}, GPT-4o-mini: {counts['gpt4o_mini']:3d}"
        )
    print()
    ...
    print("Multi-Model Consensus Rate:")
