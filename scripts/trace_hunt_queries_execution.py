...
def trace_execution(execution_id: int):
    ...
    try:
        ...
        hunt_queries_result = (
            subresults.get('hunt_queries', {}) or 
            subresults.get('HuntQueriesExtract', {}) or
            {}
        )
        ...
        if isinstance(sigma_rules, list) and len(sigma_rules) > 0:
            ...
            for idx, rule in enumerate(sigma_rules[:5]):  # Show first 5
                ...
                if isinstance(rule, dict):
                    ...
                    if not title:
                        untitled_count += 1
                        print(f"         Title: ⚠️  EMPTY (would show as 'Untitled Rule')")
                    else:
                        print(f"         Title: ✅ '{title}'")
                    ...
                    if context:
                        ...
                        if 'paragraph' in context.lower():
                            print(f"         ⚠️  Context contains 'paragraph' - this may be causing display issue")
                else:
            ...
            print(f"   ❌ No SIGMA rules in normalized result")
            ...
            if 'response' in extract_agent_log:
                ...
                if isinstance(agent_result, dict):
                    ...
                    if 'sigma_rules' in agent_result:
                        print(f"   ✅ agent_result contains sigma_rules")
