...
class TestGroupRunner:
    ...
    def run_all_groups(self, groups: list[str] | None = None) -> list[TestResult]:
    ...
    def generate_reports(self):
        ...
        with open(md_path, "w") as f:
            ...
            for r in self.results:
                ...
                f.write(
                    f"| {r.group} | {status} | {r.passed} | {r.failed} | {r.skipped} | {r.errors} | {r.duration:.2f}s |\n"
                )
...
Examples:
  ...
  python scripts/run_tests_by_group.py --verbose           # Verbose output
    ...
    parser.add_argument("--stop-on-failure", action="store_true", help="Stop execution at first group failure")

    parser.add_argument("--verbose", action="store_true", help="Verbose output")

    args = parser.parse_args()

    runner = TestGroupRunner(verbose=args.verbose, stop_on_failure=args.stop_on_failure)
