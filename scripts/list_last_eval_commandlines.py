...
def list_last_eval_commandlines():
    ...
    try:
        ...
        print(
            f"Last eval run: Config Version {latest_eval.config_snapshot.get('config_version') if latest_eval.config_snapshot else 'N/A'}"
        )
                        ...
                        for item in items:
                            if isinstance(item, str):
                                all_commandlines.append(
                                    {"commandline": item, "article": article_title, "execution_id": exec.id}
                                )
        ...
        for i, cmd in enumerate(all_commandlines, 1):
            ...
            print(
                f"   (Article: {cmd['article'][:60]}{'...' if len(cmd['article']) > 60 else ''}, Execution: {cmd['execution_id']})"
            )
            print()
