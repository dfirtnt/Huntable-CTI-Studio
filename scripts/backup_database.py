...
def get_database_info() -> dict[str, Any]:
    ...
    try:
        ...
        model_versions_cmd = get_docker_exec_cmd(
            "cti_postgres", 'psql -U cti_user -d cti_scraper -c "SELECT COUNT(*) FROM ml_model_versions;" -t'
        )
        ...
        chunk_analysis_cmd = get_docker_exec_cmd(
            "cti_postgres", 'psql -U cti_user -d cti_scraper -c "SELECT COUNT(*) FROM chunk_analysis_results;" -t'
        )
        ...
        for table in new_tables:
            try:
                count_cmd = get_docker_exec_cmd(
                    "cti_postgres", f'psql -U cti_user -d cti_scraper -c "SELECT COUNT(*) FROM {table};" -t'
                )
        ...
        return {"backup_timestamp": datetime.now().isoformat(), "postgres_version": "15-alpine"}
...
def create_backup(backup_dir: str = "backups", compress: bool = True) -> str:
    ...
    try:
        ...
        try:
            with open(backup_filepath) as f:
