...
def restore_database(backup_path: Path, metadata: Dict[str, Any], 
                    create_snapshot: bool = True, force: bool = False) -> bool:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sql', delete=False) as temp_file:
            ...
            if db_filename.endswith('.gz'):
                ...
                with open(db_backup_file, 'r') as f_in:
                    shutil.copyfileobj(f_in, temp_file)
...
def restore_database_snapshot(snapshot_path: Path) -> bool:
    ...
    return restore_database(snapshot_path.parent, snapshot_metadata, 
                          create_snapshot=False, force=True)

def restore_directory(backup_path: Path, component_name: str, 
...
def restore_docker_volume(backup_path: Path, volume_name: str, 
...
def restore_system(backup_name: str, components: Optional[List[str]] = None,
                  backup_dir: str = 'backups', create_snapshot: bool = True,
    ...
    if not force and not dry_run:
        print(f"⚠️  WARNING: This will replace current system components!")
    ...
    try:
        ...
        if 'database' in components_to_restore:
            restore_results['database'] = restore_database(
                backup_path, metadata, create_snapshot, force
            )
        ...
        for component in directory_components:
            if component in components_to_restore:
                target_dir = Path(component)
                restore_results[component] = restore_directory(
                    backup_path, component, target_dir, dry_run
                )
        ...
        for volume_name in DOCKER_VOLUMES:
            ...
            if volume_key in components_to_restore:
                restore_results[volume_key] = restore_docker_volume(
                    backup_path, volume_name, dry_run
                )
...
def list_system_backups(backup_dir: str = 'backups') -> None:
    ...
    system_backups = [d for d in backup_path.iterdir() 
