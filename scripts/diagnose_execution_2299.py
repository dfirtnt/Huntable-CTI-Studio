    ...
    try:
        ...
        if cmdline_result:
            ...
            if isinstance(cmdline_items, list) and len(cmdline_items) > 0:
                ...
                print(f"   ⚠️  No commandlines in items list")
        else:
        ...
        if extraction_content:
            ...
            cmdline_mentions = extraction_content.lower().count('commandline') + extraction_content.lower().count('command line')
        ...
        if sigma_log:
            ...
            if 'content_used' in sigma_log:
                ...
                if isinstance(content_used, str):
                    cmdline_mentions = content_used.lower().count('commandline') + content_used.lower().count('command line')
                    ...
                    if isinstance(rule, dict):
                        ...
                        if rule_yaml:
                            ...
                            if has_commandline:
                                ...
                                import re
                                cmdline_match = re.search(r'commandline[^:]*:\s*(.+?)(?:\n|$)', rule_yaml, re.IGNORECASE | re.MULTILINE)
                                if cmdline_match:
                                    print(f"      Commandline value: {cmdline_match.group(1)[:100]}")
                            else:
                                print(f"      ⚠️  Rule YAML does not contain commandline selectors")
                    ...
                    if messages and len(messages) > 0:
                        ...
                        for msg in messages:
                            if isinstance(msg, dict) and msg.get('role') == 'user':
                                ...
                                cmdline_mentions = content.lower().count('commandline') + content.lower().count('command line')
                                ...
                                if extraction_content and extraction_content in content:
                                    print(f"   ✅ Extraction content IS in the prompt")
                                elif extraction_content:
                                    # Check if at least some commandlines are mentioned
                                    cmdline_in_prompt = any(cmd[:50] in content for cmd in cmdline_items[:5] if isinstance(cmd, str))
                                    print(f"   {'✅' if cmdline_in_prompt else '❌'} Commandlines {'ARE' if cmdline_in_prompt else 'ARE NOT'} in the prompt")
                                break
