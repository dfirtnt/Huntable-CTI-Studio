...
def calculate_context(article_content: str, start_position: int, end_position: int) -> tuple[str, str]:
    ...
    context_before = article_content[
        max(0, start_position - CONTEXT_RADIUS) : start_position
    ]
    context_after = article_content[
        end_position : min(len(article_content), end_position + CONTEXT_RADIUS)
    ]
...
async def backfill_annotation_context():
    ...
    try:
        async with db_manager.get_session() as session:
            ...
            logger.info(
                f"Backfill complete: {updated} updated, {errors} errors, "
                f"{total - updated - errors} skipped"
            )
