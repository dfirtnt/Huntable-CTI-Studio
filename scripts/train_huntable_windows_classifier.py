...
def prepare_features_and_labels(
    ...
    if use_keywords and use_embeddings:
        ...
        print(f"  Combined features: {X_keyword.shape[1]} keyword + {X_embedding.shape[1]} embedding = {X.shape[1]} total")
        print(f"    Keyword breakdown: 3 counts + 8 LOLBAS + 64 perfect + 16 obfuscation = {X_keyword.shape[1]} features")
    elif use_keywords:
...
def train_and_evaluate(
    ...
    cv_scores = cross_val_score(
        classifier, X_train_scaled, y_train, cv=cv, scoring='accuracy'
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train hybrid classifier for Windows huntables detection"
    )
    parser.add_argument(
        '--data',
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Path to save training metrics JSON (optional)'
    )
    parser.add_argument(
        '--no-embeddings',
        action='store_true',
        help='Use keyword features only (no BERT embeddings)'
    )
    parser.add_argument(
        '--no-keywords',
        action='store_true',
        help='Use BERT embeddings only (no keyword features)'
    )
