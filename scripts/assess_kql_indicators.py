...
def is_likely_kql_query(content: str, indicators: set[str]) -> tuple[bool, str]:
    ...
    if len(indicators) >= 2:
        ...
        if has_table and (has_field or has_operator):
            return True, "Table + field/operator combination"
    ...
    if len(indicators) == 1:
        ...
        if indicator in {"DeviceProcessEvents", "DeviceNetworkEvents", "DeviceEvents", "EmailEvents", "AlertInfo"}:
            # Check if it appears in a query-like context
            if re.search(rf"\b{re.escape(indicator)}\b.*\|\s*(where|project|summarize)", content, re.IGNORECASE):
...
def analyze_articles(articles: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for indicator in KQL_INDICATORS:
        ...
        kql_count = sum(
            ...
            if is_likely_kql_query(a.get("content", ""), find_indicators_in_content(a.get("content", "")))[0]
        )
