...
with open(OS_DETECTION_PROMPT_PATH) as f:
    prompt_config_dict = json.load(f)

VALID_OS_LABELS = ["Windows", "Linux", "MacOS", "multiple"]


def load_articles(min_hunt_score: float = 80.0, limit: int | None = None) -> list[dict[str, Any]]:
...
async def label_article_with_llm(article: dict[str, Any], client: OpenAI, model: str = "gpt-4o") -> str | None:
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert at analyzing cybersecurity threat intelligence articles to identify target operating systems.",
                },
...
async def prepare_training_data(
    articles: list[dict[str, Any]], use_llm_labeling: bool = True, llm_model: str = "gpt-4o", api_key: str | None = None
) -> list[dict[str, Any]]:
...
def main():
    ...
    parser.add_argument(
        "--min-hunt-score", type=float, default=80.0, help="Minimum hunt score for articles (default: 80.0)"
    )
    ...
    parser.add_argument("--no-llm", action="store_true", help="Skip LLM labeling (for manual labeling workflow)")
    parser.add_argument(
        "--llm-model", type=str, default="gpt-4o", help="LLM model to use for labeling (default: gpt-4o)"
    )
    ...
    training_data = asyncio.run(
        prepare_training_data(
            articles, use_llm_labeling=not args.no_llm, llm_model=args.llm_model, api_key=args.api_key
        )
    )
