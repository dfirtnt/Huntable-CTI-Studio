...
def list_last_eval_extractions():
    ...
    try:
        ...
        latest_eval = (
            ...
            .filter(AgenticWorkflowExecutionTable.config_snapshot.has_key("eval_run"))
        ...
        print(
            f"Config Version: {latest_eval.config_snapshot.get('config_version') if latest_eval.config_snapshot else 'N/A'}"
        )
        print("=" * 80)
        ...
        for i, exec in enumerate(eval_executions, 1):
            ...
            article_title = (
                article.title[:60] + "..."
                if article and article.title and len(article.title) > 60
                else (article.title if article else f"Article {exec.article_id}")
            )
                                    ...
                                    for item in items[:10]:  # Show first 10
                                        if isinstance(item, str):
                                            ...
                                            if "value" in item:
                                                ...
                                                print(
                                                    f"        - Process: {item.get('process_tree', {}).get('process_name', 'N/A')[:100]}"
                                                )
