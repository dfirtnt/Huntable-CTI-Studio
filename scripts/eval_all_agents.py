...
async def main():
    ...
    parser.add_argument("--config", type=str, help="Path to evaluation config JSON file")
    parser.add_argument("--extract-data", type=str, help="Path to Extract Agent test data")
    parser.add_argument("--rank-data", type=str, help="Path to Rank Agent test data")
    parser.add_argument("--sigma-data", type=str, help="Path to SIGMA Agent test data")
    parser.add_argument("--os-data", type=str, help="Path to OS Detection test data")
    parser.add_argument(
        "--output-dir", type=str, default="outputs/evaluations", help="Output directory for evaluation results"
    )
    parser.add_argument(
    ...
    if extract_data:
        ...
        extract_evaluator = ExtractAgentEvaluator(evaluation_type=args.evaluation_type)
        extract_metrics = await extract_evaluator.evaluate_dataset(
            Path(extract_data), llm_service=llm_service, content_filter=content_filter
        )
        extract_evaluator.save_results(
            output_path=output_dir / "extract_agent_results.json", save_to_db=args.save_to_db, db_session=db_session
        )
    ...
    if rank_data:
        ...
        rank_evaluator = RankAgentEvaluator(evaluation_type=args.evaluation_type)
        rank_metrics = await rank_evaluator.evaluate_dataset(Path(rank_data), llm_service=llm_service)
        rank_evaluator.save_results(
            output_path=output_dir / "rank_agent_results.json", save_to_db=args.save_to_db, db_session=db_session
        )
    ...
    if sigma_data:
        ...
        sigma_evaluator = SigmaAgentEvaluator(
            evaluation_type=args.evaluation_type, llm_service=llm_service, db_session=db_session
        )
    ...
    if os_data:
        ...
        os_evaluator = OSDetectionEvaluator(evaluation_type=args.evaluation_type)
        os_metrics = await os_evaluator.evaluate_dataset(Path(os_data), os_detection_service=os_detection_service)
        os_evaluator.save_results(
            output_path=output_dir / "os_detection_results.json", save_to_db=args.save_to_db, db_session=db_session
        )
