...
def add_article_to_dataset(
    ...
    item_id: str,
    expected_count: int,
    dataset_name: str = "cmdline_extractor_gt"
):
    ...
    try:
        ...
        if "already exists" in str(e).lower() or "duplicate" in str(e).lower():
            ...
            try:
                ...
                if existing_item:
                    logger.info(f"Found existing item, deleting and recreating...")
                    # Note: Langfuse Python SDK may not have delete method
                    # If update fails, we'll log the error
                    logger.warning(f"⚠️  Cannot update existing item. Please delete manually or use a different ID.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Add an article to Langfuse cmdline_extractor_gt dataset"
    )
    parser.add_argument(
        "--url",
        required=True,
        help="Article URL (canonical_url from database)"
    )
    parser.add_argument(
        "--id",
        required=True,
        dest="item_id",
        help="Dataset item ID (e.g., 'article_63')"
    )
    parser.add_argument(
        "--expected-count",
        type=int,
        required=True,
        dest="expected_count",
        help="Expected command-line count"
    )
    parser.add_argument(
        "--dataset",
        default="cmdline_extractor_gt",
        help="Dataset name (default: cmdline_extractor_gt)"
    )
