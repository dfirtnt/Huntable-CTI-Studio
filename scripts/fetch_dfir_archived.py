...
class DFIRArchiveFetcher:
        ...
        try:
            ...
            async with self.db.get_session() as session:
                ...
                result = await session.execute(
                    select(ArticleTable.canonical_url).where(ArticleTable.source_id == dfir_source.id)
                )
    ...
    async def scrape_archive_pages(self, max_pages: int = 200):
        ...
        if sitemap_results:
            ...
            logger.info(
                f"Sitemap results: {articles_collected} created, {articles_skipped_existing} skipped (existing), {articles_skipped_recent} skipped (recent), {articles_failed} failed"
            )
        ...
        if rss_results:
            ...
            logger.info(
                f"RSS feed results: {rss_collected} created, {rss_skipped_existing} skipped (existing), {rss_skipped_recent} skipped (recent), {rss_failed} failed"
            )
            # Accumulate results
            if sitemap_results:
        ...
        logger.info(f"\n{'=' * 60}")
        logger.info("Archive scraping complete!")
    ...
    async def scrape_and_save_article(self, url: str, source: Source) -> str:
        ...
        try:
            ...
            async with self.db.get_session() as session:
                ...
                result = await session.execute(select(ArticleTable).where(ArticleTable.canonical_url == url))
...
async def main():
    ...
    parser.add_argument(
        "--max-pages", type=int, default=200, help="Maximum number of archive pages to scrape (default: 200)"
    )
