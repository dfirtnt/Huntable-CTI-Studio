...
def backfill_eval_records():
    ...
    try:
        ...
        pending_evals = (
            db_session.query(SubagentEvaluationTable).filter(SubagentEvaluationTable.status == "pending").all()
        )
        ...
        for eval_record in pending_evals:
            ...
            if not execution:
                logger.warning(
                    f"Execution {eval_record.workflow_execution_id} not found for eval record {eval_record.id}"
                )
                continue
            ...
            if not subagent_name or subagent_name != eval_record.subagent_name:
                logger.debug(
                    f"Eval record {eval_record.id} subagent mismatch: {subagent_name} vs {eval_record.subagent_name}"
                )
                continue
