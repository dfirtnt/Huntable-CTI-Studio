...
def run_psql_command(sql_command):
    ...
    cmd = ["docker", "exec", "cti_postgres", "psql", "-U", "cti_user", "-d", "cti_scraper", "-c", sql_command]
...
def get_coverage_stats():
    ...
    if result:
        lines = result.split("\n")
        for line in lines:
            if "|" in line and not line.startswith("-") and not line.startswith(" ") and "total_articles" not in line:
