...
async def backfill_simhash_async():
    """Backfill SimHash values for existing articles using async session."""
    try:
...
def backfill_simhash_sync():
    """Backfill SimHash values for existing articles using sync session."""
    try:
        # Connect to database
        database_url = os.getenv("DATABASE_URL", "postgresql://cti_user:cti_password@postgres:5432/cti_scraper")
...
def get_coverage_stats():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            logger.info("Current SimHash Coverage:")
