...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load LMStudio models with proper context length for development"
    )
    parser.add_argument(
        "model",
        nargs="?",
        help="Model name to load (e.g., 'qwen2-7b-instruct')"
    )
    parser.add_argument(
        "--context-length",
        type=int,
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Load all configured models from environment"
    )
