...
`SigmaNoveltyService.assess_novelty` uses a **single scorer** (`compare_precomputed_atoms`)
...
`src/workflows/agentic_workflow.py:683` encodes this three-way distinction. The
list endpoint (`GET /api/sigma-queue/list`) re-runs the check on-the-fly for `pending`
...
```bash
# Huntable-created rules / PR submission (customer rules repo)
...
The app uses two separate repositories:

- **Customer rules repository:** `../Huntable-SIGMA-Rules`, mounted as `sigma-repo`. This is the only repository used for Huntable-created rules and GitHub PRs. Set `SIGMA_REPO_PATH` to this repository.
...
1. Rebuild the vector index:
   ```sql
   REINDEX INDEX ix_sigma_rules_embedding_hnsw;
   ```
2. Adjust HNSW `m` / `ef_construction` parameters (the index is HNSW, not IVFFlat --
   see `scripts/migrate_pgvector_indexes.py`)
