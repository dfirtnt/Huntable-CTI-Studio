...
| `src/services/llm_generation_service.py` (removed 2026-08-31) | Central wrapper around its provider methods | Fixed and covered at time of audit | `llm_generation`, attempt, provider, model, messages, output, usage, classified errors | Not run; no active repo callers found -- module deleted, its only consumer (`scripts/benchmark_llm_providers.py`) now dispatches through `LLMService.request_chat()` instead |
