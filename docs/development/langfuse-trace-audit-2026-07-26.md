...
The vision route remains unverified because its implementation only supports OpenAI and Anthropic and does not offer a local LMStudio provider; exercising it would violate the cloud-cost guardrail. No recent `sigma_repair_attempts` score was emitted because the local route tests do not carry a workflow execution ID. These are the only remaining live-evidence gaps.
