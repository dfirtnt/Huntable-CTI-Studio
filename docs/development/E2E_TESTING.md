...
```python
@pytest.mark.e2e
def test_service_integration(page: Page):
    """Test integration between different services."""
    # 1. Verify all services are running
    health_response = page.request.get("/health")
    ...
    # 3. Test API service
    api_response = page.request.get("/api/articles")
    assert api_response.status == 200
    
    # 4. Test database service
    db_response = page.request.get("/api/sources")
    ...
    page.goto("http://localhost:8001/sources")
    page.click("button:has-text('Add Source')")
    page.fill("input[name='name']", "Service Integration Test")
    page.fill("input[name='url']", "https://example.com/service-test.xml")
    page.select_option("select[name='type']", "rss")
    page.click("button:has-text('Save')")
...
```python
@pytest.mark.e2e
def test_cross_component_data_consistency(page: Page):
    """Test data consistency across different components."""
    # 1. Create source
    page.goto("http://localhost:8001/sources")
    page.click("button:has-text('Add Source')")
    page.fill("input[name='name']", "Consistency Test")
    page.fill("input[name='url']", "https://example.com/consistency.xml")
    page.select_option("select[name='type']", "rss")
    page.click("button:has-text('Save')")
    
    # 2. Get source ID from UI
    source_element = page.locator("text=Consistency Test")
    expect(source_element).to_be_visible()
    
    # 3. Verify source in API
    api_response = page.request.get("/api/sources")
    assert api_response.status == 200
    ...
    for article in articles:
        if article["source_id"] == api_source["id"]:
            ...
            page.goto(f"http://localhost:8001/articles/{article['id']}")
            expect(page.locator(".article-content")).to_be_visible()
            expect(page.locator("text=Consistency Test")).to_be_visible()
            break
    else:
        assert False, "No articles found for created source"
```
...
```python
@pytest.mark.e2e
def test_system_load(page: Page):
    ...
    for i in range(5):
        page.click("button:has-text('Add Source')")
        page.fill("input[name='name']", f"Load Test Source {i}")
        page.fill("input[name='url']", f"https://example.com/load-{i}.xml")
        page.select_option("select[name='type']", "rss")
        page.click("button:has-text('Save')")
        expect(page.locator(f"text=Load Test Source {i}")).to_be_visible()
    ...
    page.fill("input[placeholder='Search articles...']", "load")
    page.press("input[placeholder='Search articles...']", "Enter")
    expect(page.locator(".search-results")).to_be_visible()
    ...
    expect(page.locator("text=Load Test Source 0")).to_be_visible()
