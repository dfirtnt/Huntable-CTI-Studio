...
**Agents -- SIGMA Queue / Executions (layout + Enrich):** consolidated into `tests/ui/test_workflow_comprehensive_ui.py` (`TestWorkflowQueueRegressions`, `TestWorkflowExecutionsRegressions`) -- mocked APIs; Enrich “Original Rule” = YAML; tables avoid horizontal scroll for Actions. With **`--parallel`** / `pytest-xdist`, Playwright output goes to `test-results/playwright-<worker>/` per worker (avoids teardown races on `test-results`).
...
- `tests/services/` (includes `test_sigma_sync_metadata.py`)
