...
- Bigger pattern to watch: this is the **third** instance of this pipeline reporting optimistic success:
  1. A swallowed Phase 4 `expansion_error` — `metadata.expansion_error` set at the service level was dropped at the workflow's per-group merge, so a group that lost every expansion rule still reported clean success (fixed in commit `f0cee55a`, 2026-08-15; see [[sigma-phase4-expansion-error-swallowed-2026-08-15]]).
  2. A prompt-less agent config completing as "success" with zero observables and zero rules (execution 3777), root-caused to an autosave race in preset import (fixed in commit `6cd9e1d7`, 2026-08-15; see [[preset-import-autosave-discards-prompts-2026-08-15]]).
...
## Related Issues
- [[sigma-phase4-expansion-error-swallowed-2026-08-15]] — first instance; commit `f0cee55a`, expansion_error metadata dropped at the workflow merge step.
- [[preset-import-autosave-discards-prompts-2026-08-15]] — second instance; commit `6cd9e1d7`, execution 3777's prompt-less config traced to an autosave race in preset import.
