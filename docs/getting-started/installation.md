...
- PostgreSQL: 5432
- Redis: 6379
- 8888: mapped on the web container by `docker-compose.yml`, but nothing listens on it by default (uvicorn binds only 8001)
...
- **web**: FastAPI application (uvicorn on port 8001; the 8888 mapping is unused by the default command)
...
(`config/eval_articles_data/sigma/` holds fixtures for the end-to-end Sigma eval, not a sub-agent.)
