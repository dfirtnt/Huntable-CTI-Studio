...
| Tier | Behavior | Tools |
|---|---|---|
| Auto-executable | Applies a scoped, reversible mutation immediately and writes a mandatory `audit_events` row in the same transaction. | `retry_workflow_execution`, `cancel_workflow_execution`, `toggle_source_status`, `create_annotation`, `update_annotation`, `delete_annotation` |
