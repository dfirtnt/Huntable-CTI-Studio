...
- `POST /api/workflow/executions/trigger-stuck` — Re-dispatches **stuck** `pending` executions onto the Celery workflow queue (one `trigger_agentic_workflow` task per row). Use it when a Celery task was lost — worker down at dispatch time, broker restart. Only rows pending longer than `STUCK_PENDING_AFTER` (5 min) and never started qualify; fresher rows are reported under `skipped` because their task is probably still queued (a staggered eval run can leave its tail pending for minutes with live tasks). Double runs are prevented in the worker, not here: `run_workflow` claims its row with a conditional `UPDATE … WHERE status = 'pending'` and a duplicate dispatch for an already-claimed row logs a skip and returns without invoking the graph. The age filter keeps the endpoint from queueing noise and from counting such no-op dispatches as `successful`; `count` / `successful` / `failed` describe tasks queued, not runs performed. `WorkflowTriggerService.is_stuck_pending_execution` is the single shared definition — the article-trigger path fails these same rows before starting a fresh execution. It queues work; it does not run the LangGraph pipeline in the web process, which cannot import `langgraph`.
...
- `POST /api/evaluations/subagent-eval-backfill` — Backfill pending eval records from existing workflow executions.
- `POST /api/evaluations/subagent-eval-rescore` — Repair completed count-only records that have item-level ground truth but no score, using each record's retained extractor output (no paid LLM). `?subagent=<name|all>`; `?apply=false` (default) is a dry-run reporting `candidates`/`scorable`/`unrepairable_no_output` per agent, `?apply=true` persists. Scores against the ground truth stored on the record (no disk reload) and is idempotent.
