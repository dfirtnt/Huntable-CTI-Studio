...
- **Native confirm()/prompt() replaced with styled ModalManager dialogs** (2026-07-17): added `ModalManager.confirm()` and `ModalManager.prompt()` (`src/web/static/js/modal-manager.js`) as Promise-based methods that dynamically create styled confirmation and input modals using the project's theme-variable system. Migrated 22 native `confirm()` calls across 8 templates (`agent_evals`, `agent_evals2`, `article_detail`, `articles`, `ml_hunt_comparison`, `observable_training`, `settings`) to the new API, with context-appropriate titles and button colors for destructive, warning, and informational actions.
