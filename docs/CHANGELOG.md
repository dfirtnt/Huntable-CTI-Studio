...
### Changed
- **Sigma atom similarity package rename** (2026-06-11, commit `76be4c24`): the deterministic Sigma set-math package is now distributed as `sigma_atom_similarity` instead of `sigma_semantic_similarity`, aligning the install/package name with what the engine actually does: atom extraction, DNF surface scoring, Jaccard, containment, and filter penalties. The import package remains `sigma_similarity`; the service wrapper is moving to atom-field terminology (`sigma_atom_precompute.py`, `extract_atom_fields()`, `precompute_atom_fields()`). Docker/package metadata, requirements, tests, and UI/help copy have been updated so new docs and operator commands no longer describe the dedup engine as "semantic" or embedding-based.
