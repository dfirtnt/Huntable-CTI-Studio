...
## [Unreleased]
### Added
- **Status-aware audit coverage for backup, model, embeddings, and evaluation routes** (2026-07-28): privileged mutating endpoints on `backup.py` (create, restore, restore-from-file, cron add/remove), `models.py` (retrain, evaluate, rollback), `embeddings.py` (bulk rebuild, per-article embed), `evaluation_api.py` (run, run-subagent-eval, run-sigma-eval, clear-pending, backfill, export-bundle, diagnose), `observable_training.py` and `observable_evaluation.py` emitted no durable audit events, so a successful privileged mutation left no record of the actor. Note the route manifest already declared `AuditRequirement.MANDATORY` for `/api/backup/*` and `/api/model/*` while both files had zero audit call sites -- the guarantee was declared but unmet. Each path now records an `attempted` event before its non-transactional side effect and a terminal `success`/`failure` event after it, following the `sigma_queue.py` PR-submission precedent. Adds `STATUS_ATTEMPTED` plus `backup.cron_updated`, `backup.cron_deleted`, `model.retrained`, `model.evaluated`, `model.rolled_back`, `embeddings.rebuild_requested`, `article.embedded`, `evaluation.*` and `observable_*.run_requested` to the action vocabulary; the pre-reserved `backup.created`/`backup.restored` names gained emitters for the first time. New `AsyncAuditService.record_out_of_band` opens its own short-lived session for subprocess/worker-thread paths that have no caller-owned transaction to join; it is bounded by a 5s timeout and never raises, so a stalled or failing database costs an audit row rather than a hung or 500-ing privileged request. Sink choice follows path shape: eval record clear/backfill own a transaction and use `record_mandatory` so attribution is atomic with the mutation, while dispatch-only routes (Celery `send_task`, workflow trigger) commit best-effort because the side effect has already fired. Retrain and rollback capture `ActorContext` at the route boundary and hand it to the background thread so terminal events keep human attribution. Audit emission only -- nothing re-scrapes or mutates eval-article rows or `config/eval_articles_data` fixtures, pinned by a test asserting the eval helper never queries or writes anything but its own audit row. One consequence is recorded in code and in `docs/guides/authentication.md`: a full-database restore replaces `audit_events` too, so the pre-restore `attempted` row does not survive a *successful* restore; it is what remains when a restore fails or is killed partway.
- **Neutral eval-item state** (2026-07-22): item-level subagent evaluation now supports a justification-bearing `acceptable_items` ground-truth list for genuinely defensible alternate readings. A neutral extraction is stored separately and excluded from both precision and recall denominators; it cannot duplicate an expected item, and schema tests require every neutral entry to carry a written justification. This is a score-comparability era boundary; re-baseline item-level metrics before comparing historical runs.
- **MCP eval bundle export and diagnosis tools** (2026-07-21): added 4 new MCP tools (`get_eval_bundle`, `diagnose_eval_bundle`, `list_eval_diagnoses`, `export_diagnosed_eval_bundles`) in `src/huntable_mcp/tools/evals.py` so MCP clients can inspect full eval bundles, run LLM-powered diagnosis (mirroring the web Diagnose button), list saved diagnoses, and bulk-export diagnosed bundles by config version/subagent. Registered in `stdio_server.py`, documented in `docs/reference/mcp-tools.md`, unit-tested in `tests/unit/test_mcp_eval_tools.py` and `tests/test_mcp_server_config.py`.
- **Regression guards for already-shipped fixes** (2026-07-17): a static test that every backup caller (the shell dispatchers plus `src/cli/commands/backup.py`) references only scripts that exist; an end-to-end contract test locking `prune_backups.py --stats` output to the `/api/backup/list` parser; API tests asserting `src/web/routes/ai.py` never logs key/token material; and unit tests pinning the auto-trigger hunt-score threshold default to 100 (opt-in) across `WorkflowConfigResponse` and `WorkflowTriggerService`.
- **`cut-release` skill: container rebuild check (Phase 2c)** (2026-07-17): before running `release_cut.py`, the skill now diffs the release branch against `main` for paths outside the bind-mounted `src/` volume (`Dockerfile`, `docker-compose.yml`, dependency/lockfiles, `COPY`'d packages) and asks the operator to confirm they rebuilt and re-tested against the rebuilt image -- closing the gap where a change only takes effect after `docker compose build` could ship untested.

### Fixed
- **`escapeHtml` consolidated into `/static/js/utils.js`, fixing attribute-context escaping** (2026-07-29): `escapeHtml` was redefined in seven templates plus `components/similarity-display.js` in three mutually incompatible variants -- the `createElement`/`textContent` trick (`workflow.html`, `agent_evals.html`, `observable_training.html`, `sigma_ab_test.html`, `diags.html`, `similarity-display.js`), a manual replace of only `& < >` (`article_detail.html`), and a full `& < > " '` replace (`settings.html`). Only the last escapes quote characters, yet `article_detail.html`, `agent_evals.html`, `workflow.html` and `similarity-display.js` interpolate `escapeHtml` output directly into HTML attributes (`data-scheduled-job-id="${...}"`, `title="${...}"`, and roughly 20 `value=`/`id=` sites in workflow's model and prompt rendering), so any value containing a double quote escaped its attribute. New `src/web/static/js/utils.js`, loaded first from `base.html`, defines one canonical `escapeHtml` that escapes `& < > " '` and is therefore safe in both text and attribute position; null and undefined become the empty string while `escapeHtml(0)` is now `"0"` rather than `""`. The eight duplicate definitions were deleted. `similarity-display.js` mattered most: its copy was declared at top level, so on the four pages that load it (`sigma_ab_test`, `article_detail`, `sigma_similarity_test`, `workflow`) it overwrote the shared global with the quote-unsafe version -- its `<script>` tags previously carried no cache-busting query string, so they gained `?v=20260729` to keep stale browser caches from re-shadowing the fix. `utils.js` is dual-mode -- browser globals plus `module.exports` -- because `similarity-display.js` is `require()`d in Node by `tests/unit/test_similarity_display_normalization.py` and re-exports `escapeHtml`, so the component resolves the browser global when there is one and loads the same shared file from disk otherwise; that keeps exactly one implementation rather than reintroducing a component-local copy. `diags.html`'s `formatTimestamp` also moved into utils.js. `base.html`'s inline `formatDate` had zero callers anywhere in templates or static JS and was deleted rather than relocated. The function-local `const escapeHtml` inside `article_detail.html`'s documented fallback confirm-modal block was deliberately left alone -- it is function-scoped, so it cannot clobber the global. Verified in the browser at `:8001` on all seven affected pages plus `/articles/1426`: the canonical implementation wins everywhere, quote escaping is live, `formatTimestamp` renders, every AGENTS.md contract-grade function on `/workflow` still resolves, and the config UI still renders its model and prompt containers.
- **A failing article query no longer renders as an empty article list** (2026-07-29): `AsyncDatabaseManager.list_articles` wrapped its whole body in `try/except Exception`, logged, and returned `[]`, so a database outage was indistinguishable from a legitimately empty result -- the web UI rendered its normal "no articles" state instead of an error, and the sync `DatabaseManager.list_articles` had always propagated, so the two managers disagreed. Reviewed all 13 swallow-after-log sites in `async_manager.py` individually against their callers rather than blanket-changing them, and split them by whether an empty list is meaningful data. Five now propagate: `list_articles` and `list_articles_by_source` (all 12 web callers already wrap them and turn the error into an HTTP 500 or `error.html`), `get_article_annotations` (most articles legitimately have zero annotations, so a swallowed failure was completely invisible), and `get_articles_with_source_info` / `get_articles_without_embeddings` -- the latter two mattered most operationally, because both retroactive-embedding tasks in `celery_app.py` report `{"status": "success", "message": "No articles found without embeddings"}` on an empty list, so a database error was reported as a clean run; both task handlers re-raise, so the failure is now loud. Six keep soft-empty **on purpose** and say so in code: `get_source_quality_stats` / `get_source_hunt_scores` (supplementary decoration on the sources page), `get_performance_metrics` (the database *health* endpoint must stay reachable to report degradation rather than 500 on the way to saying the database is unwell), and `search_similar_annotations` / `search_similar_articles` / `search_articles_by_lexical_terms` (RAG retrieval degrades to "no context" rather than failing the workflow that asked for it). Both halves are pinned by parametrized tests so a future edit cannot silently flip one, and route-level tests drive the real `/articles` page and `/api/articles` endpoint with a failing `list_articles` and assert a 500 plus the error template. Two methods, `get_annotations_without_embeddings` and `get_annotations_by_ids`, were found to have zero callers and were left untouched for the dead-code pass.
- **Dead duplicate JS function definitions removed from `workflow.html`** (2026-07-28): four functions were declared twice at top level in the same `<script>` block, so the later declaration silently won and every edit to the earlier copy was a no-op. `renderAgentModels` and `renderAgentPrompts` each had a dead first copy targeting `#agentModelsContainer` / `#agentPromptsContainer`, DOM ids that no longer exist anywhere in the template; the live copies target the contract-grade `#{agentprefix}-agent-model-container` / `-prompt-container` ids, and every caller passes the live signature's argument. `editPrompt` and two of three `escapeHtml` copies were byte-identical duplicates. Removed 255 lines, deletions only. Verified in the browser at `:8001`: all AGENTS.md contract-grade functions still resolve, `renderAgentModels` reports arity 1 (confirming the correct copy survived), the model and prompt containers still render, and no console errors. Note for future dedup passes: a naive grep over these templates reports commented-out code as live duplicates -- the apparent `article_detail.html` collisions are inside `/* */` blocks, and its two genuine duplicates are a deliberate documented fallback block that must not be deleted.
- **HTML-to-text no longer invents whitespace between inline elements** (2026-07-27): `ContentCleaner.html_to_text()` called `soup.get_text(separator=" ")`, which inserts a space between *every* text node. HTML gives inline elements no implicit whitespace -- real spacing lives in whitespace text nodes, which `strip=False` already preserved -- so that separator manufactured spaces absent from the document. It split any word a publisher wrapped in mid-token markup (`<span>comman</span><span>d</span>` -> `comman d`, `Power<b>Shell</b>` -> `Power Shell`, `rund<a>ll32</a>.exe` -> `rund ll32 .exe`) and detached trailing punctuation (`<em>italic text</em>.` -> `italic text .`), silently destroying the process names, file paths, and IOCs that detection engineering keys on. Found in the `cloud.google.com` UNC1549 Sigma eval fixture, whose Figure 9 caption stored as "reverse SSH comman d"; Google Cloud Blog exports from Google Docs and splits text across arbitrary `<span>` runs. Extraction now uses `separator=""`. Since the separator was also the de facto source of block separation, the block-tag list expanded from 11 tags to the full block-level set (new `_BLOCK_LEVEL_TAGS`), each block is now marked on *both* sides (marking only the trailing side left the first block in a run glued to preceding text -- real nav markup produced "Google Cloud BlogJump to Content"), and `<td>`/`<th>` separate horizontally so minified IOC tables do not glue cell values together. Boundaries are marked with a sentinel rather than `"\n"` so adjacent markers from nested blocks fold into one break instead of opening a blank line between every pair of blocks; literal newlines inside `<pre>` carry no sentinel and survive untouched. Verified content-neutral on a real article: old and new code over the same fresh HTML differ by 26 characters, all of them invented spaces. **Fix is forward-only** -- the ~5,954 of 6,802 stored articles carrying the detached-punctuation signature keep their existing text until re-scraped, so content hashes, keyword matches, and hunt scores for re-fetched articles can differ from their predecessors. No eval fixture or ground truth was re-scraped or modified.
- **Langfuse trace coverage audit** (2026-07-26): audited active provider call sites across workflow, eval diagnosis, Sigma routes, article-ranking routes, optimized ranking, vision extraction, and connection probes. Added missing generation/error observations, preserved provider usage metadata, propagated platform-adjudication article IDs, tagged connection tests separately, and added the dated coverage report plus regression tests. Live Langfuse verification remains pending because credentials were unavailable in the audit environment.
- **Sigma eval now runs against committed fixtures instead of the live DB row** (2026-07-22): `POST /run-sigma-eval` built its execution snapshot via `_sigma_eval_config_snapshot()`, which set `eval_run: True` but never set `eval_fixture_content` -- so `agentic_workflow.py` fell through to `article.content`, the flat, possibly ASCII-stripped DB text, even after `config/eval_articles_data/sigma/` fixtures were repaired (see the 2026-07-21 shared-URL reconciliation below). `run_sigma_eval` now resolves each URL's committed fixture via `_load_static_eval_articles("sigma")` and injects `eval_fixture_content` + `eval_fixture_content_sha256` into the snapshot, mirroring `run_subagent_eval`; a URL with no committed fixture now hard-fails with 422 instead of silently scoring the DB row. This is a score-comparability era boundary for sigma evals -- prior sigma eval runs scored DB text, not the committed fixtures, so historical sigma scores are not directly comparable to runs after this fix.
- **Eval bundle article-text provenance now matches scored fixtures** (2026-07-22): forensic eval bundles now prefer `config_snapshot.eval_fixture_content` for subagent eval executions and label the input source as `eval_fixture`, with the exported SHA-256 matching `eval_fixture_content_sha256`. Non-fixture executions still fall back to `articles.content` from Postgres. This closes a misleading bundle-export gap where `inputs[].article_text` could report mutable DB text the model never saw.
- **Eval fixture refresh now retains known-good URLs independently** (2026-07-21): `fetch_eval_articles_static.py` no longer abandons an entire fixture set when one URL hits a bot wall, transient fetch failure, content-regression guard, or ground-truth-retention guard. Rejected URLs retain their existing fixture entry while healthy siblings refresh atomically; partial refreshes exit nonzero and report each retained reason. Refreshed the stale HuntQueries, WindowsServices, and NetworkIndicators fixture content with Unicode preserved. No ground truth changed; scores before and after this corpus refresh are not directly comparable.
- **Shared eval fixtures reconciled to one local text per URL** (2026-07-21): aligned the eight previously divergent shared URLs without a network refresh, copying the source-faithful or better-cleaned local fixture into the lagging `windows_services` and `sigma` entries only after confirming it retained at least as much ground-truth support for every consuming set. No ground truth or expected counts changed. Eval scores for `windows_services` and `sigma` before and after this corpus correction are not directly comparable.
- **Content cleaning preserves Unicode detection signal** (2026-07-21): `ContentCleaner.clean_text_characters()` no longer replaces all non-ASCII characters with spaces. Unicode PowerShell flag dashes, typographic quotes, and non-Latin report content are retained verbatim; non-breaking spaces still fold to ordinary spaces. Newly ingested or re-fetched articles can therefore have different content hashes, keyword matches, and hunt scores than ASCII-stripped predecessors.
- **Semantic button-class migration reverted after shipping regressions** (2026-07-20): the three-commit sweep (2026-07-17/18) that replaced page-local and raw-Tailwind button styles with the global `.btn-analyze`/`.btn-manage`/`.btn-execute`/`.btn-danger`/`.btn-secondary`/`.btn-sm` classes shipped two real bugs alongside the restyle: the "Remove annotation" confirm button in `article_detail.html` lost its "Remove" label entirely (rendered blank), and the Huntable/Not-Huntable classification buttons each picked up a stray duplicated target emoji glued onto the tag. Reverted the button-class and CSS changes (17 templates plus the now-unused `.btn-secondary`/`.btn-sm` rules in `theme-variables.css`) back to their pre-2026-07-17 styling, fixing both regressions as a side effect. Left the unrelated work from the same commits untouched: the `SourceSyncService` source-init unification, the `content_hashes` table-drop migration, the ModalManager native-dialog replacement, and the skeleton-loading/empty-state partials.
- **Source initialization now uses the canonical async sync service** (2026-07-17): `cti init` now limits `SourceManager` to loading, validating, and configuring source definitions, then persists them through `SourceSyncService` and `AsyncDatabaseManager`, the same implementation used by `cti sync-sources`. This removes the second source-to-database synchronization implementation and prevents the sync and async command paths from drifting again.
- **`Create-Huntable-Agent` skill name normalized to lowercase** (2026-07-17): the skill's frontmatter `name` was `Create-Huntable-Agent` while its directory and every invocation reference used the lowercase `create-huntable-agent`, registering the skill twice under two case variants. Frontmatter now matches the canonical lowercase form.
- **Sync and async database managers unified on shared statement builders** (2026-07-17): `DatabaseManager` (sync, CLI callers) and `AsyncDatabaseManager` (async, web callers) reimplemented ~20 shared queries by hand and had already drifted -- three different `list_articles` sort orders, filters supported on only one side, a duplicated `archived` filter, and a dead CLI `search`/`export` filter path that crashed because `ArticleListFilter` never carried the fields the CLI commands built (`limit`, `author`, `tag`). New `src/database/statements.py` defines each shared query shape exactly once as pure SQLAlchemy statement builders; both managers execute the same statement their own way (sync `Session`, async `AsyncSession`), so per-manager query drift is no longer possible. Intentional divergences (CLI default-sorts `published_at`, web default-sorts `discovered_at`; async's article-detail lookup includes archived rows, the CLI's does not) are now explicit function parameters instead of silently-diverged copies; execution-side behavior (async's error-swallowing-to-`[]`, annotation-count injection, content deferral; sync's error propagation) is preserved. `ArticleListFilter` gained the full field union, fixing the CLI `search`/`export` crash; author/tag filtering now uses a `JSONB` containment cast (the JSON columns have no `LIKE`/contains operator in PostgreSQL). The hunt-score sort expression was hardened to extract via `->>` instead of `CAST(... AS VARCHAR)`, which previously crashed `CAST AS NUMERIC` on a JSON-null or quoted-string metadata value. Content-hash deduplication was also canonicalized onto `articles.content_hash` for both managers -- the legacy `content_hashes` ledger table was written only by the sync bulk-create path (312 rows against a 6,406-article corpus), so the CLI ingestion path had been deduplicating against roughly 5% of known content; a follow-up task tracks retiring the now-write-only ledger table. New parity drift-guard tests compile the statement each manager actually executes for the same filter and assert identical SQL.
- **Source config sync no longer crashes on the synchronous DatabaseManager** (2026-07-17): `SourceManager._sync_sources_to_db` awaited `list_sources`/`create_source`/`update_source`/`delete_source`, but every live caller (`cti init` via `get_managers`) passes the synchronous `DatabaseManager`, so the first `await` raised `TypeError: object list can't be used in 'await' expression`. Dropped the awaits to match the `DatabaseManager` annotation and the sibling `get_sources_due_for_check`, and fixed the update branch to read `check_frequency`/`lookback_days` from the inner `SourceConfig` (loader-built `SourceCreate` silently drops the top-level kwargs). The `cti sync-sources` command (`SourceSyncService` + `AsyncDatabaseManager`) was unaffected.
- **HTML cleaner no longer guillotines article bodies on substring class matches** (2026-07-17): `ContentCleaner` matched unwanted class/id patterns as substrings, so `ad` inside `block-paragraph_advanced` (Google Cloud TI) or `bg-gradient-to-b`/`shadow-xl`/`leading-tight` (NCSC UK, Tailwind) decomposed real body containers -- gutting a 34,834-char article to 589 chars and a 5,370-char body to 278 chars in production. Matching is now whole-token (split on whitespace and `-`/`_`).
- **CLI `collect` no longer crashes on a re-collected article and no longer drops the rest of the batch** (2026-07-17): the CLI collection path (`src/cli/commands/collect.py`) deduplicated by `content_hash` only, so a re-fetched article whose `canonical_url` already existed but whose content changed (new hash) slipped past dedup and hit the `uq_articles_canonical_url` unique constraint; the resulting flush error then poisoned the session transaction and rolled back every genuinely-new article in the same bulk insert. `collect.py` now also fetches `existing_urls` and passes them to `process_articles` (matching the worker/API collector), and `DatabaseManager.create_articles_bulk` wraps each row insert in a `begin_nested()` savepoint so a single `canonical_url` conflict is recorded in the returned `errors` list and skipped instead of aborting the whole batch. New wiring test (`tests/cli/test_collect_dedup_parity.py`) and real-DB regression test (`tests/integration/test_bulk_insert_canonical_url_resilience.py`, verified to fail pre-fix).

### Changed
- **`workflow.html`'s execution functions extracted to `/static/js/workflow/executions.js`** (2026-07-29): first slice of the incremental effort to break up `src/web/templates/workflow.html`, a 17,572-line Jinja template holding roughly 345 inline JS functions across 9 `<script>` blocks -- effectively an unbundled, unreviewable SPA. The 2,054-line `// Execution Functions` region (37 functions: status/step badges, sort state and header rendering, `loadExecutions`/`renderExecutions`, the ~1,150-line `viewExecution` detail modal and its tab/sub-tab rendering, trace-bundle download, retry, the trigger-workflow modal, filtering, pagination, and the stuck/stale/cancel-all maintenance actions) moved verbatim into a new versioned static module; the template keeps a pointer comment at the old location, matching the `escapeHtml`/`formatTimestamp` convention established on 2026-07-29. Content is byte-for-byte identical to the removed lines -- 6 insertions against 2,054 deletions. **Load order is the load-bearing detail**: the new `<script src>` sits *after* the main inline block, because the extracted functions read `executions` and `totalExecutions` (top-level `let`s declared at template line 3788 and still inline), and *before* the column-resize block, which reads `window.loadExecutions` at parse time and monkey-patches it to pin column widths after each load -- extracting to a file loaded any earlier or later breaks one of those two. Three variables local to the region (`executionSortState`, `_execPage`, `_execLimit`) were confirmed to have no references outside it, and no sliced function is redefined elsewhere in the template, so relocating the block past other definitions cannot flip which declaration wins. Kept as a classic script rather than an ES module so the global-scope contract with the remaining inline code is unchanged. Verified in the browser at `:8001`: no console errors, 50 rows and the 540/1/439/100 stat tiles render, sorting rebuilds the header with indicators, the detail modal opens and closes, and `window.loadExecutions` is confirmed to still be the resize wrapper. All 46 Playwright `workflow` project tests pass, including the 26 in `execution_detail_tabs.spec.ts` and `workflow_executions_pagination.spec.ts`.
- **Workflow state is now runtime-validated** (2026-07-23): `WorkflowState` is a strict Pydantic model accepted natively by LangGraph, rejecting missing, mistyped, and undeclared state channels at workflow boundaries. Its mapping-compatible API preserves existing node behavior while the state is incrementally migrated to attributes; the initial state now explicitly seeds all channels, including platform and CmdlineExtract intermediates.
- **Trusted-header authentication audit boundary documented** (2026-07-23): successful login, logout, MFA, session, and failed-login events remain the upstream proxy / IdP's responsibility in trusted-header deployments. Operators should correlate those SIEM records with Huntable's `X-Request-ID` authorization-denial and application-mutation audits.
- **CmdlineExtract prompt: defanged-URL angle-bracket fidelity rule propagated** (2026-07-21): synchronized the approved contract carve-out to the durable CmdlineExtract seed and all nine quickstart presets, then pushed it through the workflow prompt API. Enclosing angle brackets around a defanged URL in a command are stripped as source-rendering artifacts; `hxxp://` and `[.]` remain verbatim.
- **CmdlineExtract contract: cd exclusion and continuation-character scope** (2026-07-21): removed `cd` from valid built-ins and explicitly skips standalone `cd <path>` because it changes only process-local shell state and leaves no observable telemetry artifact; file-effect built-ins remain eligible. Scoped the continuation-character skip to a caret or PowerShell backtick at the end of a physical line, while preserving and extracting qualifying mid-token caret obfuscation. Mirrored the contract change to the drop-in. Zero GT delta: no cmdline ground truth contains `cd`, and existing caret-obfuscated schtasks items remain valid, so scores remain comparable.
- **Eval corpus is fixture-first and re-baselined** (2026-07-21): subagent eval executions now carry the committed `articles.json` body in their immutable config snapshot and feed it to the extractor, rather than using mutable `articles.content` from the development database. Per-execution fixture SHA-256 metadata makes the scored input auditable. Performed one deliberate re-scrape of the non-Sigma eval corpus through `ContentCleaner`, preserving block newlines and reusing a single fetched snapshot for duplicate URLs. The refresh utility is atomic per fixture set: a failed URL leaves that set unchanged. This is a score-comparability era boundary; routine ingestion and DB refreshes do not refresh eval fixtures.
- **Tests that mutate workflow config are blocked against the dev app on :8001** (2026-07-21): `tests/conftest.py` now refuses to run `agent_config_mutation`-marked tests (PUT /config, prompt/preset endpoints) against the dev app, which would overwrite the operator's live configuration. The guard fires when the test targets `127.0.0.1:8001` without `USE_ASGI_CLIENT=1`; override with `ALLOW_LIVE_DEV_SERVER=1`. Also removed dead `async_debugger`/`performance_profiler` fixtures and `skip_on_ci`/`skip_on_docker`/`skip_on_localhost` helpers that referenced nonexistent `tests.utils.test_environment` symbols. Fixed `run_tests.py` docstring to reflect that no test web container exists.
- **Articles list density refreshed while preserving the legacy palette** (2026-07-21): compacted the `/articles` triage list, filters, bulk-action bar, and pagination into a denser grid for faster scanning, while restoring the pre-redesign gray article cards, blue title links, muted keyword chips, and green/yellow/red RegexHuntScore badges. The row keeps full-row navigation, compact metadata, copy-content access, annotation counts, and mobile wrapping without the newer purple score ramp.

- **CmdlineExtract contract: defanged-URL angle brackets are stripped** (2026-07-21): the FIDELITY section now carves out one exception to "preserve exactly as written" -- enclosing `< >` around a defanged URL inside a command line is a source-rendering artifact, not typed input, and is stripped; `hxxp://`, `[.]`, and every other defanging convention are still preserved verbatim. Found during the CmdlineExtract eval-fixture audit: the contract's "preserve exactly / do not interpret rendering semantics" rules led three independent contract-compliant extractions to emit `curl -o sh2.txt <hxxp://173.254.204[.]72/sh2.txt>`, while ground truth stores the bracket-free `curl -o sh2.txt hxxp://173.254.204[.]72/sh2.txt` -- so a compliant model was being scored wrong for complying. Ground truth was already correct and is unchanged (zero GT delta across all five sinks), so eval scores remain comparable across this edit; only the contract and its drop-in mirror moved. Three other clarifications considered in the same audit (two-token subcommand scope, placeholder hard-list, `.exe`-suffix normalization) were deliberately NOT made: ground truth and contract-compliant extraction already agreed on all three, and the `.exe` variants stay distinct because CmdlineExtract models the typed command line, not resolved process execution.

- **LLM provider clients consolidated** (2026-07-20): shared Anthropic retry handling, `Retry-After` parsing, workflow provider AppSettings lookup, and LMStudio chat fallback/model-name retry logic now live in `src/services/llm_provider_clients.py`. `LLMService`, `SigmaGenerationService`, `/api/articles` AI routes, and Sigma queue routes delegate to the shared helpers while preserving existing private wrapper methods used by tests and legacy patch points. Added focused regression coverage for Anthropic non-retryable and transient retry behavior, LMStudio URL fallback metadata, and malformed LMStudio success responses.
- **LLM service internals split into focused modules** (2026-07-20): `src/services/llm_service.py` now composes prompt validation/constants, provider routing/context helpers, and provider HTTP clients from separate `llm_prompting.py`, `llm_routing.py`, and `llm_client.py` modules while preserving the public `LLMService` import surface and legacy patch targets. Added module-boundary regression tests so the split stays behavior-preserving for existing LLM, LMStudio, and prompt-parser callers.
- **Skeleton loading + empty-state partials added** (2026-07-18): created `src/web/partials/skeleton_loading.html` (animated pulse skeleton grid) and `src/web/partials/empty_state.html` (icon + message + refresh button). Integrated into `scraper_metrics.html`, `hunt_metrics.html`, and `analytics.html` with JS toggle to hide skeleton and show content on successful fetch or display empty state on error.
- **UI design system contract audit** (2026-07-21): added `docs/audits/ui-contract-audit-2026-07-20.md` auditing all 26 templates against `docs/contracts/ui-designer.md`. 5 rules pass (typography, dark mode, emoji, reserved classes, `!important`), 6 fail (card containers, hardcoded colors, ARIA, spacing, ASCII-only, modals). Top remediation priorities: 131 em-dashes/ellipses, 19 modal overlays missing `role`/`aria-modal`/`aria-label`, 57 hardcoded hex/rgba inline styles.
- **Native confirm()/prompt() replaced with styled ModalManager dialogs** (2026-07-17–18): added `ModalManager.confirm()` and `ModalManager.prompt()` (`src/web/static/js/modal-manager.js`) as Promise-based methods that dynamically create styled confirmation and input modals using the project's theme-variable system. Migrated all 31 native `confirm()` and 13 native `prompt()` calls across all 9 templates (`agent_evals`, `agent_evals2`, `article_detail`, `articles`, `ml_hunt_comparison`, `observable_training`, `settings`, `workflow`) to the new API, with context-appropriate titles and button colors for destructive, warning, informational, and input actions. One sync function (`promptForArticleId`) was converted to `async`.

### Removed
- **Dead `specs/README.md` removed** (2026-07-21): deleted the orphan specs directory README that had no content after prior cleanups.
- **Dead Sigma service orphans removed** (2026-07-20): removed `sigma_behavioral_normalizer.py`, `sigma_extended_validator.py`, and `sigma_huntability_scorer.py` plus their self-only tests after confirming they had no runtime imports. The active Sigma novelty path already uses `sigma_atom_precompute.py` and the external `sigma_atom_similarity` package for canonical atom extraction/comparison, and generated-rule validation continues through `sigma_validator.py`; current docs were corrected to stop advertising the deleted services as active pipeline components.
- **Confirmed-dead /api/eval/* + debug + demo endpoints removed** (2026-07-18): removed 6 endpoints with zero callers (confirmed via repo-wide grep across templates, JS, tests, docs, scripts): `POST /api/eval/hallucination`, `POST /api/eval/relevance`, `GET /api/eval/metrics`, `GET /api/eval/comparison` (`src/web/routes/evaluation.py`); `GET /api/test-route` debug registration check (`src/web/routes/debug.py`); and `GET /analytics/hunt-metrics-demo` plus its 1036-line `hunt_metrics_demo.html` template. The `/api/eval/*` router had grown from 4 to 10 endpoints since the original dead-code scan -- the 6 surviving endpoints (history, agent-metrics, trends, os-detection-manual-results, observables-count-results, rank-agent-benchmarks) have live template callers in `evaluations.html` / `agent_evaluation.html` and were preserved. `POST /api/test-hf-key` was also kept: it has a direct test caller (`test_ai_key_test_logging.py`) and validates Hugging Face tokens. `EvaluationTracker.compare_evaluations()` remains in use by the `/evaluations/compare` UI route. Net: -1,118 lines. Added `tests/api/test_removed_dead_endpoints.py` as a regression guard asserting removed routes stay gone and surviving routes remain registered.
- **Legacy `content_hashes` ledger table retired** (2026-07-17): follow-up to the sync/async dedup canonicalization onto `articles.content_hash` -- nothing read `content_hashes` anymore, and the ledger was write-only (312 rows vs 6,406 articles). Removed `ContentHashTable` from `src/database/models.py`, the `create_articles_bulk` writer and the schema-ensure PK DDL in `src/database/manager.py`, the `delete_article` cleanup in `src/database/async_manager.py`, the redundant `ContentHashTable` collision check and upsert in `scripts/backfill_image_ocr.py` (its now-unread `ArticleUpdateIntent.upsert_content_hash` field was also dropped), and the `content_hashes` cleanup step in `scripts/maintenance/dedup_articles.sql`. New `scripts/migrate_drop_content_hashes_table.py` drops the table (`DROP TABLE IF EXISTS`, idempotent) -- **run it before/alongside this deploy**: `content_hashes.article_id` has no `ON DELETE CASCADE`, so until the table is dropped, deleting one of the legacy articles that still has a row there will hit a foreign-key violation in `delete_article()` (now that its explicit cleanup delete is gone) and silently return `False` instead of `True`.

### Security
- **CI: GitHub Actions pinned to commit SHAs with a lint gate** (2026-07-17): every `uses:` in `.github/workflows/*.yml` now pins a full 40-char commit SHA (with a `# vX` comment so Dependabot's github-actions ecosystem still opens version-bump PRs), and a new `lint.yml` job fails the build on any Action not pinned to a SHA -- closing the mutable-tag supply-chain vector (the `tj-actions/changed-files` compromise class). Dependabot also gains a 7-day `cooldown` on all three ecosystems.
- **Side-effecting GET routes converted to POST / moved behind startup** (2026-07-17): two GET routes mutated database state on every call but, being safe-method routes with no explicit manifest rule, resolved to `AUTHENTICATED` (any logged-in user, no role gate) and were inherently outside CSRF token coverage. `GET /api/articles/{id}/sigma-matches` (`ai.py`) wrote `sigma_similar_cache` into article metadata via `update_article` and `force=true` re-ran an expensive behavioral-novelty assessment; converted to `POST` so the existing `/api/articles/*` (ai/llm_optimized_endpoint) manifest rule applies `ROLES` (operator/admin) + `CSRF REQUIRED`, and the `force=true` recompute is no longer an unprivileged compute/DoS lever. The `article_detail.html` fetch was updated to `method: 'POST'` (the base.html CSRF shim auto-injects the token for same-origin POSTs). `GET /api/workflow/config` (`workflow_config.py`) lazily created a default `AgenticWorkflowConfigTable` row and seeded `AppSettingsTable` threshold on first call; the lazy-init logic was extracted into `ensure_default_workflow_config()` and moved to application startup (`modern_main.py` lifespan), leaving GET truly read-only (returns 404 if no config exists, reads threshold without writing, uses in-memory prompt defaults without persisting). The `ensure_workflow_config_schema` test fixture now seeds the config row the same way. New `tests/api/test_safe_get_side_effects.py` (5 checks) asserts GET sigma-matches returns 405 (no handler, no cache write) even with `force=true`, GET workflow/config returns 200 with fixture-seeded config, the route manifest classifies POST sigma-matches as `CSRF REQUIRED` + `ROLES` with no GET route registered, and GET workflow/config remains a safe method. Not the SRF-2 auth-bypass class (auth was already required) -- impact is low, but side-effecting GETs violate safe-method expectations and dodge CSRF.
- **SRI integrity+crossorigin added to external CDN resources** (2026-07-17): pinned Tailwind CSS (`3.4.17`), Chart.js (`4.5.1`), and the Google Fonts stylesheet to versioned URLs with SHA-384 integrity hashes and `crossorigin="anonymous"` in `base.html`, closing the supply-chain vector where an attacker compromising a CDN could serve modified scripts.
...
- **Backup restore detects psql statement errors and raises PostgreSQL shared memory default** (2026-07-06): restore verification now runs `psql` with `ON_ERROR_STOP=1`, extracts `ERROR`/`FATAL`/`PANIC` lines from stderr even when psql exits 0, and connects test database create/drop commands to the `postgres` maintenance database. The local compose Postgres service now defaults `POSTGRES_SHM_SIZE` to `1g` so pgvector/HNSW index restore tests do not fail with Docker's small shared-memory default. Added unit coverage for psql stderr detection and restore command wiring.
- **Browser extension docs: cross-link client-side OCR/Vision with server-side ingest OCR** (2026-07-06): `docs/guides/browser-extension.md`'s Image Text Extraction section and `docs/features/image-ocr-ingest.md` each documented one OCR pipeline with no reference to the other, leaving no explanation of which pipeline runs when or that markers inserted by one are recognized and skipped by the other (`_parse_existing_ocr_urls` in `vision_ocr_service.py`). Cross-linked both pages. Also replaced the stale, duplicate `browser-extension/README.md` (missing the OCR/Vision feature entirely) with a short pointer to the actively maintained `docs/guides/browser-extension.md`.

### Security
- **SRF-2: GET reads on credential- and admin-scoped routes now require the same role as their write counterpart** (2026-07-10): the route-authorization manifest (`route_manifest.py`) classifies GET requests against a separate rule table (`SAFE_ROUTE_RULES`) from mutating requests (`UNSAFE_ROUTE_RULES`); `/api/settings*`, `/api/backup/*`, `/api/model/*`, `/api/observables/training/*`, and `/api/sigma-queue/*` were role-gated (`admin`/`rule_reviewer`) on the write side but had no matching read-side entry, so GET fell through to the bare `AUTHENTICATED` default — any authenticated identity, regardless of role, could read live settings values (including provider API keys) via `GET /api/settings`. Added matching `SAFE_ROUTE_RULES` entries mirroring each write-side role gate, per the documented role model (`docs/guides/authentication.md`: `admin` owns "settings, credentials, ... backup/restore, model management"; `rule_reviewer` owns "Sigma queue review actions"). Also widened the settings-audit sensitive-key check (`settings.py::_is_sensitive_setting`) to reuse `audit_service`'s broader key-name allowlist (adds `password`/`credential`/etc., previously only `TOKEN`/`SECRET`/`API_KEY`) so audit-log redaction can't silently miss a differently-named credential setting. Regression tests added to `tests/api/test_route_family_authorization.py`.
- **SRF-1: production trusted-header mode fails closed on an empty proxy allowlist** (2026-07-06): with `APP_ENV=production` and `AUTH_MODE=trusted_header`, an empty `AUTH_TRUSTED_PROXY_IPS` let any direct client forge `X-Huntable-Verified` + identity headers and obtain admin (the peer gate in `parse_trusted_identity()` passes everything when the allowlist is empty). `load_security_config()` now aborts startup in that posture unless the documented break-glass `ALLOW_INSECURE_PRODUCTION_TRUSTED_PROXY_OPEN=true` is set for deployments whose direct access is blocked at the network level. Dev/test behavior unchanged. Production also now rejects non-literal allowlist entries (`*`, CIDR ranges) at startup — the peer gate is exact-match, so an unmatchable entry would otherwise cause a silent all-requests-denied outage (adversarial-review finding). The SSO golden path was updated in the same change so it keeps booting: `config.sh sso` production hardening now writes the override with a printed warning (the overlay's proxy container IP is dynamic; its isolation comes from removing the direct app port), and `docs/guides/enterprise-sso.md`, `deploy/sso/README.md`, `docs/guides/authentication.md`, and `.env.example` document both paths. Regression tests added to `tests/unit/test_security_config.py`. The optional `identity.py` peer-gate tightening remains an open operator decision (it would change the documented optional-peer-validation contract).

### Added
- **NetworkIndicatorExtract sibling/boundary rules + drop-in prompt** (2026-07-05): closed the extractor-standard compliance gap left when NetworkIndicatorExtract shipped in v7.2.0 — all six pre-existing extractor contracts (`docs/contracts/{cmdline,proctree,registry,services,scheduled-tasks,huntquery}-extract.md`) now list NetworkIndicatorExtract as a sibling with a reciprocal boundary rule mirroring the already-shipped language in the live seed prompts (`src/prompts/*`), so this is doc-sync, not an extraction-semantics change. Added the missing seventh drop-in prompt (`docs/contracts/network-indicator-extract-dropin.md`), registered it in the mkdocs nav, and linked it from the contract. New `tests/unit/test_extractor_contract_sibling_sync.py` (28 checks) pins the cross-listing mandate across contracts, seed prompts, drop-ins, and nav.
- **Image OCR Ingest feature page** (2026-07-05): dedicated `docs/features/image-ocr-ingest.md` documenting the Tesseract ingest pre-pass — SSRF guard model (DNS-pinning httpcore backend, per-hop redirect re-validation, private/CGNAT/mapped-IPv6 rejection, byte/pixel caps), `OCR_INGEST_ENABLED` + per-source `image_ocr_enabled` tri-state mechanics, the five seeded news-source opt-outs, `ocr_status` vocabulary, `scripts/backfill_image_ocr.py` usage, and the `/api/health` tesseract block. Registered in the mkdocs nav and cross-linked from `docs/index.md`. New `tests/unit/test_docs_structure_sync.py` pins feature-page nav registration and `docs/solutions/` frontmatter schema; `tests/unit/test_audit_action_vocabulary.py` pins the spec-reserved audit action names against future dead-code sweeps.
- **MCP ambient context resources** (2026-07-05): registered read-only MCP resources so agents can attach current Huntable context without bespoke tool calls: `huntable://sigma-queue/status` exposes global queue status counts, `huntable://sigma-queue/recent-rules` exposes the ten most recent generated Sigma queue entries, and `huntable://workflow/active-config` exposes the active config id/version, thresholds, toggles, model assignments, and prompt agent names while intentionally omitting prompt bodies. Added focused resource contract tests plus stdio-server registration coverage; docs updated in `docs/reference/mcp-tools.md`.
- **Enterprise auth Chunk C completion: workflow trigger/retry audit + initiating-actor attribution** (2026-07-04): closed the last deferred Chunk C task (Task 5). Human-initiated workflow trigger (`POST /api/workflow/articles/{id}/trigger`) and retry (`POST /api/workflow/executions/{id}/retry`) now record mandatory same-transaction `workflow.triggered` / `workflow.retried` audit events (audit-write failure rolls the mutation back and surfaces a 500 via the new `WorkflowAuditError`), and both persist a redacted-safe `initiated_by` snapshot (`user_id`/`email`/`roles`/`auth_mode`) under the execution's `config_snapshot` so worker-side attribution references the originating human without impersonation — service-originated triggers (ingest auto-trigger) carry no `initiated_by` and are attributed to the service by its absence. `WorkflowTriggerService.trigger_workflow` gained backward-compatible `initiated_by`/`audit_event_factory` parameters (the route builds the event; the service stays HTTP-free). Added the denied-request audit contract suite `tests/api/test_audit_denials.py` (401/403/unclassified each emit exactly one best-effort `auth.request_denied`; allowed requests emit none; denials never depend on audit storage) plus workflow trigger/retry atomicity tests in `tests/api/test_audit_mutations.py`. With MCP writes already service-attributed and no worker/scheduler code emitting audit events or internal HTTP calls, this completes Chunk C service-identity wiring; backup/model/embeddings/eval route audit remains the documented follow-up.
- **Risk-tiered MCP write tools** (2026-07-03): the Huntable MCP server now exposes scoped, audited write tools for low-blast-radius actions while keeping `execute_sql` permanently read-only. Auto-executable tools cover workflow retry/cancel, source active-state toggling, article reviewed markers, and annotation create/update/delete; higher-risk Sigma queue mutations and article deletion create pending `mcp_write_confirmations` rows instead of applying production mutations through MCP. New audit persistence records MCP writes and confirmation requests in `audit_events`, with metadata redaction and focused regression coverage in `tests/unit/test_mcp_write_tools.py`. Docs updated in `docs/reference/mcp-tools.md`, `README.md`, and `docs/index.md`.
- **Enterprise auth, authorization, and audit foundation** (2026-06-18): added trusted-header identity enforcement with a central route manifest, fail-closed authorization middleware, narrowed public health surface, role-gated high-risk route families, and durable database-backed audit events. Audit support includes the `audit_events` table, schema validation, redaction helpers, same-transaction settings/secret audit writes, best-effort authorization-denial audit, admin-only audit read/export/retention endpoints, and documentation for retention plus the DB-backed tamper-risk boundary. OAuth/OIDC and full mandatory-audit migration for every mutation family remain follow-up work.
- **Enterprise auth Chunks B & C: CSRF, mandatory audit, and SSO deployment** (2026-06-25): completed the enterprise-auth foundation. Added signed, user-bound CSRF protection (`src/web/security/csrf.py`, `CsrfMiddleware`, a base-template token meta tag + same-origin `fetch` shim) gated by `CSRF_ENABLED=auto` with production fail-closed on a missing/weak `SECRET_KEY`. Migrated mandatory same-transaction audit to the sigma_queue, workflow (cancel/cleanup/stale), source-config, annotation create/delete, and scheduled-jobs route families via backward-compatible optional-session refactors in `async_manager`/services; added service-identity audit helpers for workers and `AUDIT_RETENTION_DAYS` wiring. Added an opt-in enterprise-SSO step to `setup.sh` and a `config.sh` CLI (`sso` / `sso --disable` / `rotate-secret` / `set` / `show`) that edits `.env` without re-running the installer, a ready-to-run nginx + oauth2-proxy reverse-proxy scaffold (`deploy/sso/`), a localhost login-gate variant for testing Entra/Google with no domain or TLS (`deploy/sso/local/`), and an Enterprise SSO setup guide (`docs/guides/enterprise-sso.md`). Backup/model/embeddings/eval audit remains documented follow-up.
- **Local Entra login-gate toggle** (2026-06-25): `./config.sh entra on|off|status` (delegating to `deploy/sso/local/gate.sh`) forces every visitor on a local Docker setup through the Entra login by un-publishing the `cti_web` host port via a committed `docker-compose.entra-isolate.yml` overlay (`ports: !override []`), so the oauth2-proxy gate on `:4180` becomes the only entry point. The helper self-locates the running stack from the container's compose labels (works from any checkout), requires an interactive confirmation that a real login succeeds *before* it closes the port, and fails closed -- auto-reverting the port if the app is not reachable through the gate or if a stopped container would otherwise be misreported as gated. This is a login gate (authentication only); per-user roles remain the trusted-header RBAC path. `config.sh`'s `.env` commands stay docker-free; only `entra` orchestrates containers.

### Fixed
- **AGENTS.md solutions frontmatter description** (2026-07-05): the Orientation table claimed `docs/solutions/` frontmatter carries a `tags` key; none of the 11 solution files do and no code consumes one — corrected to the actual key set (`title`, `date`, `module`, `problem_type`).
- **setup.sh dropped interactively-entered OpenAI/Anthropic keys and the Redis password** (2026-07-05): `create_env_file` substituted secrets into `.env.example` via `sed` on placeholder strings (`your_openai_api_key_here`, `your_anthropic_api_key_here`, `your_secure_redis_password_change_this`) that no longer exist in the template -- `.env.example` ships those keys as bare `KEY=` with no placeholder text, and has no `REDIS_PASSWORD` line at all, so the `sed` calls were silent no-ops and entered values never reached the generated `.env`. Switched to `startup_set_env_key` (mirroring the existing `SECRET_KEY` fix) so `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `REDIS_PASSWORD` are always written directly. Added `tests/unit/test_setup_env_secret_substitution.py`.

### Removed
- **Dead-code cleanup: 8 orphan files + 49 zero-reference symbols (-2,203 lines)** (2026-07-05): deleted eight modules nothing imports (`cmdline_caliper`, `query_safety`, `sigma_novelty_detector`, `encoder_classifier`, `regex_windows`, `cmd_extractor`, `dspy_cmdline_module`, `text_highlight` — mostly BERT/sklearn-era leftovers) plus 27 zero-reference class methods, 14 module functions, and 4 dead constants across 28 modules, each re-verified by fresh whole-repo grep before removal. The seven unreferenced audit `ACTION_*` constants were kept with a reserved-vocabulary comment (five are enumerated in the enterprise-auth auditability spec's stable action names; the two `backup.*` names await the spec's open backup-permission decision). Per-section test suite counts were identical before and after (smoke 85 / unit 4129 / api 469 / integration 39 passed, 0 failed); the stale `sigma_novelty_detector` row was dropped from `docs/features/sigma-rules.md`.
- **Domains + Products classification dimensions (Phase D) — removed as YAGNI cruft** (2026-06-25): the entity-driven Domains/Products dimensions shipped in 7.5.0 (`src/services/entity_dimension_classifier.py` + `config/domain_classification_kb.yaml` / `config/product_classification_kb.yaml`) were computed in `os_detection_node`, stored in `error_log.os_detection_result.{domains,products}`, and rendered as static labels in the Platform Detection trace — but had **zero functional consumer** from day one (verified across the workflow, API serializers, the MCP server, exports, DB queries, Sigma generation, and downstream nodes: nothing read them for any routing, filter, grouping, or decision). Domain/product routing was never built (design open-question #4 unresolved) and not scheduled, so the producer was inert trace decoration that implied a capability the system did not have, plus a second parallel keyword scorer and two duplicated-token KBs to maintain. Excised as a clean unit — classifier service, both KB YAMLs, workflow wiring (`agentic_workflow.py` os_detection_node), the two trace-UI rows (`workflow.html`), the 6 unit tests, and the Playwright `workflow_platform_dimensions` spec — with no schema migration (the fields lived in the existing `error_log` JSONB). The curated KBs (42 domain + 19 product entries) remain recoverable from git history (commit `8c03d744`) for a clean re-add if a real consumer is ever designed. Design/rationale: `docs/superpowers/specs/2026-06-19-entity-driven-platform-classification-design.md` (Phase D marked ⛔ REVERTED).

### Changed
- **Sigma generation: network-observable abstraction guidance + reasoning-model token budget** (2026-07-04): both Sigma generation prompts (`sigma_generation.txt`, `sigma_generate_multi.txt`) gain a "Network Observable Abstraction (no atomic IOCs)" section — atomic network values (full URLs/domains/IPs/User-Agents, JA3/JA4, cert hashes) are evidence only and must be generalized into durable behavior (`|startswith`/`|endswith`/`|contains` shapes) or discarded, never emitted as bare selection values. All 9 quickstart presets synced. Reasoning-model `max_tokens` for Sigma generation raised 4000 -> 10000 (`sigma_generation_service.py`). Landed alongside workflow execution-detail UI wiring (Langfuse debug links, trace/eval bundle export) in `workflow_executions.py` / `workflow.html` with Playwright coverage.
- **Platform Detection trace labels renamed to match the post-embedding reality** (2026-06-25): the OS/platform classifier has used deterministic keyword-KB scoring (`kb_scoring`), not embeddings, since the 2026-06-19 entity-driven rewrite, so the trace's "similarity" wording was stale. Renamed the per-execution Platform Detection labels in `workflow.html`, `workflow_executions.html`, and `article_detail.html`: "View Platform Similarity Scores" -> "View Platform Score Distribution" and "Max Similarity" -> "Top Platform Score". Display text only; the underlying `similarities` / `max_similarity` data keys are unchanged (backward-compat). The Sigma rule-dedup "Max Similarity" (genuine cosine/atom similarity) was deliberately left untouched.
- **MkDocs nav curation — promote reference docs out of `not_in_nav`** (2026-06-22): the v7.5.0 CI-unblock (commit `4ced3c10`) parked 43 reference/internal pages in `mkdocs.yml`'s `not_in_nav` and fully excluded `docs/superpowers/`. Reviewed all 43 against their content and promoted 15 genuinely user/developer-facing pages into the published nav: new **Deployment** (`deployment/docker-architecture`, `deployment/technical-readout`) and **Operations** (`operations/ml-model-runbook`, `operations/verify-ml-versions-backup`) sections; **Architecture** gains `internals/architecture` (diagrams) + `architecture/agent-config-schema`; **Development** gains `development/ui-test-tiers` + `development/sigma-eval-system`; **Reference** gains `reference/prompt-mapping-table`, `reference/versioning`, and `reference/do-not` (anti-patterns); **Contracts** gains `NetworkIndicatorExtract` as a peer plus a **Drop-in Prompts** subsection for the six `*-extract-dropin.md` copy-paste variants; and a top-level **Brand & Style Guide**. The remaining 28 stay out of nav (deprecated QA contracts, dated audits/plans, `solutions/**`, `reports/**`, `adr/**`). `docs/superpowers/` moved from `exclude_docs` to a `not_in_nav: superpowers/` glob, so the 16 design specs/plans now build and are reachable by URL but stay out of the menu; the six dead `../../../src|tests/…` relative links in the three enterprise-auth specs were rewritten to absolute GitHub blob URLs (relative links from `docs/` to source files don't resolve under MkDocs). `python -m mkdocs build --strict` exits 0 before and after.

## [7.5.0 "Europa"] - 2026-06-22
### Added
- **Linux-aware Sigma generation guidance** (2026-06-19): a reviewed-Linux-pilot follow-up. The base Sigma-generation prompt is Windows-tuned, so a per-platform/logsource group tagged `linux` now receives an **additive** Linux guidance block (`src/services/sigma_generation_service.py::_platform_sigma_guidance`; no edit to the base prompt file, so the quickstart presets stay in sync) steering toward Linux `process_creation` fields (`CommandLine`/`Image` full path, no Windows-only fields), **behavior-matching** detection, and Linux ATT&CK tags (`T1059.004`/`T1222.002`/`T1053.003`/`T1105`). Validated on article 4441: the chmod rule went from `CommandLine|contains: /tmp` (matches almost every command, mis-tagged `T1203`) to `CommandLine|contains: 'chmod 777 /dev/shm'` tagged `T1222.002`. This is the §10 "split the prompt if Linux quality lags" promotion, triggered by the live pilot rather than guessed.
- **Platform Telemetry Expansion — platform-aware extractor routing & Linux Sigma (phase one)** (2026-06-19): the agentic workflow is no longer Windows-only. *Platform Detection* (renamed from OS Detection) returns platform context instead of a hard pass/fail gate; extractors run only for the platforms they support, and the Windows-only extractors (`RegistryExtract`/`ServicesExtract`/`ScheduledTasksExtract`) are skipped on non-Windows evidence with structured `reason_code`/`supported_platforms`/`detected_platforms` records rendered in the per-execution trace. Every observable now carries platform + telemetry metadata (`platform`, `platform_confidence`, `platform_rationale`, `telemetry_category`, `telemetry_confidence`, `logsource_hint`). Linux command/process evidence generates backend-neutral Sigma with an explicit logsource plus `generation_basis`/`detection_readiness` metadata (kept out of the pySigma-validated YAML the same way `observables_used` is), and mixed Windows/Linux articles produce **separate per-platform/logsource rule groups** (a single combined rule is not phase-one behavior); macOS-only articles generate no macOS Sigma even under the full-content fallback. Linux-generated rules enter the **same review queue** distinguished by a platform badge, and Workflow Config gains a read-only **platform capability matrix**. Platform/telemetry fields flow through the existing execution/review APIs; new executions only (no backfill), legacy serializers unchanged. Proof: `tests/workflows/test_platform_telemetry_phase_one.py` (one test per acceptance bullet, every emitted rule validated through real pySigma) + workflow Playwright specs (`workflow_platform_capability_matrix`, `workflow_platform_badge`, `workflow_platform_detection`). Design: `docs/superpowers/specs/2026-06-17-platform-telemetry-expansion-design.md`; follow-ups in `2026-06-19-platform-telemetry-followups.md`.
- **Entity-driven platform classification replaces the embedding OS detector** (2026-06-19): the CTI-BERT/SEC-BERT embedding OS detector was measured non-discriminative for non-Windows content (0/4 on clearly-Linux articles, ~0.04 class margins, the `Other`/`multiple` tie-break pseudo-classes winning, and it classifies on the 512-token boilerplate head of raw pre-filter content) and is retired as the decider. New explainable hybrid: a deterministic entity-keyword **knowledge-base gate** (`src/services/platform_classifier.py` + `config/platform_classification_kb.yaml`) decides the common case for free, with cited evidence and **margin-based** confidence and an evidence floor that refuses to claim a platform on thin signal; an **LLM adjudicator** (`src/services/platform_adjudicator.py`, the configured gpt-4o-mini) narrows only the low-confidence / Unknown tail; and **ATT&CK technique citations** (`src/services/attack_platform_signal.py` + curated seed `config/attack_technique_platforms.json`, regenerable from MITRE `x_mitre_platforms` via `scripts/build_attack_platform_map.py`) **reinforce** KB-evidenced platforms only — reinforce-only because technique citations enumerate threat capabilities that span platforms and must not originate a verdict (live-validated: an originating signal forced a Unix-citing Linux article to `multiple(linux,macos)`, degrading its observables to `unknown` and suppressing Linux Sigma). Adds the **Domains + Products taxonomy dimensions** (`src/services/entity_dimension_classifier.py` + `config/domain_classification_kb.yaml` / `config/product_classification_kb.yaml`) computed deterministically and surfaced in the per-execution Platform Detection trace (stored in `error_log.os_detection_result`, no schema change). The dead embedding OS-detection code paths (`_detect_with_classifier`/`_detect_with_similarity`/`_load_classifier`/`_precompute_os_embeddings`, ~143 lines) were removed; the embedding *infra* (`_get_embedding`/`_load_model`/`train_classifier`) is retained as it is still shared by `huntable_windows_service` and the OS-classifier trainer. Verified end-to-end by a live pilot (article 4441 → 3 Linux `process_creation` rules queued with Linux badges, Windows-only extractors skipped). Design: `docs/superpowers/specs/2026-06-19-entity-driven-platform-classification-design.md`.
- **Server-side image OCR during article ingest** (2026-06-16): inline article images are now OCR'd locally with Tesseract (`pytesseract` + the `tesseract-ocr`/`tesseract-ocr-eng` binary added to both `Dockerfile` and `Dockerfile.prod`) during ingest, and the extracted text is folded into `article.content` as `[Image OCR: <url>]` blocks. Because the text lands in the article body *before* `ContentProcessor` cleaning, every downstream consumer — the Cmdline/ProcTree/HuntQueries/Registry/Services/ScheduledTasks extractors, Sigma generation, RAG search, and dedup — sees the image text with **no consumer-side changes** (the URL-header marker matches the existing `[Image OCR:]` regex in `src/web/routes/scrape.py`). New service `src/services/vision_ocr_service.py` runs an atomic pre-processing pre-pass (`ocr_raw_articles`) wired into all four source-ingest entrypoints — `check_all_sources`, `check_source`, `collect_from_source`, and the CLI `collect` (per-source, before the flatten) — each wrapped so an OCR failure can never break ingest. **Disabled by default and inert until enabled:** global env `OCR_INGEST_ENABLED` plus a per-source tri-state `source.config['image_ocr_enabled']` (no schema migration). The image fetch is **SSRF-guarded**: a custom `httpcore` network backend resolves DNS once and pins the validated IP at connect time (defeats DNS-rebind), rejecting loopback/link-local/private/CGNAT/IPv4-mapped-IPv6 targets, with per-redirect-hop revalidation, `trust_env=False`, a byte cap, and a Pillow decompression-bomb guard; the surface had an adversarial security review during implementation and a clean independent security scan. Candidate images are filtered strictly and scoped to the selected main-content node (no sidebar/related images). Also adds: a retry-stable backfill script (`scripts/backfill_image_ocr.py`) that recovers each row's `content_hash` basis by comparison (never by `content_hashes`-row existence), upserts `content_hashes`, and is collision-safe; a `tesseract` block in `/api/health` and a worker-startup probe; and spec-§6 observability logging. Behavior-preserving refactor of `ContentCleaner` (extracted `prepare_soup_for_selection` + `find_main_content_node`, golden-snapshot pinned) lets the pre-pass inject into the same subtree the cleaner later selects. Design: `docs/superpowers/specs/2026-06-15-image-ocr-ingest-design.md`; 164 feature tests + smoke green.
...
- **Real-Tesseract OCR regression tests** (2026-06-22): three unmocked-engine tests in `tests/services/test_vision_ocr_service.py` plus a checked-in golden fixture (`tests/fixtures/ocr/known_text_sample.png`) that prove the OCR path actually reads text — `ocr_image_bytes` recovers known tokens from a real PNG, `ocr_article_images` carries that text into a `[Image OCR: …]` content block end-to-end through the orchestrator with only the network fetch stubbed, and the `/api/health` Tesseract probe reports a real non-empty version on a true install. Every other OCR test mocks `pytesseract`, so none exercise the real binary; these guard against a silent Tesseract/pytesseract upgrade that degrades extraction and against a probe that disagrees with the install. All three auto-skip where the binary is absent and run in CI's Docker image (which installs `tesseract-ocr`).

### Changed
- **Image OCR ingest switched on by default** (2026-06-17): `OCR_INGEST_ENABLED` now defaults to `true` in `docker-compose.yml` for the web, worker, and cli services (was opt-in at initial ship, 2026-06-16). Five news sources with high image-to-signal noise ratios (general cybersecurity news outlets) were seeded with an explicit `image_ocr_enabled: false` override in `config/sources.yaml` to opt out individually; all other sources OCR by default. Design: `docs/superpowers/specs/2026-06-17-image-ocr-global-default-source-override-design.md`.
- **ATT&CK platform-signal map regenerated from the maintained source; version stamp fixed** (2026-06-21): `config/attack_technique_platforms.json` regenerated from MITRE's full `x_mitre_platforms` taxonomy — **46-entry curated seed → 210 platform-discriminative (sub-)techniques** (ATT&CK v19.1) used by the reinforce-only ATT&CK→platform signal. `scripts/build_attack_platform_map.py` was repointed from the **deprecated `mitre/cti`** bundle (which no longer embeds the `x-mitre-collection` version object, so the provenance stamp read `mitre_attack_stix:unknown`) to MITRE's maintained **`attack-stix-data`** repo, and the version lookup was extracted into a hardened `_collection_version()` that degrades to `"unknown"` — never the literal `"None"` — when the field is absent; `_source` now reads `mitre_attack_stix:19.1`. New `tests/scripts/test_build_attack_platform_map.py` locks version extraction + the platform-discriminative filtering doctrine. Design context: `docs/superpowers/specs/2026-06-20-unified-keyword-platform-scoring-design.md`.
- **Hunt scoring made platform-complete: high-fidelity macOS/Linux perfect discriminators** (2026-06-21): the keyword hunt scorer (`HUNT_SCORING_KEYWORDS`, `src/utils/content.py`) was ~99% Windows, so genuinely-huntable non-Windows articles scored near-zero — with zero perfect discriminators the geometric formula caps an article at ~25, well below the 85 auto-trigger gate, so macOS/Linux articles were dropped before processing. Added validated low-FP non-Windows carriers as **perfect discriminators** (the 75-pt tier — the only one that can move the 85 gate): macOS `osascript`, `do shell script`, `launchctl`, `LaunchAgents`, `LaunchDaemons`, `com.apple.quarantine`, `xattr`, `TCC.db`, `dscl`, `plutil`, `kextload`, `.plist` (`osascript`/`TCC.db` promoted from good); Linux `xmrig`, `memfd_create`, `chattr +i`, `proc/self/exe`, `history -c`, `rc.local`, `ld.so.preload`, `cron.d`, plus `base64 -d` and `chmod 777` (promoted from good after per-token context review). Generic admin tokens (`systemctl`/`crontab`/`chmod +x`/`nohup`/`insmod`/`bashrc`/`dev/shm`) stay **good-tier** to avoid benign-sysadmin false positives (`chmod +x` good vs `chmod 777` perfect is deliberate). The shared perfect-discriminator set (92→114) also fixes the content/junk filter for free — macOS/Linux command chunks are now auto-preserved. Real-article re-score: stuck non-Windows articles lifted from ~8–24 to **70–91** (BlueNoroff 23.7→89.4, Bad Apples 12.3→85.0, PeerBlight 73.1→91.3). The **85-gate-vs-75-perfect-asymptote** geometry remains the binding constraint for single/double-carrier articles; the configurable `auto_trigger_hunt_score_threshold` is the remaining lever (operator's call). Calibration spike + per-token FP analysis: spec §9.1–§9.4. Tests: `tests/test_threat_hunting_scorer.py::TestNonWindowsHuntScoring` (8, TDD) + content-filter/keyword consumers green. Counts: perfect 92→114, good 89→94 (docs updated).
- **Unified keyword/platform scoring — faceted registry as single source of truth** (2026-06-22): completes the design in `docs/superpowers/specs/2026-06-20-unified-keyword-platform-scoring-design.md`. **Phase 1:** `config/keyword_registry.yaml` (591 faceted entries — each carries an optional hunt-scoring *tier* and/or *platform* tag) becomes the one source of truth; `HUNT_SCORING_KEYWORDS` is now *derived* from it (`src/utils/keyword_registry.py`), guarded by a byte-equal parity test against a committed snapshot (decision D-A). **Phase 2:** `os_classification` is computed at scoring time (`build_os_classification`) and stored in `article_metadata` at every hunt-score persistence site (ingest/scrape/pdf/reprocess/rescore), go-forward. **Phase 3:** `os_detection_node` consumes that precomputed verdict via `detect_os(precomputed=...)` instead of re-scanning — the Windows safety net, low-confidence LLM adjudication, Windows-similarity override, and ATT&CK reinforcement are all unchanged; legacy articles fall back to a fresh scan. **Carrier drift-fix (§9.5):** 7 platform-discriminative §9 carriers (`do shell script`/`launchctl` → macOS; `memfd_create`/`chattr +i`/`proc/self/exe`/`rc.local`/`ld.so.preload` → Linux) now feed OS classification — cross-platform carriers (`xmrig`/`base64 -d`/`chmod 777`) and KB-covered ones deliberately excluded to avoid mis-routing/double-counting; 6/27 carrier articles change verdict (3 rescued from Unknown). **G4 cleanup (§14):** `config/platform_classification_kb.yaml` deleted, `classify_platforms`/`DEFAULT_KB_PATH` retired, and the `detect_os` fallback repointed to `project_platform` — the registry is the sole platform vocabulary, so precomputed and fallback share one verdict. **Phase 4 (single-pass scan-engine dedup) deliberately NOT built** (§13 — the hunt scorer's word-boundary matcher and the platform classifier's substring matcher can't unify in one pass without breaking parity, for marginal gain on a low-traffic axis). Hunt-score + junk-filter parity preserved throughout (tiers untouched). Tests: `tests/test_keyword_registry.py` + `detect_os`/platform-classifier/workflow suites; 133+ regression green.
- **Low-confidence platform-adjudication prompt is now operator-configurable** (2026-06-22): the LLM fallback (`platform_adjudicator`, which runs only when the deterministic registry classifier is low-confidence/Unknown) now uses the configurable `OSDetectionAgent` prompt as its **system** message — `build_adjudication_messages`/`adjudicate_platforms` gained a `system_prompt` param, and `os_detection_node` resolves `config_obj.agent_prompts["OSDetectionAgent"]` and threads it through `_maybe_adjudicate_platform`, defaulting to the built-in `ADJUDICATION_SYSTEM`. The strict-JSON output contract stays hardcoded in the **user** message so operator prompt edits can't break parsing. The `OSDetectionAgent` seed prompt was rewritten from the retired single-label format to the adjudicator's multi-platform contract — giving the previously-vestigial prompt a real purpose (it never fed the deterministic detector). Tests: `tests/services/test_platform_adjudicator.py` (custom system prompt used, blank→built-in default, passed to the LLM, seed aligns with contract).
- **Security dependency bumps (CVE remediation)** (2026-06-22): bumped six packages flagged by `pip-audit`. Runtime (pyproject `dependencies` + `requirements.txt`): `starlette` 1.0.1->1.3.1 (CVE-2026-48817/48818/54282/54283 - request/response + multipart; FastAPI 0.136.3 requires only `starlette>=0.46.0`, so the bump is resolver-safe), `pydantic-settings` 2.14.1->2.14.2 (GHSA-4xgf-cpjx-pc3j), `pypdf` 6.13.0->6.13.3 (GHSA-jm82-fx9c-mx94), `langsmith` 0.8.9->0.8.18 (GHSA-f4xh-w4cj-qxq8). Test-only (pyproject `test` group, not mirrored into the runtime `requirements.txt`): `pip` 26.1->26.1.2 (PYSEC-2026-196), `msgpack` 1.1.2->1.2.1 (GHSA-6v7p-g79w-8964) - both transitive via `pip-audit`. Lockfile bumped surgically with `uv lock --upgrade-package` (only the six stanzas changed). `torch`/`diskcache` left as-is (no upstream fix, verified not reachable). `pip-audit` now reports 2 (those two) down from 11; full category suite green on the new pins (smoke/unit/api/integration/ui).

### Removed
- **OS-detection embedding model config (CTI-BERT/SEC-BERT) for Platform Detection** (2026-06-22): platform detection has been entity-driven since 2026-06-19, so the embedding model was never loaded for it — the config field was dead and the Workflow Config UI misleadingly showed a "Platform Detection Embedding Model" dropdown. Removed end-to-end: the `Embeddings.OsDetection` schema field + its `OSDetectionAgent_embedding` serialization (`workflow_config_schema.py`), the load/save mappings (`workflow_config_loader.py`), the v1→v2 build (`workflow_config_migrate.py` now strips the deprecated key from stored configs so they pass `extra="forbid"`), the two config-modal dropdowns + all save/preset/display JS (`workflow.html`, replaced with a "deterministic — no model" note), the vestigial workflow read (`agentic_workflow.py` now constructs `OSDetectionService()` with no model), and the embedding key from the 9 quickstart presets. Stored configs carrying the old field load cleanly (stripped, no validation error). Sigma similarity's embedding (`Embeddings.Sigma`) and the separate Huntable-Windows CTI-BERT classifier are unaffected. Docs updated (`docs/features/os-detection.md` rewritten to the entity-driven reality; `agent-config-schema.md`, `concepts/agents.md`, `reference/api.md`). Tests: full `tests/config/` suite green (659).

### Fixed
- **PDF export resolves theme color tokens before canvas capture** (2026-06-22): `exportArticleToPDF` in `src/web/templates/article_detail.html` now reads the computed `--*-print`/keyword-highlight CSS custom properties into concrete color values up front (a `resolveThemeColor` helper feeding a `pdfColors` map) instead of handing raw `var(--…)` strings to html2canvas/html2pdf, which cannot resolve CSS custom properties at capture time. Previously the print container, title, content wrapper, the five keyword-highlight categories, and the html2canvas `backgroundColor` could capture with unresolved or blank colors. Regression: `tests/playwright/article_detail.spec.ts` `[ARTICLE-006]` stubs `html2pdf` and asserts no `var(` survives into the captured container/content/title styles or the canvas background, and that the export raises no error notification.
- **Test runner no longer accumulates Allure results without bound** (2026-06-22): the global pytest `addopts` forced `--alluredir=allure-results` on *every* invocation (including ad-hoc `pytest <path>` runs) and nothing pruned the directory, so it grew to millions of files and filled the disk. Removed `--alluredir` from the global `addopts` (`run_tests.py` re-adds it itself only when a report is wanted), and `tests_runner/runner.py` now clears `allure-results/` at the start of each run via a new `_clear_directory_contents` helper. Tests: `tests/test_run_tests_artifact_cleanup.py` (helper clears files/subdirs but keeps the directory; absent dir is a no-op; the global `addopts` carries no `--alluredir`).
- **Enterprise-auth adversarial-review fixes** (2026-06-25): added value-level secret scrubbing to audit redaction so a credential embedded in a free-text string (e.g. a GitHub token in a git error message) cannot reach audit metadata; role-gated the destructive article routes (`DELETE /api/articles/{id}`, bulk-action, mark-reviewed) that were reachable by any authenticated zero-role user; tightened the route-manifest validator to flag any unsafe route silently downgraded to authenticated-only (explicit allowlist); and stopped the CSRF fetch shim from attaching the token to protocol-relative cross-origin URLs. Also fixed `setup.sh` silently discarding the generated `SECRET_KEY` (a dead `sed` placeholder) by writing it through `startup_set_env_key`.
- **Full-repo documentation audit (accuracy, relevancy, gap-fill)** (2026-07-05): reviewed ~185 Markdown files (root, `docs/**`, historical dirs) against runtime code, tests, and git history; ~55 factual corrections across ~50 files (missing `NetworkIndicatorExtract` in agent enumerations and UI-contract tables, wrong API paths/HTTP codes, stale eval-set counts, dead code references, pre-v7.5.0 Platform-Detection-as-gate framing). Two cross-cutting findings: `docs/index.md`/`reference/versioning.md` still called image-OCR-ingest "disabled by default" though `docker-compose.yml` has defaulted `OCR_INGEST_ENABLED=true` since commit `464b3452` (2026-06-17, in this release's range) with no CHANGELOG record of the flip until now; and `README.md`/`AGENTS.md` described the removed autonomous source-healing service (`source_healing_service.py`, moved out of the codebase in v6.2.1) as a live automatic feature — corrected to describe the actual mechanism, the operator-invoked `source-healing` Claude Code skill. Deleted `docs/guides/evaluate-models.md` (documented `scripts/eval_extract_agent.py`/`compare_evaluations.py`, both removed in v6.2.0). Gap-filled two undocumented route modules (`scheduled_jobs.py`, `observable_training.py`) into existing pages. Deferred two follow-ups as Todoist tasks (Huntable CTI Studio project, Intake section): reciprocal `NetworkIndicatorExtract` boundary rules across the six pre-existing extractor contracts plus its missing drop-in prompt (extraction-semantics content requiring operator sign-off, not auto-generated); and a dedicated `docs/features/image-ocr-ingest.md` page. `mkdocs build --strict`: 0 warnings/errors throughout.

## [7.4.1 "Europa"] - 2026-06-14
### Changed
- **CmdlineExtract spec: detection-logic de-laning + Complete-Artifact Rule (heavy instance) + objective-only relevance gate (score-comparability era boundary)** (2026-06-13): Cmdline iteration of the fleet-wide Complete-Artifact Rule ratified in `docs/development/extractor-lane-partial-enablement-2026-06-12.md` — the **heavy instance** (§7.2), the last extractor to de-lane. `docs/contracts/cmdline-extract.md` no longer defers commands that appear only inside detection logic to HuntQueriesExtract: detection/hunting/mitigation **prose** was already eligible (2026-06-04 relaxation), and now the rule/query **bodies** are too. **Complete-Artifact Rule (heavy instance):** a command matched inside a rule/query is extractable only when the matched value is the command ITSELF — a verbatim single-line invocation that independently satisfies POSITIVE SCOPE — never a predicate. Unlike the structured siblings (whose positive scope auto-gates fragments), a command line is a free-form string that detection logic stores mostly as fragments, so the guard is explicit and two-signalled: (1) **matching operator** discloses fidelity — exact match (Sigma default/`|equals`, KQL `==`/`=~`, SPL exact) carries a full command and is EXTRACTABLE; fragment operators (`|contains`/`|all`/`|startswith`/`|endswith`/`|re`, KQL `contains`/`has`/`matches regex`, SPL wildcard/`like`/`rex`) carry a substring and SKIP; (2) **value-shape fallback** for ambiguous-operator vendor queries — the matched string must independently pass positive scope. Anchored on the existing ARGV "representation, not a command line" exclusion + the POSITIVE/1 "not assembled from fields or representations" rule, so it extends current logic rather than adding a new philosophy. The ARCHITECTURE CONTEXT HuntQueriesExtract boundary bullet is reworded from region-wide deference to type-discipline (don't emit the finished rule/query; a command inside it IS extractable under the rule). YARA stays excluded entirely (file-content signatures, not telemetry). Also drops the subjective "no detection-engineering value → SKIP" half of the DETECTION RELEVANCE GATE, keeping only the objective Sysmon-1/4688/EDR-observability requirement — parity with the ProcTree ruling, removing a model-variance source. Mirror edits applied to `docs/contracts/cmdline-extract-dropin.md`; doc footer date corrected to 2026-06-13. Eval ground truth re-audited under the new spec in the same session. Total extraction volume may rise under full enablement via multi-facet extraction — by design, not double-counting (see design doc §4).
- **ServicesExtract spec: detection-logic de-laning + Complete-Artifact Rule (score-comparability era boundary)** (2026-06-13): Services iteration of the fleet-wide Complete-Artifact Rule ratified in `docs/development/extractor-lane-partial-enablement-2026-06-12.md` (§7.3 light instance). `docs/contracts/services-extract.md` no longer defers service artifacts that appear only inside detection logic to HuntQueriesExtract, and detection/hunting/mitigation prose describing a service is now a valid source — under partial agent enablement those deference rules were coverage holes. **Complete-Artifact Rule (light instance):** a service artifact matched inside a rule/query is extractable only when the matched value is a full `service_name` or `image_path` satisfying Gate 2; a `ServiceName|contains: 'Mal'` fragment or partial path already fails Gate 2 automatically. The ARCHITECTURE CONTEXT HuntQueriesExtract boundary rule is reworded from region-wide deference to a type-discipline rule (don't emit the finished-detection-logic artifact; service attributes inside such queries ARE extractable under the Complete-Artifact Rule). The "ONLY inside a YARA rule" exclusion is retained. Mirror edits applied to `docs/contracts/services-extract-dropin.md`. Eval ground truth re-audited under the new spec in the same session. Total extraction volume may rise under full enablement via multi-facet extraction — by design, not double-counting (see design doc §4).
- **ScheduledTasksExtract spec: detection-logic de-laning + Complete-Artifact Rule (score-comparability era boundary)** (2026-06-12): ScheduledTasks iteration of the fleet-wide Complete-Artifact Rule ratified in `docs/development/extractor-lane-partial-enablement-2026-06-12.md` (§7.3 light instance). `docs/contracts/scheduled-tasks-extract.md` no longer defers scheduled-task identities that appear only inside detection logic to HuntQueriesExtract — detection queries and rules (Sigma, KQL, SPL, EQL, XQL, vendor hunting queries) are now a valid source when a `TaskName` condition carries a **complete** task name or task path as the matched value. **Complete-Artifact Rule (light instance):** a `TaskName|contains:`, `|endswith:`, `|re:`, or other partial-match condition carries a fragment and is still excluded — the existing positive-scope requirement for a full task name/path already gates this automatically. The boundary-rule bullet at `ARCHITECTURE CONTEXT` is reworded from a region-wide deference ("do not extract detection queries — HuntQueriesExtract owns those") to a type-discipline rule ("do not extract the finished-detection-logic artifact; task identities inside such queries ARE extractable under the Complete-Artifact Rule"). Mirror edits applied to `docs/contracts/scheduled-tasks-extract-dropin.md`. Also fixes a doc-internal contradiction in the confidence scoring range: `0.3-0.59` for "bare task_name only" corrected to `0.5-0.59`, consistent with the `< 0.5` fail-closed floor and the dropin. Eval ground truth re-audited under the new spec in the same session. Total extraction volume may rise under full enablement via multi-facet extraction — by design, not double-counting (see design doc §4).
- **RegistryExtract spec: detection-logic de-laning + Complete-Artifact Rule (score-comparability era boundary)** (2026-06-12): Registry pilot of the fleet-wide Complete-Artifact Rule ratified in `docs/development/extractor-lane-partial-enablement-2026-06-12.md` (§7.3 light instance). `docs/contracts/registry-extract.md` no longer defers registry artifacts that appear only inside detection logic (Sigma/KQL/SPL/EQL/XQL/vendor hunting queries) to HuntQueriesExtract, and detection/hunting/mitigation prose grounded in the article is now a valid source — under partial agent enablement those deference rules were coverage holes (an enabled RegistryExtract extracted nothing from rule bodies even when no HuntQueriesExtract was running). New **COMPLETE-ARTIFACT RULE**: a registry artifact matched inside a rule/query is extractable only when the matched value is a full hive-rooted path; hive-less fragments under `|contains`/`|endswith`/`|startswith`/`|re` still SKIP (the existing hive-root positive scope is the gate), and value name/data only when literally present in the matched value. The "ONLY inside a YARA rule" exclusion is retained (operator ruling 2026-06-12: YARA is file-content signature territory, not telemetry detection logic). Mirror edits applied to `docs/contracts/registry-extract-dropin.md`. Eval ground truth re-audited under the new spec in the same session. Total extraction volume may rise under full enablement via multi-facet extraction (one source span can yield Cmdline + Registry + HuntQueries records) — by design, not double-counting. Two adjudication clarifications ratified during the same audit: (a) FIDELITY transport-noise exception — HTML-to-text whitespace artifacts and command-syntax escaping (PowerShell doubled backslashes, `HKCU:` drive-colon) are stripped, not preserved; (b) COUNT SEMANTICS — prose-enumerated value-name lists collapse into the parent key item unless a value name has its own dedicated statement.
...
### Fixed
- **Article detail keyword highlights survive saved annotation restore** (2026-06-12): saved huntability annotations now wrap the existing article DOM fragment instead of replacing it with plain text, preserving server-rendered keyword highlights inside annotated ranges. Automatic keyword overlap display now lets Perfect discriminator highlights supersede all other matches, while non-Perfect overlaps prefer the longer, more-specific match so compound LOLBAS executables such as `cmd.exe` stay intact instead of fragmenting into `cmd` + `.exe`.
