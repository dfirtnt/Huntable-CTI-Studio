...
### Changed
- **Action buttons now use the shared semantic color system** (2026-07-17): replaced page-local and raw Tailwind button colors across evaluation, analytics, article, diagnostics, jobs, settings, Sigma, sources, and workflow templates with the global analyze/manage/danger/secondary button classes. Added reusable secondary and small-size variants and removed duplicate page-local button CSS.
- **Additional raw-Tailwind buttons migrated to semantic classes** (2026-07-18): migrated remaining raw `bg-emerald-500`, `bg-red-500/800`, `bg-green-600` primary buttons in `analytics.html`, `article_detail.html` (Huntable/Not-Huntable classification, Remove annotation), `articles.html` (bulk Delete), and `observable_training.html` (annotation Save) to `btn-analyze`, `btn-manage`, `btn-danger`, and `btn-execute`.
