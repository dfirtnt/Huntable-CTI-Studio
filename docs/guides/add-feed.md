...
- The Celery Beat periodic task `check-all-sources-every-30min` (fixed in `src/worker/celery_app.py`, not `docker-compose.yml`) sweeps **all** active sources every 30 minutes; it does not currently gate on each source's `check_frequency`. `check_frequency` (seconds, default 14400 / 4 hours) is still stored per source, exposed via the API/UI, and used by source scheduling helpers.
