...
### In scope
- **Route manifest** (`src/web/security/route_manifest.py`): enumerate every route registered by `register_routes()` in [`src/web/routes/__init__.py`](../../../src/web/routes/__init__.py) with: path, methods, route module, endpoint name, public/authenticated/role classification, audit requirement (`none` | `best_effort` | `mandatory`), CSRF requirement for unsafe browser routes.
...
- **Detailed health → `operator`/`admin`.** Only minimal liveness/readiness stays public; the rich endpoints in [`src/web/routes/health.py`](../../../src/web/routes/health.py) (DB counts, dedup/SimHash internals, Redis/Celery state, model names, OCR/LangFuse status, ingestion analytics) move behind a role.
...
- **Correction to carry:** the parent spec's "Existing tests intentionally prevent explicit auth dependencies on some source endpoints" claim **could not be located** in the test suite during verification. Before writing the migration task that "updates tests asserting no explicit route auth guards," confirm such a test actually exists; if it does not, drop that migration task rather than chase a phantom.
