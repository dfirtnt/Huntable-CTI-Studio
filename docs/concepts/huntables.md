...
Huntables are repeatable, telemetry-rich observables extracted from CTI content—command lines, process chains, registry paths, Sigma-style queries, and Windows Event IDs that can drive detections. They are tracked at two levels:
...
- Structured process and command-line patterns (cmd.exe, powershell, LOLBAS usage)
- Registry keys and persistence paths
- Event IDs and Sigma-like query fragments
