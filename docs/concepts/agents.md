...
Two different "status" fields show up in the UI and API — they are not the same thing.
...
A `pending` execution older than the stale cutoff is re-triggerable (see `src/services/workflow_trigger_service.py`).

### Agent evaluation badges (Agent Evals page)

On `/agent-evals`, the comparison table shows per-row badges comparing a predicted value against ground truth. Styles are defined in `src/web/templates/agent_evals.html`:

| Badge | Meaning |
|-------|---------|
| `exact` | Prediction matches ground truth exactly |
| `mismatch` | Prediction differs from ground truth |
| `pending` | Evaluation not yet produced a verdict |
| `warning` | Prediction is close but outside the strict-match threshold |

The live dot (`eval-status-dot--live`) on the header indicates an evaluation run is currently streaming results.
