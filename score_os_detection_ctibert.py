#!/usr/bin/env python3
"""Score OS detection performance using CTI-BERT embeddings with similarity-based classification."""
...
RESULTS_FILE = "ctibert_os_detection_scores.json"
...
class CTIBERTOSDetector:
    """OS detection using CTI-BERT embeddings."""
    
    def __init__(self, model_name: str = "ibm-research/CTI-BERT", use_gpu: bool = True):
        """Initialize CTI-BERT model."""
    ...
    def _load_model(self):
        ...
        try:
            print(f"Loading CTI-BERT model: {self.model_name}")
            ...
            print(f"Successfully loaded CTI-BERT model on device {self.device}")
            
            # Pre-compute OS indicator embeddings
            self._precompute_os_embeddings()
            
        except Exception as e:
            print(f"Failed to load CTI-BERT model: {e}")
    ...
    def detect_os(self, content: str) -> Dict[str, Any]:
        """
        Detect OS from content using CTI-BERT embeddings.
...
def evaluate_detection(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    metrics = {
        'json_valid': True,  # Always valid for CTI-BERT (not JSON-based)
...
def detect_os_from_article(article_id: int, title: str, source: str, url: str, content: str) -> Optional[Dict[str, Any]]:
    """Detect OS from a single article using CTI-BERT."""
    try:
        detector = CTIBERTOSDetector()
...
def main():
    ...
    print(f"Detecting OS from {len(articles)} articles with CTI-BERT...\n")
