#!/usr/bin/env python3
"""Score extract agent performance using GPT-4o with JSON output evaluation."""
...
RESULTS_FILE = "gpt4o_extract_scores.json"
...
async def extract_article(article_id: int, title: str, source: str, url: str, content: str, max_retries: int = 3) -> Optional[Dict[str, Any]]:
    """Extract observables from a single article using GPT-4o."""
            ...
            try:
                response = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": prompt_config_dict.get("role", "You are a detection engineer LLM.")
                            },
                            {
                                "role": "user",
                                "content": user_prompt
                ...
                if response.status_code == 429:
                    try:
                        ...
                        retry_after = max(retry_after, 20.0)
                    except:
                        retry_after = 30.0
                    
                    if attempt < max_retries - 1:
                        print(f"Article {article_id}: Rate limited, waiting {retry_after:.1f}s...")
...
async def main():
    ...
    print(f"Extracting from {len(articles)} articles with GPT-4o...\n")
