...
class ContentProcessor:
    ...
    def __init__(
        self, similarity_threshold: float = 0.85, max_age_days: int = 90, enable_content_enhancement: bool = True
    ):
    ...
    async def _process_single_article(
        self, article: ArticleCreate, source_config: dict[str, Any] | None = None
    ) -> ArticleCreate | None:
        """
        ...
        try:
            ...
            logger.info(
                f"ContentProcessor: After cleaning - {len(normalized_content)} chars, {non_printable_after} non-printable"
            )
    ...
    def _check_duplicates(self, article: ArticleCreate) -> str | None:
        """
        ...
        if hasattr(article, "source_id") and article.source_id:
            ...
            if fingerprint in self.source_fingerprints[article.source_id]:
                logger.debug(
                    f"Content similarity duplicate detected within source {article.source_id}: {fingerprint[:10]}..."
                )
    ...
    def _passes_quality_filter(self, article: ArticleCreate) -> bool:
        ...
        if content_length < min_length:
            logger.warning(
                f"Article '{article.title[:50]}...' content too short: {content_length} chars (min: {min_length})"
            )
            return False
...
class BatchProcessor:
    ...
    def __init__(self, processor: ContentProcessor, batch_size: int = 100, max_concurrent: int = 3):
