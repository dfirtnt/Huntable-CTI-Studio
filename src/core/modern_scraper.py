...
from src.utils.http import HTTPClient, normalize_url
...
class URLDiscovery:
    ...
    async def discover_urls(self, source: Source) -> list[str]:
        ...
        discovery_config = (
            source.config.get("discovery")
            if isinstance(source.config, dict)
            else getattr(source.config, "discovery", None)
        )
        ...
        for sitemap_url in sitemap_urls:
            try:
                ...
                if discovered_count == 0:
                    logger.debug(
                        f"No URLs found with find_all('loc'), trying namespace-aware parsing for {sitemap_url}"
                    )
    ...
    def _filter_by_scope(self, urls: list[str], source: Source) -> list[str]:
        ...
        logger.info(
            f"Filtering {len(urls)} URLs for {source.name}: allowed_domains={allowed_domains}, patterns={post_url_patterns}"
        )
                    ...
                    for pattern in post_url_patterns:
                        try:
                            ...
                            if not matched and len([u for u in urls if u == url]) == 1:  # First occurrence
                                logger.debug(
                                    f"❌ URL {url} did NOT match pattern {pattern_normalized} (compiled: {compiled_pattern.pattern})"
                                )
...
class StructuredDataExtractor:
    ...
    def extract_structured_data(html: str, base_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            data = {"json-ld": [], "opengraph": [], "microdata": [], "microformat": []}
...
class ModernScraper:
    ...
    async def _extract_article(self, url: str, source: Source) -> ArticleCreate | None:
        """
        ...
        try:
            ...
            structured_data = self.structured_extractor.extract_structured_data(response.text, url)
            ...
            if not article_data.get("title") or not article_data.get("content"):
                logger.warning(
                    f"Missing required fields for {url}: title={bool(article_data.get('title'))}, content={bool(article_data.get('content'))}"
                )
    ...
    def _extract_with_selectors(self, soup: BeautifulSoup, source: Source, url: str) -> dict[str, Any]:
            ...
            for fallback_selector in [
                ...
                if content_elem:
                    ...
                    if text_length > 500:  # Substantial content
                        content_html = str(content_elem)
                        logger.debug(
                            f"Found content with fallback selector '{fallback_selector}' for {url} ({text_length} chars)"
                        )
                        break
        ...
        if content_html:
            ...
            if text_length > 100:
                ...
                logger.warning(
                    f"Content too short after cleaning for {url} ({text_length} chars), may be mostly navigation/UI"
                )
                        ...
                        if parent:
                            ...
                            if len(parent_text) > 1000:
                                ...
                                if len(cleaned_parent.strip()) > 100:
                                    ...
                                    logger.debug(
                                        f"Found content near h1 for {url} ({len(cleaned_parent.strip())} chars)"
                                    )
                                    break
                            parent = parent.parent
        else:
        ...
        if not data.get("summary"):
            summary = og_data.get("description") or meta_data.get("description") or meta_data.get("og:description")
            if summary:
