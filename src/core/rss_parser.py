...
class RSSParser:
    ...
    async def _extract_content(
        ...
        if "thehackernews.com" in url.lower():
            ...
            if content and len(ContentCleaner.html_to_text(content).strip()) > 100:
                logger.info(
                    f"Using RSS content for The Hacker News: {len(ContentCleaner.html_to_text(content).strip())} chars"
                )
                return ContentCleaner.clean_html(content)
            return None
    ...
    def _is_quality_content(
        ...
        words = text.split()
        min_words = 50  # Default minimum word count
        if source_config and "min_word_count" in source_config:
            min_words = source_config["min_word_count"]
        elif source_config and source_config.get("rss_only", False):
            # Be much more lenient for RSS-only sources which often have summaries
            min_words = 3  # Very low threshold for RSS content
    ...
    def _get_feed_content(self, entry: Any) -> Optional[str]:
        ...
        logger.info(
            f"  has content={hasattr(entry, 'content')}, has desc={hasattr(entry, 'description')}, has summary={hasattr(entry, 'summary')}"
        )
        ...
        if hasattr(entry, "description"):
            ...
            logger.info(
                f"  description: type={type(desc_val).__name__}, length={desc_len}, first100={str(desc_val)[:100] if desc_val else 'EMPTY'}"
            )
