...
class SourceConfigLoader:
    ...
    def _parse_source(self, source_data: dict[str, Any]) -> SourceCreate:
        ...
        source_config = SourceConfig(
            check_frequency=check_frequency, lookback_days=lookback_days, config=config_dict or {}
        )
...
class SourceManager:
    ...
    def __init__(self, database_manager: DatabaseManager | None = None, http_client: HTTPClient | None = None):
        ...
        for config in source_configs:
            if config.rss_url:
                try:
                    ...
                    validation_result = await self.feed_validator.validate_feed(config.rss_url, self.http_client)

                    if validation_result["valid"]:
                        logger.info(
                            f"RSS feed valid for {config.identifier}: {validation_result['entry_count']} entries"
                        )
    ...
    def export_sources_config(
        self, output_path: str, source_ids: list[int] | None = None, include_inactive: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            config_data = {"version": "1.0", "sources": []}
    ...
    def validate_source_config(self, config_path: str) -> dict[str, Any]:
        ...
        """
        result = {"valid": False, "sources_count": 0, "errors": [], "warnings": []}
            ...
            for i, config in enumerate(source_configs):
                ...
                if config.check_frequency < 60:
                    result["warnings"].append(
                        f"Source '{config.identifier}': Check frequency {config.check_frequency}s is very aggressive"
                    )
    ...
    def get_source_config(self, identifier: str) -> SourceConfig | None:
    ...
    def get_statistics(self) -> dict:
        ...
        return {"total_sources": total_sources, "active_sources": active_sources, "inactive_sources": inactive_sources}
