    ...
    async def record_source_check(
        ...
        try:
            async with self.get_session() as session:
                ...
                await session.commit()
                logger.debug(
                    f"Recorded source check for source {source_id}: success={success}, method={method}, articles={articles_found}"
                )
        ...
        try:
            async with self.get_session() as session:
                ...
                (
                    ...
                    new_article,
                    similar_articles,
                ) = await dedup_service.create_article_with_deduplication(article)
            ...
            async with self.get_session() as session:
                ...
                most_annotated_result = await session.execute(
                    select(
                        ...
                        func.count(ArticleAnnotationTable.id).label("count"),
                    )
        ...
        try:
            async with self.get_session() as session:
                ...
                query = (
                    ...
                    .select_from(
                        ArticleAnnotationTable.__table__.join(
                            ArticleTable.__table__,
                            ArticleAnnotationTable.article_id == ArticleTable.id,
                        ).join(
                    ...
                    .order_by(text("embedding <=> :query_vector"))
                    .limit(limit)
                )
                ...
                result = await session.execute(query.params(query_vector=embedding_str))
