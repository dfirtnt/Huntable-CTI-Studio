...
class ArticleTable(Base):
    ...
    chunk_analysis_results = relationship(
        "ChunkAnalysisResultTable", back_populates="article", cascade="all, delete-orphan"
    )
...
class ChunkAnalysisResultTable(Base):
    ...
    def __repr__(self):
        return (
            f"<ChunkAnalysisResult(id={self.id}, article_id={self.article_id}, model_version='{self.model_version}')>"
        )
...
class MCPWriteConfirmationTable(Base):
    """Pending human confirmation request for high-risk MCP write actions."""

    __tablename__ = "mcp_write_confirmations"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    operation = Column(String(150), nullable=False, index=True)
    risk_tier = Column(String(50), nullable=False, default="confirmation_required", index=True)
    status = Column(String(50), nullable=False, default="pending", index=True)
    target_type = Column(String(100), nullable=False, index=True)
    target_id = Column(String(255), nullable=True, index=True)
    requested_by = Column(String(255), nullable=False, default="service:mcp")
    request_metadata = Column(JSONB, nullable=False, default=dict)
    confirmation_instructions = Column(Text, nullable=False)
    created_at = Column(DateTime, nullable=False, default=func.now(), index=True)
    updated_at = Column(DateTime, nullable=False, default=func.now(), onupdate=func.now())
    ...
    def __repr__(self):
        return (
            f"<MCPWriteConfirmation(id={self.id}, operation='{self.operation}', "
            f"status='{self.status}', target='{self.target_type}:{self.target_id}')>"
        )
...
class AgenticWorkflowConfigTable(Base):
    ...
    def __repr__(self):
        return (
            f"<AgenticWorkflowConfig(id={self.id}, min_hunt_score={self.min_hunt_score}, is_active={self.is_active})>"
        )
