...
class CTIBERTNERExtractor:
    ...
    def _extract_entities_with_patterns(self, text: str) -> dict[str, list[str]]:
        ...
        filtered_domains = [
            d
            for d in domains
            if not any(d.endswith(ext) for ext in [".com", ".org", ".net", ".edu", ".gov"]) or len(d.split(".")) > 2
        ]
        ...
        try:
            ...
            try:
                ...
                similarity = torch.nn.functional.cosine_similarity(
                    content_embedding.unsqueeze(0), cti_context_embedding.unsqueeze(0)
                ).item()
            ...
            return CTIBERTExtractionResult(
                ...
                raw_count=total_count,
                validated_count=validated_count,
                metadata={"error": str(e), "fallback_used": True, "uses_ctibert_embeddings": False},
            )
