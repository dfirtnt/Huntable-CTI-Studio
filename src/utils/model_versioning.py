...
class MLModelVersionManager:
    ...
    async def save_model_version(self, metrics: Dict[str, Any], training_config: Dict[str, Any], 
                                feedback_count: int = 0, model_file_path: str = None) -> int:
        """
    ...
    async def get_latest_version(self) -> Optional[MLModelVersionTable]:
        """Get the latest model version."""
        try:
            async with self.db_manager.get_session() as session:
                result = await session.execute(
                    select(MLModelVersionTable)
                    .order_by(desc(MLModelVersionTable.version_number))
                    .limit(1)
                )
    ...
    async def get_version_by_id(self, version_id: int) -> Optional[MLModelVersionTable]:
    ...
    async def compare_versions(self, old_version_id: int, new_version_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            precision_huntable_improvement = new_version.precision_huntable - old_version.precision_huntable if old_version.precision_huntable else 0
            recall_huntable_improvement = new_version.recall_huntable - old_version.recall_huntable if old_version.recall_huntable else 0
            ...
            comparison = {
                ...
                'improvements': {
                    ...
                    'accuracy_change_percent': (accuracy_improvement / old_version.accuracy * 100) if old_version.accuracy else 0,
            ...
            if accuracy_improvement > 0.01:
                comparison['summary']['key_improvements'].append(f"Accuracy improved by {accuracy_improvement:.3f} ({comparison['improvements']['accuracy_change_percent']:.1f}%)")
            elif accuracy_improvement < -0.01:
                comparison['summary']['areas_of_concern'].append(f"Accuracy decreased by {abs(accuracy_improvement):.3f}")
            
            if precision_huntable_improvement > 0.01:
                comparison['summary']['key_improvements'].append(f"Precision for huntable content improved by {precision_huntable_improvement:.3f}")
            elif precision_huntable_improvement < -0.01:
                comparison['summary']['areas_of_concern'].append(f"Precision for huntable content decreased by {abs(precision_huntable_improvement):.3f}")
            
            if new_version.feedback_samples_count > old_version.feedback_samples_count:
                comparison['summary']['key_improvements'].append(f"Added {new_version.feedback_samples_count - old_version.feedback_samples_count} feedback samples")
    ...
    async def run_comparison_test(self, old_model_path: str, new_model_path: str, 
                                 test_articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
