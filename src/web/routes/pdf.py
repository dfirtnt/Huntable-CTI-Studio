    ...
    try:
        ...
        try:
            ...
            async with async_db_manager.get_session() as session:
                manual_source = await session.execute(select(SourceTable).where(SourceTable.identifier == "manual"))
                ...
                async with async_db_manager.get_session() as session:
                    try:
                        ...
                        if manual_source_obj:
                            ...
                            raise HTTPException(status_code=500, detail="Failed to create manual source")

                    except IntegrityError as exc:
                        logger.warning(f"IntegrityError creating manual source (likely race condition): {exc}")
