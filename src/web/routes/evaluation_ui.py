...
async def evaluations_page(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "error.html",
            {"request": request, "error": f"Error loading evaluations page: {str(e)}"},
            status_code=500
        )


@router.get("/evaluations/compare", response_class=HTMLResponse)
async def compare_evaluations_page(
    request: Request,
...
async def agent_evaluation_page(
    request: Request,
    agent_name: str,
    ...
    try:
        try:
            ...
            if evaluation_id:
                ...
                selected_evaluation = db_session.query(AgentEvaluationTable).filter(
                    AgentEvaluationTable.id == evaluation_id
                ).first()
                if selected_evaluation:
                    selected_evaluation = {
                        ...
                        'created_at': selected_evaluation.created_at.isoformat() if selected_evaluation.created_at else None,
