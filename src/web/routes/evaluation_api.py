    ...
    try:
        ...
        try:
            ...
            setting = (
                ...
                .filter(AppSettingsTable.key == key)
                .first()
            )
...
def get_langfuse_client() -> Langfuse:
    ...
    host = _get_langfuse_setting(
        "LANGFUSE_HOST", "LANGFUSE_HOST", "https://cloud.langfuse.com"
    )

    if not public_key or not secret_key:
        raise ValueError(
            "LANGFUSE_PUBLIC_KEY and LANGFUSE_SECRET_KEY must be set in Settings or environment variables"
        )
        ...
        if hasattr(dataset, "items") and dataset.items:
            try:
                for item in dataset.items:
                    expected_output = (
                        item.expected_output if hasattr(item, "expected_output") else {}
                    )
                    ...
                    if (
                        ...
                        if article_url and isinstance(article_url, str):
                            ...
                            if match:
                                ...
                                logger.info(
                                    f"Extracted article_id {article_id} from article_url: {article_url}"
                                )
                    ...
                    if (
                        ...
                        if (
                            article_text and len(article_text) > 100
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if not article_id:
                                        for snippet_size in [500, 300, 200, 100]:
                                            content_snippet = article_text[
                                                :snippet_size
                                            ].strip()
                                            if content_snippet:
                                                # Escape special characters for LIKE query
                                                content_snippet_escaped = (
                                ...
                                logger.error(
                                    f"Error during article lookup: {e}", exc_info=True
                                )
                    ...
                    if isinstance(item.input, dict):
                        input_info = {
                            ...
                            "article_title_preview": item.input.get(
                                "article_title", ""
                            )[:50]
                    ...
                    logger.info(
                        f"Dataset item {item.id}: input={input_info}, article_id={article_id}"
                    )
                    ...
                    if (
                        ...
                        logger.warning(
                            f"Non-numeric article_id found: {article_id} (type: {type(article_id)})"
                        )
                        article_id = None
                    ...
                    items.append(
                        {
                            ...
                            "lookup_failed": article_id
                            is None,  # Flag for UI to show warning
                        }
                    )
        ...
        try:
            ...
            for article_id in eval_request.article_ids:
                ...
                for config_id in eval_request.config_ids:
                    ...
                    logger.info(
                        f"Eval execution {execution.id}: Using config {config.id} (v{config.version})"
                    )
...
def resolve_article_by_url(url: str) -> Optional[int]:
    ...
    try:
        ...
        try:
            ...
            article = (
                db_session.query(ArticleTable)
                .filter(ArticleTable.canonical_url == url)
                .first()
            )
            ...
            article = (
                db_session.query(ArticleTable)
                .filter(ArticleTable.canonical_url.like(f"{normalized_url}%"))
                .first()
            )
...
async def get_subagent_eval_articles(
    ...
    try:
        ...
        if not config_path.exists():
            raise HTTPException(
                status_code=404, detail="eval_articles.yaml config file not found"
            )
    ...
    try:
        ...
        try:
            ...
            if not active_config:
                raise HTTPException(
                    status_code=404, detail="No active workflow config found"
                )
            ...
            for url in eval_request.article_urls:
                ...
                if not article_id:
                    ...
                    article_mappings.append(
                        {"url": url, "article_id": None, "found": False}
                    )
                else:
                    article_mappings.append(
...
async def get_subagent_eval_results(
    ...
    eval_run_id: Optional[int] = Query(
        None, description="Optional: filter by eval record ID"
    ),
):
    ...
    try:
        ...
        try:
            eval_record = (
                ...
                .filter(SubagentEvaluationTable.id == eval_record_id)
                .first()
            )

            if not eval_record:
                raise HTTPException(
                    status_code=404, detail="Evaluation record not found"
                )
            ...
            all_records = (
                ...
                .filter(
                    ...
                    SubagentEvaluationTable.workflow_config_version
                    == eval_record.workflow_config_version,
                )
                .all()
            )
            ...
            if completed_records:
                ...
                mean_score = sum(r.score for r in completed_records) / len(
                    completed_records
                )
            else:
                accuracy = None
...
async def clear_pending_eval_records(
    request: Request, subagent: str = Query(..., description="Subagent name")
):
            ...
            for eval_record in pending_evals:
                ...
                execution = (
                    ...
                    .filter(
                        ...
                        == eval_record.workflow_execution_id
                    )
                    .first()
                )
...
async def get_subagent_eval_aggregate(
    request: Request,
    subagent: str = Query(..., description="Subagent name"),
    config_version: Optional[int] = Query(
        None, description="Optional: filter by config version"
    ),
):
