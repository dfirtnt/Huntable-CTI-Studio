...
async def api_create_backup(background_tasks: BackgroundTasks):
    """Create a new database backup using direct PostgreSQL commands."""
    try:
        import gzip
        import tempfile
        ...
        # Use SQLAlchemy to create backup
        from sqlalchemy import text
        from src.database.async_manager import AsyncDatabaseManager
        
        db_manager = AsyncDatabaseManager()
        
        logger.info("Creating backup using SQLAlchemy...")
        ...
        # Get database connection and dump all tables
        async with db_manager.get_session() as session:
            ...
            # Add schema creation for each table
            for table_name in tables:
                ...
                if columns:
                    backup_content.append(f"-- Schema for table {table_name}")
                    
                    # Check for sequences first
                    sequence_result = await session.execute(text(f"""
                    ...
                    """))
                    sequences = sequence_result.fetchall()
                    
                    # Create sequences if they exist
                    for seq_col, seq_default in sequences:
                        if seq_default:
                            # Extract sequence name from nextval('sequence_name'::regclass)
                            seq_name = seq_default.split("'")[1] if "'" in seq_default else f"{table_name}_{seq_col}_seq"
                            backup_content.append(f"CREATE SEQUENCE IF NOT EXISTS \"{seq_name}\";")
                    
                    backup_content.append(f"CREATE TABLE IF NOT EXISTS \"{table_name}\" (")
        ...
        logger.info(f"Backup completed: {sql_path.stat().st_size:,} bytes")
        ...
        try:
            async with db_manager.get_session() as session:
                ...
                stats['article_count'] = result.scalar()
                
                # Get source count
                result = await session.execute(text('SELECT COUNT(*) FROM sources'))
                stats['source_count'] = result.scalar()
        ...
        logger.info(f"   Articles: {stats.get('article_count', 'unknown')}")
        logger.info(f"   Sources: {stats.get('source_count', 'unknown')}")
        
        return {
            "success": True,
            "message": "Backup created successfully",
            "backup_file": backup_filename,
            "backup_size": f"{file_size_mb:.2f} MB",
            "article_count": str(stats.get('article_count', 'unknown')),
            "source_count": str(stats.get('source_count', 'unknown')),
            "timestamp": datetime.now().isoformat()
        }
...
async def api_restore_backup(request: Request):
    """Restore database from a backup file using direct SQLAlchemy."""
    try:
        ...
        try:
            ...
            # Parse output to extract restore information
            output_lines = result.stdout.strip().split('\n')
            article_count = None
            source_count = None
            
            for line in output_lines:
                if 'Articles restored:' in line:
                    article_count = line.split('Articles restored:')[1].strip()
                elif 'Sources restored:' in line:
                    source_count = line.split('Sources restored:')[1].strip()
            ...
            return {
                ...
                "article_count": article_count or "unknown",
                "source_count": source_count or "unknown",
