...
@click.option('--metric', type=click.Choice(['weighted_average', 'proportion_weighted', 'confidence_sum_normalized', 'top_percentile', 'user_proposed'], case_sensitive=False), 
              default='weighted_average', help='Metric calculation method (default: weighted_average)')
        ...
        try:
            ...
            if article_id:
                ...
                if not force and article.article_metadata and 'ml_hunt_score' in article.article_metadata:
                    console.print(f"⚠️  Article {article_id} already has an ML hunt score. Use --force to override.", style="yellow")
                    return
                ...
                if not score_result:
                    ...
                    console.print("   ML hunt scores require chunk analysis results. Run chunk analysis first.", style="yellow")
                    return
                ...
                console.print(f"   Huntable Chunks: {score_result['huntable_chunks']} ({score_result['huntable_proportion']*100:.1f}%)")
                ...
                if force:
                    ...
                    articles_to_recalculate = [
                        a for a in articles 
                        if not a.article_metadata or 'ml_hunt_score' not in a.article_metadata
                    ]
                    console.print(f"🔄 Recalculating ML hunt scores for {len(articles_to_recalculate)} articles missing scores...")
                ...
                with Progress(
                    ...
                    for i, article in enumerate(articles_to_recalculate, 1):
                        try:
                            ...
                            if not score_result:
                                skipped_count += 1
                                progress.update(task, advance=1, description=f"Processed {i}/{len(articles_to_recalculate)} (skipped: {skipped_count})")
                                continue
                            
                            if not dry_run:
                                if service.update_article_ml_hunt_score(article.id, metric=metric, model_version=model_version):
                                    success_count += 1
                                else:
                                    error_count += 1
                            else:
                            ...
                            progress.update(task, advance=1, description=f"Processed {i}/{len(articles_to_recalculate)}")
