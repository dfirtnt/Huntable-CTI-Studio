...
def add(article_id: int | None, source_id: int | None, dry_run: bool, force: bool):
    ...
    try:
        if article_id:
            ...
            with db_manager.get_session() as session:
                ...
                session.query(ArticleTable).filter(ArticleTable.id == article_id).update({"archived": True})
                session.commit()
            ...
            with db_manager.get_session() as session:
                ...
                session.query(ArticleTable).filter(ArticleTable.source_id == source_id).update({"archived": True})
                session.commit()
...
@click.option("--force", is_flag=True, help="Skip confirmation prompts")
def remove(article_id: int | None, source_id: int | None, dry_run: bool, force: bool):
        ...
        if article_id:
            ...
            with db_manager.get_session() as session:
                ...
                article = (
                    ...
                    .filter(ArticleTable.id == article_id, ArticleTable.archived == True)
                    .first()
                )
                ...
                session.query(ArticleTable).filter(ArticleTable.id == article_id).update({"archived": False})
                session.commit()
            ...
            with db_manager.get_session() as session:
                ...
                archived_articles = (
                    session.query(ArticleTable)
                    .filter(ArticleTable.source_id == source_id, ArticleTable.archived == True)
                    .all()
                )
                ...
                if dry_run:
                    console.print(
                        f"[yellow]Would unarchive {len(archived_articles)} articles from source {source_id}[/yellow]"
                    )
                    return
                ...
                session.query(ArticleTable).filter(ArticleTable.source_id == source_id).update({"archived": False})
                session.commit()
...
def list(limit: int):
    ...
    try:
        with db_manager.get_session() as session:
            ...
            archived_articles = (
                session.query(ArticleTable)
                .filter(ArticleTable.archived == True)
...
def cleanup(force: bool):
    ...
    try:
        with db_manager.get_session() as session:
            ...
            console.print("[blue]Article Statistics:[/blue]")
            ...
            console.print(
                f"  Archive percentage: {(archived_count / total_articles * 100):.1f}%"
                if total_articles > 0
                else "  Archive percentage: 0%"
            )
