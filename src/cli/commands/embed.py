...
@pass_context
def embed(ctx: CLIContext, batch_size: int, source_id: Optional[int], dry_run: bool):
...
@pass_context
def search_semantic(ctx: CLIContext, limit: int, threshold: float, source_id: Optional[int]):
        ...
        try:
            ...
            if stats.get('source_stats'):
                ...
                for source_stat in stats['source_stats']:
                    click.echo(f"   {source_stat['source_name']}: {source_stat['embedded_articles']}/{source_stat['total_articles']} ({source_stat['coverage_percent']}%)")
