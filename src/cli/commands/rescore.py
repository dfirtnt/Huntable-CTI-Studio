        ...
        try:
            if article_id:
                ...
                if not force and article.article_metadata and "threat_hunting_score" in article.article_metadata:
                    console.print(
                        f"⚠️  Article {article_id} already has a score. Use --force to override.", style="yellow"
                    )
                    return
                ...
                if "threat_hunting_score" in enhanced_metadata:
                    ...
                    if not dry_run:
                        ...
                        article.article_metadata["intelligence_matches"] = enhanced_metadata.get(
                        ...
                        try:
                            ...
                            if "threat_hunting_score" in enhanced_metadata:
                                if not dry_run:
                                    ...
                                    # Update threat hunting score and keyword matches
                                    article.article_metadata["threat_hunting_score"] = enhanced_metadata[
                                    ...
                                    article.article_metadata["lolbas_matches"] = enhanced_metadata.get(
                                        "lolbas_matches", []
                                    )
                                    article.article_metadata["intelligence_matches"] = enhanced_metadata.get(
                                        "intelligence_matches", []
                            ...
                            progress.update(
                                task, advance=1, description=f"Processed {i}/{len(articles_to_rescore)} articles"
                            )
                ...
                if not dry_run:
                    ...
                    console.print(
                        f"Articles with threat hunting scores: {articles_with_score}/{len(updated_articles)} ({articles_with_score / len(updated_articles) * 100:.1f}%)"
                    )
