...
class ChunkAnalysisBackfillService:
    ...
    def get_eligible_articles(self, min_hunt_score: float = 50.0) -> list[dict[str, Any]]:
        ...
        try:
            ...
            eligible_articles = (
                self.db.query(ArticleTable)
                .filter(ArticleTable.id.notin_(self.db.query(articles_with_chunks.c.article_id)))
                .all()
            )
    ...
    def backfill_article(self, article_id: int, min_confidence: float = 0.7) -> dict[str, Any]:
        ...
        try:
            ...
            hunt_score = article.article_metadata.get("threat_hunting_score", 0)
            if hunt_score <= 50:
                return {"success": False, "article_id": article_id, "error": f"Hunt score {hunt_score} is not > 50"}
    ...
    def backfill_all(
        self, min_hunt_score: float = 50.0, min_confidence: float = 0.7, limit: int = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for i, article_info in enumerate(eligible_articles, 1):
                ...
                if result["success"]:
                    ...
                    results["failed"] += 1
                    results["errors"].append({"article_id": article_id, "error": result.get("error", "Unknown error")})
            ...
            return {"success": False, "error": str(e), "duration": f"{time.time() - start_time:.1f}s"}
