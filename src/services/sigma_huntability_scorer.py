...
class SigmaHuntabilityScorer:
    ...
    def score_rule(self, rule_yaml: str, rule_data: dict | None = None) -> HuntabilityScore:
        """
        ...
        if rule_data is None:
            ...
            try:
                ...
                return HuntabilityScore(
                    score=0.0, false_positive_risk="high", coverage_notes="Failed to parse rule", breakdown={}
                )

        if not rule_data or not isinstance(rule_data, dict):
            return HuntabilityScore(
                score=0.0, false_positive_risk="high", coverage_notes="Invalid rule structure", breakdown={}
            )

        detection = rule_data.get("detection", {})
        if not isinstance(detection, dict) or not detection:
            return HuntabilityScore(
                score=0.0, false_positive_risk="high", coverage_notes="No detection section", breakdown={}
            )
        ...
        return HuntabilityScore(
            score=total_score, false_positive_risk=fp_risk, coverage_notes=coverage_notes, breakdown=breakdown
        )
    ...
    def _generate_coverage_notes(
        self, cmdline_score: float, ttp_score: float, parent_child_score: float, telemetry_score: float, fp_risk: str
    ) -> str:
