...
class SigmaPRService:
    ...
    def _check_repo_status(self) -> dict[str, Any]:
        ...
        """
        if not self.repo_path.exists():
            return {"valid": False, "error": f"Repository path does not exist: {self.repo_path}"}

        if not (self.repo_path / ".git").exists():
            return {"valid": False, "error": f"Not a git repository: {self.repo_path}"}
    ...
    def submit_pr(self, rules: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if not repo_status.get("valid"):
            return {"success": False, "error": repo_status.get("error", "Repository validation failed")}
        ...
        try:
            ...
            pr_url = self._create_github_pr(
                branch_name=branch_name, title=commit_message, body=self._generate_pr_body(files_added, rules)
            )
    ...
    def _create_github_pr(self, branch_name: str, title: str, body: str) -> str | None:
        """
