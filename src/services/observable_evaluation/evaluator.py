...
class ObservableModelEvaluator:
    ...
    def _load_model(self, model_path: Optional[str] = None) -> bool:
        ...
        if self.inference_service is None:
            if model_path is None:
                model_path_obj = ObservableModelInference.find_model_path(
    ...
    def evaluate(
        ...
        if not self._load_model(model_path):
            ...
            if model_path is None:
                logger.warning(f"Model {self.model_name} v{self.model_version} not found. Attempting to find latest available model...")
    ...
    def _load_annotations_by_article(
        self, session: Session, usage: str
    ) -> Dict[int, List[ArticleAnnotationTable]]:
        """
        Load annotations grouped by article.
    ...
    def _compute_eval_metrics(
        ...
        avg_token_overlap = (
            sum(total_overlap_scores) / len(total_overlap_scores)
            if total_overlap_scores
            else 0.0
        )
        avg_length_delta = (
            sum(total_length_deltas) / len(total_length_deltas)
            if total_length_deltas
            else 0.0
        )
    ...
    def _compute_gold_metrics(
        ...
        exact_match_rate = (
            total_exact_matches / total_gold_spans if total_gold_spans > 0 else 0.0
        )
        over_extraction_rate = (
            total_over_extractions / total_gold_spans if total_gold_spans > 0 else 0.0
        )
        under_extraction_rate = (
            total_under_extractions / total_gold_spans if total_gold_spans > 0 else 0.0
        )
        hallucination_rate = (
            total_hallucinations / total_gold_spans if total_gold_spans > 0 else 0.0
        )
        multi_command_merge_rate = (
            total_merged_commands / total_gold_spans if total_gold_spans > 0 else 0.0
        )
        
        # Article-level metrics
        zero_fp_pass_rate = (
            articles_with_zero_fp / total_articles if total_articles > 0 else 0.0
        )
    ...
    def _store_failure_taxonomy(
        ...
        for failure_type, article_counts in failure_taxonomy.items():
            for article_id, count in article_counts.items():
                ...
                article_result = next(
                    (r for r in article_results if r["article_id"] == article_id), None
                )
        ...
        session.commit()
        logger.info(
            f"Stored failure taxonomy: {sum(len(v) for v in failure_taxonomy.values())} failure records"
        )
