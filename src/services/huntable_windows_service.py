...
class HuntableWindowsService:
    ...
    def __init__(self, model_path: Path | None = None, scaler_path: Path | None = None):
        """
        Initialize service.
        ...
        """
        self.model_path = (
            model_path or Path(__file__).parent.parent.parent / "models" / "huntable_windows_classifier.pkl"
        )
    ...
    def detect_windows_huntables(
        ...
        """
        results = {"has_windows_huntables": False, "confidence": 0.0, "method": "unknown"}
        ...
        if use_ml_classifier:
            ...
            if self.classifier and self.scaler and article_metadata:
                try:
                    ...
                    ml_prediction = {"prediction": int(prediction), "probability": float(probability)}
        ...
        if perfect_check and ml_prediction:
            ...
            if perfect_check["has_windows_huntables"] or ml_prediction["prediction"] == 1:
                results["has_windows_huntables"] = True
                results["confidence"] = max(perfect_check.get("confidence", 0.0), ml_prediction.get("probability", 0.0))
