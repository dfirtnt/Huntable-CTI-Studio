...
class SigmaMatchingService:
    ...
    def __init__(self, db_session: Session, config_models: dict[str, str] | None = None):
        """
    ...
    def match_article_to_rules(self, article_id: int, threshold: float = 0.0, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute(query_text, {"embedding": embedding_str, "zero_vec": zero_vector, "limit": limit})
    ...
    def match_chunks_to_rules(
        self, article_id: int, threshold: float = 0.7, limit_per_chunk: int = 5
    ) -> list[dict[str, Any]]:
        """
            ...
            for chunk in chunks:
                try:
                    ...
                    result = self.db.execute(
                        query, {"embedding": embedding_str, "zero_vec": zero_vector, "limit": limit_per_chunk}
                    )
    ...
    def get_coverage_summary(self, article_id: int) -> dict[str, int]:
        ...
        """
        try:
            matches = self.db.query(ArticleSigmaMatchTable).filter_by(article_id=article_id).all()

            summary = {"covered": 0, "extend": 0, "new": 0, "total": len(matches)}
...
Return JSON array only, no markdown formatting."""
                ...
                try:
                    ...
                    for score_data in llm_scores:
                        ...
                        if 0 <= idx < len(top_candidates):
                            top_candidates[idx]["similarity"] = float(
                                score_data.get("similarity", top_candidates[idx]["similarity"])
                            )
                            top_candidates[idx]["llm_explanation"] = score_data.get(
                        ...
                        matches.append(
                            {
                                ...
                                "logic_shape_similarity": match.get(
                                    "logic_shape_similarity"
                                ),  # Preserve None for early exit
                                # Explainability
    ...
    def _apply_semantic_clustering(
        self, matches: list[dict[str, Any]], tight_threshold: float = 0.05
    ) -> list[dict[str, Any]]:
        """
