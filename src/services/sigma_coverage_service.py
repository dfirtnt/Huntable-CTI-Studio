...
from src.database.models import (
    ArticleTable, SigmaRuleTable, ChunkAnalysisResultTable
)
...
class SigmaCoverageService:
    ...
    def extract_article_behaviors(self, article_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Extracted behaviors for article {article_id}: "
    ...
    def calculate_behavior_overlap(
        self,
        article_behaviors: Dict[str, Any],
    ...
    def classify_match(
        ...
        try:
            ...
            if similarity_score >= 0.85 and overlap_ratio >= 0.7:
                ...
                reasoning = (f"Article behaviors are well covered by this rule "
                ...
                reasoning = (f"Partial overlap detected "
                           f"({matched_count}/{overlap['total_behaviors']} behaviors matched, "
                           f"{similarity_score:.2f} similarity). "
            ...
            logger.debug(f"Classified match for article {article_id} and rule {sigma_rule.rule_id}: "
    ...
    def analyze_article_coverage(self, article_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if coverage_counts['covered'] > 0:
                recommendation = (f"{coverage_counts['covered']} existing rule(s) cover this article. "
                                f"No new rules needed.")
            elif coverage_counts['extend'] > 0:
                recommendation = (f"{coverage_counts['extend']} rule(s) partially cover this article. "
