...
class LangfuseEvalClient:
    ...
    def create_experiment(
        self, eval_run_id: UUID, snapshot_id: UUID, snapshot_data: dict[str, Any]
    ) -> dict[str, Any] | None:
        """
            ...
            if experiment and experiment.get("trace"):
                ...
                trace = experiment_span.start_span(
                    ...
                    input={
                        ...
                        if hasattr(article_item, "input")
                        else "",
                        "article_title": article_item.input.get("article_title", "")
                        if hasattr(article_item, "input")
                        else "",
                    },
                    metadata={
        ...
        try:
            ...
            if extraction_result:
                ...
                if full_result:
                    ...
                    output["full_result_keys"] = (
                        list(full_result.keys()) if isinstance(full_result, dict) else "not a dict"
                    )
    ...
    def finalize_experiment(self, experiment: dict[str, Any] | None, aggregate_metrics: dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            if "accuracy" in aggregate_metrics:
                experiment_trace.score(name="accuracy", value=aggregate_metrics["accuracy"])

            if "mean_count_diff" in aggregate_metrics:
                experiment_trace.score(name="mean_count_diff", value=aggregate_metrics["mean_count_diff"])

            if "passed" in aggregate_metrics:
                experiment_trace.score(name="passed", value=1 if aggregate_metrics["passed"] else 0)
