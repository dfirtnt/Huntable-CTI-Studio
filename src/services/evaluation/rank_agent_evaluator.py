...
class RankAgentEvaluator(BaseAgentEvaluator):
    ...
    async def evaluate_dataset(
        self, test_data_path: Path, llm_service: LLMService | None = None, **kwargs
    ) -> dict[str, Any]:
        """
        ...
        try:
            for example in test_data:
                ...
                # Evaluate article
                try:
                    result = await self._evaluate_article(article, llm_service, expected_score, ground_truth_hunt_score)
    ...
    async def _evaluate_article(
        ...
        ground_truth_hunt_score: float | None = None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"article_id": article.id, "error": str(e), "ranking_result": None, "evaluation": None}
        ...
        evaluation = self._evaluate_ranking_result(
            score, reasoning, expected_score, ground_truth_hunt_score, processing_time
        )
    ...
    def _evaluate_ranking_result(
        ...
        ground_truth_hunt_score: float | None,
        ...
        if reasoning:
            ...
            evaluation["reasoning_has_structure"] = any(
                marker in reasoning.lower() for marker in ["because", "therefore", "however", "additionally"]
            )
        else:
    ...
    def calculate_metrics(self) -> dict[str, Any]:
        ...
        # Threshold accuracy
        threshold_accuracies = [e["threshold_accuracy"] for e in evaluations if e.get("threshold_accuracy") is not None]
        ...
        metrics = {
            ...
            # Threshold accuracy
            "threshold_accuracy": sum(threshold_accuracies) / len(threshold_accuracies)
            if threshold_accuracies
            else None,
            # Reasoning quality
            "reasoning_rate": sum(1 for e in evaluations if e.get("has_reasoning")) / len(evaluations),
            "avg_reasoning_length": statistics.mean([e.get("reasoning_length", 0) for e in evaluations])
            if evaluations
            else 0,
            # Processing time
