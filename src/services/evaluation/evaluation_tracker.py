...
class EvaluationTracker:
    ...
    def compare_evaluations(
        ...
        """
        baseline = self.db_session.query(AgentEvaluationTable).filter(
            AgentEvaluationTable.id == baseline_id
        ).first()
        
        current = self.db_session.query(AgentEvaluationTable).filter(
            AgentEvaluationTable.id == current_id
        ).first()
