...
class EvalRunner:
    ...
    def run_evaluation(
        ...
        """
        # Load eval run
        eval_run = self.db.query(EvalRunTable).filter(
            EvalRunTable.id == eval_run_id
        ).first()
        ...
        # Load snapshot
        snapshot = self.db.query(EvalPresetSnapshotTable).filter(
            ...
            if hasattr(dataset, "items") and dataset.items:
                for item in dataset.items:
                    try:
                        ...
                        article_text = item.input.get("article_text", "") if hasattr(item, "input") and isinstance(item.input, dict) else ""
                        article_title = item.input.get("article_title", "") if hasattr(item, "input") and isinstance(item.input, dict) else ""
                        ...
                        try:
                            ...
                            if predicted_count == 0 and len(cmdline_items) == 0:
                                ...
                                if full_result:
                                    ...
                                    logger.warning(f"Full result cmdline_items: {type(full_result.get('cmdline_items'))}, length: {len(full_result.get('cmdline_items', []))}")
                            ...
                            if trace and article_text:
                                try:
                                    trace.update(
                                        input={
                                            "article_text": article_text[:1000] + "..." if len(article_text) > 1000 else article_text,
                            ...
                            logger.error(f"Extraction failed for item {getattr(item, 'id', 'unknown')}: {e}", exc_info=True)
                        ...
                        if trace:
                            try:
                                ...
                                trace.end()
                                logger.debug(f"Trace ended for article extraction")
                            except Exception as e:
    ...
    def _run_extraction(self, item: Any, snapshot_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Extracting from article: title='{article_title[:50] if article_title else 'N/A'}...', text_length={len(article_text)}")
        
        if not article_text:
            logger.error(f"Article text is empty! Dataset item structure: input={type(item.input)}, has_input={hasattr(item, 'input')}")
            if hasattr(item, "input"):
                logger.error(f"Item input keys: {list(item.input.keys()) if isinstance(item.input, dict) else 'not a dict'}")
        ...
        if not isinstance(cmdline_items, list):
            ...
            cmdline_items = [] if cmdline_items is None else [cmdline_items] if not isinstance(cmdline_items, list) else cmdline_items
        ...
        logger.info(f"Extraction result: count={count} (from field), array_length={array_count}, items={len(cmdline_items)}")
