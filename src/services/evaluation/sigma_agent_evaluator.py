...
from src.services.sigma_extended_validator import SigmaExtendedValidator
from src.services.sigma_huntability_scorer import SigmaHuntabilityScorer
from src.services.sigma_novelty_detector import SigmaNoveltyDetector
from src.services.sigma_semantic_scorer import SigmaSemanticScorer
from src.services.sigma_stability_tester import SigmaStabilityTester
...
class SigmaAgentEvaluator(BaseAgentEvaluator):
    ...
    async def evaluate_dataset(self, test_data_path: Path, generate_rule_func=None, **kwargs) -> dict[str, Any]:
        """
        ...
        for example in test_data:
            ...
            if not rule_yaml:
                ...
                results.append({"article_id": article_id, "error": "No rule provided", "evaluation": None})
                continue

            # Evaluate rule
            evaluation = await self._evaluate_single_rule(
                rule_yaml, reference_rule=reference_rules[0] if reference_rules else None, article_id=article_id
            )

            results.append({"article_id": article_id, "rule_yaml": rule_yaml, "evaluation": evaluation})

        self.results = results
        return self.calculate_metrics()

    async def _evaluate_single_rule(
        self, rule_yaml: str, reference_rule: str | None = None, article_id: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        if not structural_result.final_pass:
            return {
                "structural_validation": {
                    "pySigma": {"passed": structural_result.pySigma_passed, "errors": structural_result.pySigma_errors},
                    "extended_checks": {
        ...
        if reference_rule:
            try:
                semantic_comparison = await self.semantic_scorer.compare_rules(rule_yaml, reference_rule)
            except Exception as e:
    ...
    def calculate_metrics(self) -> dict[str, Any]:
        ...
        structural_passed = sum(1 for e in evaluations if e.get("structural_validation", {}).get("final_pass", False))
        ...
        novelty_scores = [
            ...
            for e in evaluations
            if e.get("novelty")
            and isinstance(e.get("novelty"), dict)
            and e.get("novelty", {}).get("novelty_score") is not None
        ]
