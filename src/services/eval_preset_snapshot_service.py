...
from src.database.models import AgenticWorkflowConfigTable, AgentPromptVersionTable, EvalPresetSnapshotTable
...
class EvalPresetSnapshotService:
    ...
    def create_snapshot(self, preset_id: int | None = None, description: str | None = None) -> UUID:
        """
        ...
        if preset_id:
            config = (
                self.db.query(AgenticWorkflowConfigTable).filter(AgenticWorkflowConfigTable.id == preset_id).first()
            )
        else:
            # Get active config
            config = (
        ...
        if config.agent_prompts:
            logger.info(
                f"Loading prompts from config.agent_prompts. Available agents: {list(config.agent_prompts.keys())}"
            )
            for agent_name, prompt_data in config.agent_prompts.items():
                ...
                if isinstance(prompt_data, dict) and "prompt" in prompt_data:
                    ...
                    prompt_version = (
                        self.db.query(AgentPromptVersionTable)
                        .filter(
        ...
        existing = (
            self.db.query(EvalPresetSnapshotTable)
            .filter(EvalPresetSnapshotTable.snapshot_hash == snapshot_hash)
            .first()
        )
    ...
    def get_snapshot(self, snapshot_id: UUID) -> EvalPresetSnapshotTable | None:
        """Get snapshot by UUID."""
        return self.db.query(EvalPresetSnapshotTable).filter(EvalPresetSnapshotTable.id == snapshot_id).first()
