...
def fix_file(file_path: Path):
    ...
    if 'from src.models.article import ArticleCreate' not in content:
        ...
        lines = content.split('\n')
        last_import_idx = max(i for i, line in enumerate(lines) if line.startswith('import ') or line.startswith('from '))
...
'''
        ...
        lines = content.split('\n')
        class_idx = next(i for i, line in enumerate(lines) if line.strip().startswith('@pytest.mark.') or line.strip().startswith('class '))
