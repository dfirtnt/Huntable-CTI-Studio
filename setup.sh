...
# Optional: configure enterprise SSO (trusted-header reverse proxy) during setup.
configure_enterprise_auth() {
    # Opt-in only. Non-interactive installs stay local (AUTH_MODE=disabled).
    if [[ "$NON_INTERACTIVE" == "true" ]]; then
        startup_set_env_key ".env" "AUTH_MODE" "disabled"
        return 0
    fi
    ...
    startup_set_env_key ".env" "AUTH_MODE" "trusted_header"
    startup_set_env_key ".env" "CSRF_ENABLED" "auto"
    startup_set_env_key ".env" "TRUSTED_HOSTS" "$hostname"
    startup_set_env_key ".env" "CORS_ALLOWED_ORIGINS" "https://$hostname"
    startup_set_env_key ".env" "AUTH_TRUSTED_PROXY_HEADER" "X-Huntable-Verified"
    startup_set_env_key ".env" "AUTH_TRUSTED_PROXY_VALUE" "true"
    [[ -n "$admin_g" ]] && startup_set_env_key ".env" "AUTH_ADMIN_GROUPS" "$admin_g"
    [[ -n "$operator_g" ]] && startup_set_env_key ".env" "AUTH_OPERATOR_GROUPS" "$operator_g"
    [[ -n "$reviewer_g" ]] && startup_set_env_key ".env" "AUTH_REVIEWER_GROUPS" "$reviewer_g"
    [[ -n "$analyst_g" ]] && startup_set_env_key ".env" "AUTH_ANALYST_GROUPS" "$analyst_g"
    [[ "$make_prod" == "true" ]] && startup_set_env_key ".env" "APP_ENV" "production"
