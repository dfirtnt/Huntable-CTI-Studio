...
def test_agent_config_context_tokens_default_none():
    """ContextTokens defaults to None."""
    agent = AgentConfig(Provider="openai", Model="gpt-4", Temperature=0.0, TopP=0.9, Enabled=True)
    assert agent.ContextTokens is None


def test_agent_config_context_tokens_accepts_int():
    """ContextTokens accepts a positive int."""
    agent = AgentConfig(Provider="openai", Model="gpt-4", Temperature=0.0, TopP=0.9, Enabled=True, ContextTokens=32000)
    assert agent.ContextTokens == 32000


def test_flatten_context_tokens_emitted_when_set():
    """flatten_for_llm_service emits {prefix}_context_tokens when ContextTokens is set."""
    raw = {
        "Version": "2.0",
    ...
    assert flat["CmdlineExtract_context_tokens"] == 32000


def test_flatten_context_tokens_absent_when_none():
    """flatten_for_llm_service does not emit context_tokens key when ContextTokens is None."""
    raw = {
        "Version": "2.0",
        "Metadata": {"CreatedAt": "x", "Description": "x"},
        "Thresholds": {
    ...
    config = WorkflowConfigV2.model_validate(raw)
    flat = config.flatten_for_llm_service()
    assert "CmdlineExtract_context_tokens" not in flat
