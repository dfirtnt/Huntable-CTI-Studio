...
class TestSigmaMatchPersistence:
    """store_match / get_article_matches persistence helpers."""
    ...
    def test_store_match_creates_new_record(self, service, mock_db_session):
        filter_by = mock_db_session.query.return_value.filter_by
        filter_by.return_value.first.return_value = None

        result = service.store_match(
            article_id=1,
            sigma_rule_id=42,
            ...
            coverage_reasoning="novel rule",
            matched_discriminators=["cmdline"],
        )
    ...
    def test_store_match_updates_existing_record(self, service, mock_db_session):
        existing = Mock()
        filter_by = mock_db_session.query.return_value.filter_by
        filter_by.return_value.first.return_value = existing
        ...
        result = service.store_match(
            ...
            matched_lolbas=["rundll32.exe"],
        )

        assert result is existing
        assert existing.similarity_score == 0.55
        assert existing.coverage_status == "extend"
        assert existing.matched_lolbas == ["rundll32.exe"]
        mock_db_session.add.assert_not_called()
        mock_db_session.commit.assert_called_once()

    def test_store_match_returns_none_on_db_error(self, service, mock_db_session):
        mock_db_session.query.return_value.filter_by.return_value.first.side_effect = RuntimeError("db down")
    ...
    def test_get_article_matches_returns_formatted_rows(self, service, mock_db_session):
        match = Mock()
        match.id = 10
        match.similarity_score = 0.88
        match.match_level = "article"
        match.chunk_id = None
        match.coverage_status = "covered"
        match.coverage_confidence = 0.9
        match.coverage_reasoning = "same atoms"
        match.matched_discriminators = ["image"]
        match.matched_lolbas = []
        match.matched_intelligence = []
        match.created_at = Mock(isoformat=Mock(return_value="2026-07-18T12:00:00"))
        ...
        rule.level = "medium"
        rule.status = "stable"
        rule.file_path = "rules/test.yml"

        ordered = mock_db_session.query.return_value.join.return_value.filter.return_value.order_by.return_value
        ordered.all.return_value = [(match, rule)]

        rows = service.get_article_matches(article_id=1)

        assert len(rows) == 1
        assert rows[0]["match_id"] == 10
        assert rows[0]["rule_id"] == "abc-123"
        assert rows[0]["similarity_score"] == 0.88
        assert rows[0]["created_at"] == "2026-07-18T12:00:00"

    def test_get_article_matches_filters_by_match_level(self, service, mock_db_session):
        chain = mock_db_session.query.return_value.join.return_value.filter.return_value
        chain.order_by.return_value.all.return_value = []

        service.get_article_matches(article_id=5, match_level="chunk")

        chain.filter.assert_called_once()

    def test_get_article_matches_returns_empty_on_error(self, service, mock_db_session):
        mock_db_session.query.side_effect = RuntimeError("db down")
        assert service.get_article_matches(article_id=1) == []
