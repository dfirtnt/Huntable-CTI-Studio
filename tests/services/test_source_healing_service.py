...
class TestUARotationProbe:
    """Test browser-header retry on 403 before flagging bot protection."""

    @pytest.mark.asyncio
    async def test_ua_rotation_recovers_plain_403(self):
        """403 without bot signals + successful retry returns ua_rotation_recovered=True, no bot flag."""
        from src.services.source_healing_service import SourceHealingService

        forbidden = Mock(spec=httpx.Response)
        forbidden.status_code = 403
        forbidden.url = httpx.URL("https://example.com/")
        forbidden.headers = {"content-type": "text/html"}
        forbidden.text = "<html><body>Forbidden</body></html>"

        recovered = Mock(spec=httpx.Response)
        recovered.status_code = 200
        recovered.url = httpx.URL("https://example.com/")
        recovered.headers = {"content-type": "text/html"}
        recovered.text = "<html><body>Welcome to the blog</body></html>"

        def _router(url, **kwargs):
            if "Accept" in kwargs.get("headers", {}):
                return recovered
            return forbidden
        ...
        with patch("httpx.AsyncClient.get", AsyncMock(side_effect=_router)):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        http_results = [r for r in probe_results if r.get("label") == "url"]
        assert len(http_results) == 1
        assert http_results[0]["ua_rotation_recovered"] is True
        assert http_results[0]["status_code"] == 200

        bot_results = [r for r in probe_results if r.get("label") == "bot_protection_detected"]
        assert len(bot_results) == 0

    @pytest.mark.asyncio
    async def test_ua_rotation_retry_sends_accept_headers(self):
        """The retry must include Accept, Accept-Language, and Accept-Encoding headers."""
        from src.services.source_healing_service import SourceHealingService
        ...
        recovered.url = httpx.URL("https://example.com/")
        recovered.headers = {"content-type": "text/html"}
        recovered.text = "<html><body>Blog</body></html>"
        ...
        with patch("httpx.AsyncClient.get", mock_get):
            await SourceHealingService._probe_urls(source_snapshot)

        retry_calls = [call for call in mock_get.call_args_list if "Accept" in call.kwargs.get("headers", {})]
        assert len(retry_calls) >= 1
        retry_headers = retry_calls[0].kwargs["headers"]
        assert "Accept-Language" in retry_headers
        assert "Accept-Encoding" in retry_headers

    @pytest.mark.asyncio
    async def test_ua_rotation_does_not_suppress_cloudfront(self):
        """CloudFront 403 is still flagged as bot protection even after a failed retry."""
        from src.services.source_healing_service import SourceHealingService

        cf_response = Mock(spec=httpx.Response)
        cf_response.status_code = 403
        cf_response.url = httpx.URL("https://example.com/")
        cf_response.headers = {"content-type": "text/html", "x-amz-cf-id": "abc123"}
        cf_response.text = "Request blocked. Generated by cloudfront (CloudFront)"
        ...
        with patch("httpx.AsyncClient.get", return_value=cf_response):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        bot_results = [r for r in probe_results if r.get("label") == "bot_protection_detected"]
        assert len(bot_results) >= 1
        assert bot_results[0]["provider"] == "CloudFront"


class TestPlatformDetection:
    """Test platform fingerprinting probe and LLM pre-filter short-circuit."""

    @pytest.mark.asyncio
    async def test_probe_detects_ghost_from_response_header(self):
        """X-Ghost-Cache-Status header identifies a Ghost site with /rss/ hint."""
        from src.services.source_healing_service import SourceHealingService

        ghost_response = Mock(spec=httpx.Response)
        ghost_response.status_code = 200
        ghost_response.url = httpx.URL("https://blog.example.com/")
        ghost_response.headers = {"content-type": "text/html", "x-ghost-cache-status": "HIT"}
        ghost_response.text = "<html><head><title>Blog</title></head><body>Posts</body></html>"
        ...
        with patch("httpx.AsyncClient.get", return_value=ghost_response):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        platform_results = [r for r in probe_results if r.get("label") == "platform_detection"]
        assert len(platform_results) == 1
        assert platform_results[0]["platform"] == "ghost"
        assert platform_results[0]["rss_hint"].endswith("/rss/")

    @pytest.mark.asyncio
    async def test_probe_detects_ghost_from_meta_generator(self):
        """Meta generator tag with Ghost value identifies a Ghost site."""
        from src.services.source_healing_service import SourceHealingService

        ghost_response = Mock(spec=httpx.Response)
        ghost_response.status_code = 200
        ghost_response.url = httpx.URL("https://blog.example.com/")
        ghost_response.headers = {"content-type": "text/html"}
        ghost_response.text = '<html><head><meta name="generator" content="Ghost 5.0"/></head><body>Blog</body></html>'
    ...
    @pytest.mark.asyncio
    async def test_probe_detects_substack_from_url(self):
        """Source URL containing substack.com is identified as Substack with /feed hint."""
        from src.services.source_healing_service import SourceHealingService

        substack_response = Mock(spec=httpx.Response)
        substack_response.status_code = 200
        substack_response.url = httpx.URL("https://example.substack.com/")
        substack_response.headers = {"content-type": "text/html"}
        substack_response.text = "<html><body>Newsletter</body></html>"
        ...
        with patch("httpx.AsyncClient.get", return_value=substack_response):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        platform_results = [r for r in probe_results if r.get("label") == "platform_detection"]
        assert platform_results[0]["platform"] == "substack"
        assert "/feed" in platform_results[0]["rss_hint"]

    @pytest.mark.asyncio
    async def test_probe_no_platform_for_generic_site(self):
        """Generic HTML without platform signals returns platform=None."""
        from src.services.source_healing_service import SourceHealingService

        generic_response = Mock(spec=httpx.Response)
        generic_response.status_code = 200
        generic_response.url = httpx.URL("https://example.com/")
        generic_response.headers = {"content-type": "text/html"}
        generic_response.text = (
            "<html><head><title>CTI Blog</title></head><body><p>Security research articles</p></body></html>"
        )
        ...
        with patch("httpx.AsyncClient.get", return_value=generic_response):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        platform_results = [r for r in probe_results if r.get("label") == "platform_detection"]
        assert len(platform_results) == 1
        assert platform_results[0]["platform"] is None

    @pytest.mark.asyncio
    async def test_platform_prefilter_applies_ghost_rss_template(self):
        """Ghost detection on round 1 sets rss_url to /rss/ without calling LLM."""
        from src.services.source_healing_config import SourceHealingConfig
        from src.services.source_healing_service import SourceHealingService
        ...
        probe_results = [
            {
                "label": "platform_detection",
                "platform": "ghost",
                "rss_hint": "https://blog.example.com/rss/",
            }
        ]
        ...
        mock_llm_cls.assert_not_called()
        assert result["actions"] == [{"field": "rss_url", "value": "https://blog.example.com/rss/"}]
        assert "ghost" in result["diagnosis"].lower()

    @pytest.mark.asyncio
    async def test_platform_prefilter_applies_substack_feed_template(self):
        """Substack detection on round 1 sets rss_url to /feed without calling LLM."""
        from src.services.source_healing_config import SourceHealingConfig
        from src.services.source_healing_service import SourceHealingService
        ...
        probe_results = [
            {
                "label": "platform_detection",
                "platform": "substack",
                "rss_hint": "https://example.substack.com/feed",
            }
        ]
        ...
        mock_llm_cls.assert_not_called()
        assert result["actions"] == [{"field": "rss_url", "value": "https://example.substack.com/feed"}]
        assert "substack" in result["diagnosis"].lower()

    @pytest.mark.asyncio
    async def test_platform_prefilter_skips_when_rss_already_matches(self):
        """If current rss_url already matches the platform hint, fall through to LLM."""
        from src.services.source_healing_config import SourceHealingConfig
        from src.services.source_healing_service import SourceHealingService
        ...
        with (
            ...
            llm._canonicalize_provider.return_value = "openai"
            llm.request_chat.return_value = {
                "choices": [{"message": {"content": '{"diagnosis": "LLM fallback", "actions": []}'}}]
            }
            mock_llm_cls.return_value = llm
    ...
    @pytest.mark.asyncio
    async def test_platform_prefilter_skips_on_retry_round(self):
        """On rounds 2+, platform pre-filter is bypassed even if platform is detected."""
        from src.services.source_healing_config import SourceHealingConfig
        from src.services.source_healing_service import SourceHealingService
        ...
        previous_attempts = [
            {
                "round": 1,
                "diagnosis": "Ghost platform detected",
                "actions_applied": [],
        ...
        with (
            ...
            llm._canonicalize_provider.return_value = "openai"
            llm.request_chat.return_value = {
                "choices": [{"message": {"content": '{"diagnosis": "LLM retry", "actions": []}'}}]
            }
            mock_llm_cls.return_value = llm

            await service._analyze_with_llm(source_snapshot, [], probe_results, previous_attempts=previous_attempts)

        llm.request_chat.assert_called_once()


class TestTrafilaturaProbeFallback:
    """Test trafilatura content extraction probe."""

    @pytest.mark.asyncio
    async def test_probe_returns_empty_when_not_installed(self):
        """Probe degrades silently to [] when trafilatura is not importable."""
        import sys
        ...
        ok_response.url = httpx.URL("https://example.com/")
        ok_response.headers = {"content-type": "text/html"}
        ok_response.text = "<html><body>Blog content</body></html>"
        ...
        with (
            patch("httpx.AsyncClient.get", return_value=ok_response),
            patch.dict(sys.modules, {"trafilatura": None}),
        ):
            probe_results = await SourceHealingService._probe_urls(source_snapshot)

        trafilatura_results = [r for r in probe_results if r.get("label") == "trafilatura_extraction"]
        assert len(trafilatura_results) == 0

    @pytest.mark.asyncio
    async def test_probe_returns_sample_text_on_successful_extraction(self):
        """Probe returns sample_text when trafilatura extracts substantial content."""
        import sys
        ...
        ok_response.url = httpx.URL("https://example.com/")
        ok_response.headers = {"content-type": "text/html"}
        ok_response.text = "<html><body>Article body here</body></html>"

        mock_trafilatura = MagicMock()
        mock_trafilatura.extract.return_value = (
            "Threat actor TA505 has been observed deploying a new loader variant. " * 5
        )
        ...
        trafilatura_results = [r for r in probe_results if r.get("label") == "trafilatura_extraction"]
        assert len(trafilatura_results) == 1
        assert "sample_text" in trafilatura_results[0]
        assert len(trafilatura_results[0]["sample_text"]) > 0

    @pytest.mark.asyncio
    async def test_probe_returns_no_content_verdict_for_short_extraction(self):
        """When extracted text is shorter than 100 chars, returns no_content_extracted verdict."""
        import sys
        from unittest.mock import MagicMock
        ...
        ok_response.url = httpx.URL("https://example.com/")
        ok_response.headers = {"content-type": "text/html"}
        ok_response.text = "<html><body>Loading...</body></html>"

        mock_trafilatura = MagicMock()
        mock_trafilatura.extract.return_value = "Too short."
        ...
        trafilatura_results = [r for r in probe_results if r.get("label") == "trafilatura_extraction"]
        assert len(trafilatura_results) == 1
        assert trafilatura_results[0].get("verdict") == "no_content_extracted"


class TestPromptBuilderNewProbes:
    """Test _build_user_prompt rendering for platform_detection and trafilatura_extraction."""

    def _build(self, probe_results):
        from src.services.source_healing_service import SourceHealingService

        return SourceHealingService._build_user_prompt(
            source_snapshot={"name": "Test", "url": "https://example.com/", "rss_url": None, "config": {}},
            error_history=[],
            probe_results=probe_results,
        )

    def test_prompt_renders_trafilatura_sample_text(self):
        """Trafilatura extraction with content renders with selector-inference hint."""
        prompt = self._build(
            [
                {
                    "label": "trafilatura_extraction",
                    "sample_text": "Threat actor TA505 deploying a new loader variant targeting financial institutions.",
                }
            ]
        )
        assert "trafilatura" in prompt.lower()
        assert "body_selectors" in prompt
        assert "TA505" in prompt

    def test_prompt_renders_trafilatura_no_content_verdict(self):
        """Trafilatura no-content verdict renders a brief diagnostic note."""
        prompt = self._build([{"label": "trafilatura_extraction", "verdict": "no_content_extracted"}])
        assert "no substantial content" in prompt.lower()

    def test_prompt_renders_ghost_platform_with_hint(self):
        """Ghost platform detection renders platform name and rss_hint URL."""
        prompt = self._build(
            [
                {
                    "label": "platform_detection",
        ...
        assert "https://blog.example.com/rss/" in prompt

    def test_prompt_renders_platform_none_as_not_identified(self):
        """When platform=None, prompt says no known platform identified."""
        prompt = self._build([{"label": "platform_detection", "platform": None}])
        assert "no known platform" in prompt.lower()
