...
class TestQAAgentService:
    ...
    def mock_llm_service(self):
        ...
        service = Mock()
        service.request_chat = AsyncMock(
            return_value={
                "choices": [
                    {
                        "message": {
                            "content": json.dumps(
                                {"summary": "Output is compliant and accurate", "issues": [], "verdict": "pass"}
                            )
                        }
                    }
                ]
    ...
    def sample_agent_output(self):
        """Sample agent output."""
        return {"rank": 8.5, "reasoning": "High threat intelligence value", "indicators": ["PowerShell", "persistence"]}
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_with_issues(
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_critical_failure(
        self, service, sample_article, sample_agent_output, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_with_config(
        self, service, sample_article, sample_agent_output, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_content_truncation(
        self, service, sample_article, sample_agent_output, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_error_handling(
        self, service, sample_article, sample_agent_output, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_evaluate_agent_output_json_parsing(
        self, service, sample_article, sample_agent_output, mock_llm_service
    ):
