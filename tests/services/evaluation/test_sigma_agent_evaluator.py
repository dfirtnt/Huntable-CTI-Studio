...
class TestSigmaAgentEvaluator:
    ...
    def mock_validator(self):
        ...
        validator.validate = Mock(
            return_value=ExtendedValidationResult(
                pySigma_passed=True,
                pySigma_errors=[],
    ...
    def mock_normalizer(self):
        ...
        normalizer.extract_behavioral_core = Mock(
            return_value=BehavioralCore(
                behavior_selectors=[], commandlines=[], process_chains=[], core_hash="test-hash", selector_count=0
            )
        )
        return normalizer
    ...
    def mock_huntability_scorer(self):
        ...
        scorer = Mock()
        scorer.score_rule = Mock(
            return_value=HuntabilityScore(
                score=85.0, false_positive_risk="low", coverage_notes="Good coverage", breakdown={}
            )
        )
        return scorer
    ...
    def mock_stability_tester(self):
        ...
        tester = Mock()
        tester.test_stability = AsyncMock(
            return_value=StabilityResult(
                unique_hashes=1,
    ...
    def mock_novelty_detector(self):
        ...
        detector = Mock()
        detector.detect_novelty = AsyncMock(
            return_value={"novelty_label": "NOVEL", "novelty_score": 1.0, "logsource_key": "", "top_matches": []}
        )
        return detector
    ...
    def evaluator(
        ...
        mock_validator,
        mock_normalizer,
        mock_semantic_scorer,
        mock_huntability_scorer,
        mock_stability_tester,
        mock_novelty_detector,
    ):
        """Create SigmaAgentEvaluator with mocked dependencies."""
        with (
            patch("src.services.evaluation.sigma_agent_evaluator.SigmaExtendedValidator", return_value=mock_validator),
            patch(
                "src.services.evaluation.sigma_agent_evaluator.SigmaBehavioralNormalizer", return_value=mock_normalizer
            ),
            patch(
                "src.services.evaluation.sigma_agent_evaluator.SigmaSemanticScorer", return_value=mock_semantic_scorer
            ),
            patch(
                "src.services.evaluation.sigma_agent_evaluator.SigmaHuntabilityScorer",
                return_value=mock_huntability_scorer,
            ),
            patch(
                "src.services.evaluation.sigma_agent_evaluator.SigmaStabilityTester", return_value=mock_stability_tester
            ),
            patch(
                "src.services.evaluation.sigma_agent_evaluator.SigmaNoveltyDetector", return_value=mock_novelty_detector
            ),
        ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_evaluate_single_rule_success(
        self,
        evaluator,
        sample_rule,
        sample_reference_rule,
        mock_validator,
        mock_normalizer,
        mock_semantic_scorer,
        mock_huntability_scorer,
        ...
        result = await evaluator._evaluate_single_rule(
            rule_yaml=sample_rule, reference_rule=sample_reference_rule, article_id=1
        )
    ...
    @pytest.mark.asyncio
    async def test_evaluate_single_rule_validation_failure(
        self, evaluator, sample_rule, mock_validator, mock_normalizer
    ):
        ...
        result = await evaluator._evaluate_single_rule(rule_yaml=sample_rule, reference_rule=None, article_id=1)
    ...
    async def test_evaluate_dataset(self, evaluator, tmp_path, sample_rule):
        ...
        result = await evaluator.evaluate_dataset(test_data_path=test_file, generate_rule_func=mock_generate_rule)
