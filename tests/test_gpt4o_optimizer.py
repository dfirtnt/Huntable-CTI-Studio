"""Tests for LLM content optimization functionality."""
...
from src.utils.llm_optimizer import LLMOptimizer, optimize_article_content, estimate_llm_cost, estimate_gpt4o_cost, get_optimization_stats


class TestLLMOptimizer:
    """Test LLMOptimizer functionality."""
    ...
    def test_init_with_model_path(self):
        ...
        optimizer = LLMOptimizer(model_path)
        assert optimizer.content_filter.model_path == model_path

    @pytest.mark.asyncio
    async def test_optimize_content_success(self, optimizer, sample_content, mock_filter_result):
        ...
        with patch.object(optimizer.content_filter, 'model', None):
            with patch.object(optimizer.content_filter, 'load_model') as mock_load:
                with patch.object(optimizer.content_filter, 'filter_content', return_value=mock_filter_result):
                    result = await optimizer.optimize_content(sample_content)
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_custom_parameters(self, optimizer, sample_content, mock_filter_result):
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_filtering_failure(self, optimizer, sample_content):
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_model_not_loaded(self, optimizer, sample_content, mock_filter_result):
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_model_already_loaded(self, optimizer, sample_content, mock_filter_result):
    ...
    def test_get_cost_estimate_exception(self, optimizer, sample_content):
        ...
        with patch('src.utils.content_filter.ContentFilter') as mock_filter_class:
            mock_filter = Mock()
            mock_filter.model = None
            mock_filter.load_model.side_effect = Exception("Test error")
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_stats_update(self, optimizer, sample_content, mock_filter_result):
        ...
        with patch.object(optimizer.content_filter, 'model', None):
            with patch.object(optimizer.content_filter, 'load_model'):
                with patch.object(optimizer.content_filter, 'filter_content', return_value=mock_filter_result):
                    await optimizer.optimize_content(sample_content)
    ...
    def test_cost_calculation_accuracy(self, optimizer, sample_content):
        ...
        # Cost calculation uses GPT-4o pricing as reference: $5.00 per 1M input tokens, $15.00 per 1M output tokens
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_large_content(self, optimizer):
        ...
        with patch.object(optimizer.content_filter, 'model', None):
            with patch.object(optimizer.content_filter, 'load_model'):
                with patch.object(optimizer.content_filter, 'filter_content') as mock_filter:
                    ...
                    result = await optimizer.optimize_content(large_content)
    ...
    @pytest.mark.asyncio
    async def test_optimize_content_empty_content(self, optimizer):
...
class TestConvenienceFunctions:
    ...
    async def test_optimize_article_content(self, sample_content):
        ...
        with patch('src.utils.llm_optimizer.llm_optimizer.optimize_content') as mock_optimize:
    ...
    def test_estimate_llm_cost(self, sample_content):
        """Test estimate_llm_cost convenience function."""
        with patch('src.utils.llm_optimizer.llm_optimizer.get_cost_estimate') as mock_estimate:
            mock_estimate.return_value = {'total_cost': 0.05}
            
            result = estimate_llm_cost(sample_content, use_filtering=True)
        
        assert result['total_cost'] == 0.05
        mock_estimate.assert_called_once_with(sample_content, True)

    def test_estimate_gpt4o_cost_backward_compat(self, sample_content):
        """Test estimate_gpt4o_cost backward compatibility alias."""
        with patch('src.utils.llm_optimizer.llm_optimizer.get_cost_estimate') as mock_estimate:
            mock_estimate.return_value = {'total_cost': 0.05}
    ...
    def test_get_optimization_stats(self):
        ...
        with patch('src.utils.llm_optimizer.llm_optimizer.get_optimization_stats') as mock_stats:
...
class TestLLMOptimizerIntegration:
    """Integration tests for LLMOptimizer."""
    ...
    async def test_end_to_end_optimization(self, sample_content):
        ...
        assert 'cost_savings' in cost_result
        assert 'pricing_note' in cost_result
        
        # Verify stats update
        assert stats['total_requests'] == 1
        assert stats['total_cost_savings'] > 0
        assert stats['total_tokens_saved'] > 0
        assert 'pricing_note' in stats
        ...
        with patch.object(optimizer.content_filter, 'model', None):
            with patch.object(optimizer.content_filter, 'load_model'):
                with patch.object(optimizer.content_filter, 'filter_content') as mock_filter:
                    ...
                    for i in range(3):
                        await optimizer.optimize_content(f"Content {i}")
