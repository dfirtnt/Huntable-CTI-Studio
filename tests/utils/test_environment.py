...
def assert_test_environment():
    ...
    if not test_db_url:
        raise RuntimeError("TEST_DATABASE_URL must be set for tests. Never use DATABASE_URL in test environment.")
    ...
    if "test" not in test_db_url.lower():
        raise RuntimeError(f"TEST_DATABASE_URL must contain 'test' in database name: {test_db_url}")
