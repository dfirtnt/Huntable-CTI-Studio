...
class PerformanceProfiler:
    ...
    def _generate_recommendations(self, metrics: PerformanceMetrics):
        ...
        if metrics.duration > self.config.slow_test_threshold:
            recommendations.append(f"Test is slow ({metrics.duration:.2f}s). Consider optimizing or mocking external dependencies.")
        
        # Memory recommendations
        if metrics.memory_usage > self.config.memory_threshold:
            recommendations.append(f"High memory usage ({metrics.memory_usage:.1f}MB). Check for memory leaks or large data structures.")
        
        if metrics.memory_peak > metrics.memory_usage * 2:
            recommendations.append(f"High memory peak ({metrics.memory_peak:.1f}MB). Consider reducing data size or using generators.")
        
        # CPU recommendations
        if metrics.cpu_percent > self.config.cpu_threshold:
            recommendations.append(f"High CPU usage ({metrics.cpu_percent:.1f}%). Consider optimizing algorithms or using async operations.")
        
        # Function call recommendations
        if metrics.function_calls > 10000:
            recommendations.append(f"Many function calls ({metrics.function_calls}). Consider reducing complexity or using caching.")
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        cpu_intensive_tests = sorted(
            self.profile_results,
            key=lambda m: m.cpu_percent,
            reverse=True
        )[:5]
...
def save_performance_report(filename: Optional[str] = None) -> Path:
...
def profile_performance(test_name: Optional[str] = None):
