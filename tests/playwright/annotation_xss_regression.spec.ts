 ...
 * renderExistingAnnotation ~L358) assembled `container.innerHTML = beforeText +
 * highlight.outerHTML + afterText` where beforeText/afterText are UNESCAPED
 * substrings of the article's raw scraped text. An article containing literal
 * markup like `<img src=x onerror=alert(1)>` would execute when an annotation
 * was created or when existing annotations rendered on page load.
 *
 ...
 * concatenation. (The mobile manager carried the same fix until it was removed
 * as unreferenced dead code with the observables-mode cleanup; its cases
 * ANN-XSS-002/005/006 went with it.)
 *
 * This test loads the JS file directly via addScriptTag on about:blank so
