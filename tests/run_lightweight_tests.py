...
def main():
    ...
    parser.add_argument(
        "suite", choices=["light", "full", "unit", "all", "critical", "rag", "embedding"], help="Test suite to run"
    )
    ...
    if args.coverage:
        base_cmd.extend(["--cov=src", "--cov-report=html:htmlcov", "--cov-report=term-missing"])
    ...
    suites = {
        ...
        "unit": {"cmd": base_cmd + ["-m", "unit", "tests/"], "description": "Unit Tests"},
        "critical": {
            "cmd": base_cmd
            + [
                "-m",
                "integration_light",
                "tests/integration/test_lightweight_integration.py::TestCriticalPathIntegration",
            ],
            "description": "Critical Path Integration Tests (lightweight)",
        },
        "all": {"cmd": base_cmd + ["tests/"], "description": "All Tests"},
        "rag": {
