...
async def test_cmdline_articles_expose_expected_items(async_client: httpx.AsyncClient):
    ...
    assert annotated, "No cmdline article has expected_items in the API response -- ground_truth.json merge is broken"
...
async def test_annotated_subagent_articles_have_expected_items_field(async_client: httpx.AsyncClient, subagent: str):
    ...
    """
    response = await async_client.get(f"/api/evaluations/subagent-eval-articles?subagent={subagent}")
    assert response.status_code == 200
    articles = response.json()["articles"]
    static_articles = [a for a in articles if a.get("from_static")]
    ...
    for article in static_articles:
        assert "expected_items" in article, f"{subagent}: article missing 'expected_items' key: {article['url']}"
