...
# ---------------------------------------------------------------------------
# psql stderr error extraction
# ---------------------------------------------------------------------------


def test_extract_psql_errors_finds_statement_errors():
    stderr = (
        "psql:/tmp/restore.sql:9231: ERROR:  could not resize shared memory segment to 63999680 bytes: No space left on device\n"
        "psql:/tmp/restore.sql:9232: STATEMENT:  CREATE INDEX ix_articles_embedding_hnsw ON public.articles USING hnsw (embedding vector_cosine_ops);\n"
    )
