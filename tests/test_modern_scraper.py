...
from src.core.modern_scraper import (
    ModernScraper, URLDiscovery, StructuredDataExtractor, LegacyScraper
)
...
class TestModernScraper:
    ...
    async def test_extract_article_jsonld_preference(self, modern_scraper, sample_source, mock_http_client):
        ...
        jsonld_data = {
            ...
            'articleBody': 'JSON-LD content'
        }

        with patch.object(modern_scraper.structured_extractor, 'extract_structured_data', return_value={'json-ld': [jsonld_data]}):
            with patch.object(modern_scraper.structured_extractor, 'find_article_jsonld', return_value=jsonld_data):
                with patch.object(modern_scraper.structured_extractor, 'extract_from_jsonld', return_value={
            ...
            with patch('src.utils.content.MetadataExtractor.extract_authors', return_value=['Test Author']):
                with patch('src.utils.content.MetadataExtractor.extract_tags', return_value=['test']):
                    with patch('src.utils.content.MetadataExtractor.extract_canonical_url', return_value='https://example.com/article'):
                        with patch('src.utils.content.MetadataExtractor.extract_meta_tags', return_value={'description': 'Test description'}):
                            with patch('src.utils.content.MetadataExtractor.extract_opengraph', return_value={}):
                                data = modern_scraper._extract_with_selectors(soup, sample_source, 'https://example.com/article')
