...
def _make_wp_post(
    *,
    ...
    if date_gmt is None:
        # Relative to "now" so the post always falls inside the default
        # 180-day lookback window regardless of when the suite runs.
        date_gmt = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S")
    return {
        "id": post_id,
