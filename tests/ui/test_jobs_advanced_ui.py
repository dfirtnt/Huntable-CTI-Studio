        ...
        def handle_route(route):
            if "/api/jobs/status" in route.request.url:
                mock_response = {
                    "status": "ok",
                    "worker_stats": {
                        "worker1@hostname": {"pool": {"processes": [1, 2, 3]}, "total": {"SUCCESS": 10, "FAILURE": 2}}
                    },
                    "active_tasks": {},
                }
                route.fulfill(status=200, body=json.dumps(mock_response), headers={"Content-Type": "application/json"})
    ...
    def test_queue_status_display(self, page: Page):
        ...
        def handle_route(route):
            if "/api/jobs/queues" in route.request.url:
                mock_response = {"queues": {"default": 5, "high_priority": 2}}
                route.fulfill(status=200, body=json.dumps(mock_response), headers={"Content-Type": "application/json"})
        ...
        def handle_route(route):
            if "/api/jobs/status" in route.request.url:
                mock_response = {
                    ...
                    "active_tasks": {
                        "worker1@hostname": [
                            {"id": "task-123", "name": "tasks.collect_source", "time_start": 1234567890, "args": [1]}
                        ]
                    },
                }
                route.fulfill(status=200, body=json.dumps(mock_response), headers={"Content-Type": "application/json"})
