...
class TestErrorDisplay:
    ...
    def test_error_notification_display(self, page: Page):
        ...
        def handle_route(route):
            if "/api/" in route.request.url:
                route.fulfill(status=500, body='{"detail": "Error message"}', headers={"Content-Type": "application/json"})
            else:
                route.continue_()
    ...
    def test_error_message_clarity(self, page: Page):
        ...
        def handle_route(route):
            if "/api/" in route.request.url:
                route.fulfill(status=400, body='{"detail": "Invalid request parameters"}', headers={"Content-Type": "application/json"})
            else:
                route.continue_()
...
class TestRateLimitHandling:
    ...
    def test_rate_limit_error(self, page: Page):
        ...
        def handle_route(route):
            if "/api/" in route.request.url:
                route.fulfill(status=429, body='{"detail": "Rate limit exceeded"}', headers={"Content-Type": "application/json"})
            else:
                route.continue_()
