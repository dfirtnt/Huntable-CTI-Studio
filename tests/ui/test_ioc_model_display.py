                ...
                if ioc_modal.is_visible(timeout=10000):
                    ...
                    if generation_area.is_visible():
                        ...
                        if llm_checkbox.is_visible():
                            ...
                            if not is_checked:
                                if "Generated by" in generation_text or any(
                                    model in generation_text.lower()
                                    for model in ["gpt", "claude", "anthropic"]
                                ):
                                    print(
                                        "ERROR: Shows model info when LLM validation is disabled!"
                                    )
                                    assert False, (
                                        f"Shows model info when LLM validation is disabled! Text: {generation_text}"
                                    )
                                else:
                                    print(
                                        "SUCCESS: No model info shown when LLM validation is disabled!"
                                    )
                                ...
                                print(
                                    "LLM validation enabled - would show model info on regeneration"
                                )
                                ...
                                for test_model in test_models:
                                    ...
                                    model_field = test_model.get(
                                        "ai_model"
                                    ) or test_model.get("model_name")
                                    expected = test_model["expected"]
                                    print(
                                        f"Model field '{model_field}' would display as '{expected}'"
                                    )

                                print(
                                    "SUCCESS: Model detection logic handles different AI models!"
                                )
...
def test_model_field_mapping():
    ...
    test_cases = [
        ...
        (
            {"ai_model": "anthropic", "model_name": "gpt-4"},
            "anthropic",
        ),  # ai_model takes precedence
